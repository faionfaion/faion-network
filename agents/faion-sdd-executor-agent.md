...
1. Find feature dir:
   ...
   - Also check: .product/in-progress/, .product/todo/, .product/backlog/
   - Path: {project_root}/{docs_dir}/{state}/{feature_name}/
...
3. Read project context:
   - constitution.md (if exists in docs dir)
   - CLAUDE.md / AGENTS.md (project instructions)

4. If feature is in todo/, move it to in-progress/:
   mv {docs_dir}/todo/{feature}/ {docs_dir}/in-progress/{feature}/
```
...
| **Docs** | Only docs changes | Direct edit, no git |
...
mv {docs_dir}/in-progress/{feature}/ {docs_dir}/done/{feature}/
```
