---
name: faion-task-executor-agent
description: ""
model: opus
...
Use these skills based on task context:

### Core
| Skill | When to Use |
|-------|-------------|
| `faion-sdd-domain-skill` | SDD workflow, task structure, quality gates |

### Development
| Skill | When to Use |
|-------|-------------|
| `faion-dev-django-skill` | Django models, views, services, DRF, Celery |
| `faion-dev-frontend-skill` | React/Vue components, Storybook, design systems |
| `faion-backend-skill` | Go, Ruby, PHP, Java, C#, Rust patterns |

### Testing
| Skill | When to Use |
|-------|-------------|
| `faion-testing-skill` | pytest, Jest, Vitest, coverage, TDD patterns |

### API & Integration
| Skill | When to Use |
|-------|-------------|
| `faion-api-skill` | REST, GraphQL, OpenAPI specs, API design |
| `faion-browser-automation-skill` | Puppeteer, Playwright, E2E testing |

### How to Detect Which Skill to Use

1. **Read constitution.md** → detect tech stack
2. **Match stack to skill:**
   - `Django`, `DRF`, `Celery` → `faion-dev-django-skill`
   - `React`, `Vue`, `Storybook` → `faion-dev-frontend-skill`
   - `Go`, `Ruby`, `PHP` → `faion-backend-skill`
   - `pytest`, `Jest`, `coverage` → `faion-testing-skill`
