...
Validate an artefact produced by the rust-tokio-async methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/rust-tokio-async.json", "type": "object", "required": ["runtime_flavour", "worker_threads", "blocking_offload", "channel_policy", "timeout_default_ms"], "properties": {"runtime_flavour": {"type": "string", "enum": ["multi_thread", "current_thread"]}, "worker_threads": {"type": "integer", "minimum": 1}, "blocking_offload": {"type": "string", "enum": ["spawn_blocking", "rayon", "none"]}, "channel_policy": {"type": "object", "required": ["bounded", "default_capacity"]}, "timeout_default_ms": {"type": "integer", "minimum": 1}, "shutdown_signal": {"type": "string"}}}')
