#!/usr/bin/env python3
"""validate-architecture-decision-records.py — validate a Architecture Decision Records artefact JSON against the output contract.

Usage:
  validate-architecture-decision-records.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('adr_id', 'title', 'status', 'context', 'decision', 'alternatives', 'consequences', 'supersedes', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['status not in allowed set', 'alternatives length < 2', 'consequences.positive empty OR consequences.negative empty', 'edits to ADR with status==Accepted']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-architecture-decision-records.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
