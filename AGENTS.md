...
- **`INDEX.xml` entries are added by hand**: one `<methodology slug tier path>` block with a `<summary>`, kept alphabetical, with the `count=` attribute bumped to match. The three frontmatter-reading builders that used to sit in `scripts/` were deleted 2026-08-14 — they read metadata F-067 moved to `meta.json`, so they emitted `count="0"` and silently emptied the index they targeted. `regen-domains-xml.py` is the working `meta.json`-based generator, but it is **not yet a drop-in replacement**: it filters L2 entries by `status`, so a `status: "draft"` methodology drops out of the index while `tier-manifest.json` still carries it. Settle that before trusting `--write`.
