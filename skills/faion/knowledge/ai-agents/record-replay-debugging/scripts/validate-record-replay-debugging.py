#!/usr/bin/env python3
"""Validate a record-replay-debugging spec JSON.

Usage: validate-record-replay-debugging.py <file.json> | --self-test | --help
...
DEC = ["record_mode", "replay_mode", "redaction_policy", "ci_recordings"]
DRV = ["failure_reproduction_difficulty", "production_failures_per_month", "tool_count"]
...
def _self_test() -> int:
    good = {
        ...
        "audit": {"rule_refs": ["r1-record-all-calls"]},
    }
