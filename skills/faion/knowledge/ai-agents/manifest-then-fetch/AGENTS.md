...
### `templates/tool-inventory.yaml`

```yaml
tool_outputs_avg_bytes: TODO          # fill with concrete value per the driver definition
store_backend: TODO          # fill with concrete value per the driver definition
sensitivity: TODO          # fill with concrete value per the driver definition
```

### `templates/manifest_then_fetch.py`

```python
"""Reference artefact for manifest-then-fetch. Self-test only verifies the scaffold compiles."""
...
def _self_test() -> int:
    out = build({"slug": "manifest-then-fetch", "version": "2.0.0"})
    return 0 if out["slug"] == "manifest-then-fetch" else 1
