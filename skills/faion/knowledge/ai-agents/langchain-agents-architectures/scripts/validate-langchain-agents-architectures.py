#!/usr/bin/env python3
"""validate-langchain-agents-architectures.py

Purpose:
    Validate an agent result JSON against the schema in content/02-output-contract.xml.
...
def validate(obj: dict) -> list[str]:
    errs: list[str] = []
    if not isinstance(obj.get("final_answer"), str) or not obj["final_answer"]:
        errs.append("final_answer: required non-empty string")
    iters = obj.get("iterations")
    if not isinstance(iters, int) or iters < 0:
        errs.append("iterations: required non-negative int")
    sid = obj.get("session_id")
    if not isinstance(sid, str) or len(sid) < 1:
        errs.append("session_id: required non-empty string")
    return errs
