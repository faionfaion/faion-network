#!/usr/bin/env python3
"""validate-fixed-price-vs-tm-cr-pricing-playbook.py — validate a Fixed-Price vs T&M Change-Request Pricing Playbook artefact JSON.

Usage:
  validate-fixed-price-vs-tm-cr-pricing-playbook.py <artefact.json>
...
ID_RE = re.compile(r"^fpcr-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^fpcr-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "fpcr-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/fixed-price-vs-tm-cr-pricing-playbook.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-fixed-price-vs-tm-cr-pricing-playbook.py <artefact.json>\n")
        return 2
    try:
