#!/usr/bin/env python3
"""validate-ai-enabled-business-analysis.py — validate a AI-Enabled Business Analysis (Business Analyst) artefact JSON.

Usage:
  validate-ai-enabled-business-analysis.py <artefact.json>
...
ID_RE = re.compile(r"^aiba2-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^aiba2-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "aiba2-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/ai-enabled-business-analysis.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-ai-enabled-business-analysis.py <artefact.json>\n")
        return 2
    try:
