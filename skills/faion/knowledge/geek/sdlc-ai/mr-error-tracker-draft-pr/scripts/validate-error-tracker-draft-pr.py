#!/usr/bin/env python3
"""validate-error-tracker-draft-pr.py — validate the playbook-step artefact for the mr-error-tracker-draft-pr methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/mr-error-tracker-draft-pr.json", "type": "object", "required": ["tracker", "thresholds", "structured_signal_keys", "draft_only", "regression_test_required", "codeowners_present"], "properties": {"tracker": {"enum": ["sentry", "bugsink", "rollbar", "starsling"]}, "thresholds": {"type": "object", "required": ["event_count_min", "fixability_score_min"], "properties": {"event_count_min": {"type": "integer", "minimum": 1}, "fixability_score_min": {"type": "number", "minimum": 0, "maximum": 1}}}, "structured_signal_keys": {"type": "array", "minItems": 5, "uniqueItems": true, "items": {"enum": ["stack_trace", "breadcrumbs", "distributed_trace", "linked_repo", "recent_diffs"]}}, "draft_only": {"const": true}, "regression_test_required": {"const": true}, "codeowners_present": {"const": true}}}""")
VALID_FIXTURE = json.loads(r"""{"tracker": "sentry", "thresholds": {"event_count_min": 50, "fixability_score_min": 0.7}, "structured_signal_keys": ["stack_trace", "breadcrumbs", "distributed_trace", "linked_repo", "recent_diffs"], "draft_only": true, "regression_test_required": true, "codeowners_present": true}""")
INVALID_FIXTURE = json.loads(r"""{"tracker": "homemade", "thresholds": {"event_count_min": 0}, "draft_only": false, "regression_test_required": false, "structured_signal_keys": [], "codeowners_present": false}""")
