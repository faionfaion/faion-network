#!/usr/bin/env python3
"""validate-shellcheck-hadolint-iac-floor.py — validate the config artefact for the lint-shellcheck-hadolint-iac-floor methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lint-shellcheck-hadolint-iac-floor.json", "type": "object", "required": ["linters_present", "rc_files_committed", "pre_commit_present", "ci_present", "ci_fails_on_violation"], "properties": {"linters_present": {"type": "array", "minItems": 1, "uniqueItems": true, "items": {"enum": ["shellcheck", "hadolint", "yamllint", "actionlint", "tflint"]}}, "rc_files_committed": {"const": true}, "pre_commit_present": {"const": true}, "ci_present": {"const": true}, "ci_fails_on_violation": {"const": true}, "exclusions_justified": {"const": true}}}""")
VALID_FIXTURE = json.loads(r"""{"linters_present": ["shellcheck", "hadolint", "yamllint", "actionlint", "tflint"], "rc_files_committed": true, "pre_commit_present": true, "ci_present": true, "ci_fails_on_violation": true, "exclusions_justified": true}""")
INVALID_FIXTURE = json.loads(r"""{"linters_present": [], "rc_files_committed": false, "pre_commit_present": false, "ci_present": false, "ci_fails_on_violation": false}""")
