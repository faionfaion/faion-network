#!/usr/bin/env python3
"""validate-test-self-healing-locators-audited.py

Validate the config artefact for the test-self-healing-locators-audited methodology against the schema
...
REQUIRED = ['framework', 'match_dimensions', 'heal_diff_path', 'auto_apply_to_next_run', 'blocklisted_pages', 'rollback_window_days', 'scope']
SCHEMA_PROPS = {'framework': {'enum': ['playwright', 'cypress', 'selenium', 'webdriverio']}, 'match_dimensions': {'minItems': 2}, 'auto_apply_to_next_run': {'const': False}, 'blocklisted_pages': {'minItems': 1}, 'rollback_window_days': {'minimum': 1, 'maximum': 30}, 'scope': {'enum': ['changed-only', 'whole-suite']}}
...
OK = {'framework': 'playwright', 'match_dimensions': ['role', 'accessible_name'], 'heal_diff_path': 'tests/e2e/healed-selectors.diff', 'auto_apply_to_next_run': False, 'blocklisted_pages': ['/checkout', '/login', '/account/settings'], 'rollback_window_days': 7, 'scope': 'changed-only'}
BAD = {'framework': 'playwright', 'match_dimensions': ['css'], 'auto_apply_to_next_run': True, 'blocklisted_pages': [], 'rollback_window_days': 0, 'scope': 'whole-suite'}
