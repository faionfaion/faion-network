#!/usr/bin/env python3
"""validate-test-golden-master-legacy-rewrite.py

Validate the code artefact for the test-golden-master-legacy-rewrite methodology against the schema
...
REQUIRED = ['module_under_rewrite', 'corpus_path', 'pair_count', 'sampling_strategy', 'canonicalize_fn', 'approved_diffs_path', 'pass_criterion']
SCHEMA_PROPS = {'pair_count': {'minimum': 10}, 'sampling_strategy': {'enum': ['top-n+tail+fuzz', 'stratified', 'production-mirror']}, 'pass_criterion': {'enum': ['byte-equal', 'approved-row-diff']}}
...
OK = {'module_under_rewrite': 'src/legacy/pricing.py', 'corpus_path': 'tests/golden/pricing/', 'pair_count': 1200, 'sampling_strategy': 'top-n+tail+fuzz', 'canonicalize_fn': 'tests/golden/canon.py:canonicalize_pricing', 'approved_diffs_path': 'tests/golden/approved_diffs.yaml', 'pass_criterion': 'byte-equal'}
BAD = {'module_under_rewrite': 'src/legacy/pricing.py', 'pair_count': 3, 'pass_criterion': 'fuzzy'}
