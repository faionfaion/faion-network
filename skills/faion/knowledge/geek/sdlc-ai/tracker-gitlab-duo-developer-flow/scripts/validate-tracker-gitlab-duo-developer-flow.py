#!/usr/bin/env python3
"""validate-tracker-gitlab-duo-developer-flow.py

Validate the config artefact for the tracker-gitlab-duo-developer-flow methodology against the schema
...
REQUIRED = ['gitlab_version', 'flows', 'approval_rules_respected', 'audit_log_emitted']
SCHEMA_PROPS = {'gitlab_version': {'pattern': '^\\d+\\.\\d+'}, 'flows': {'minItems': 1}, 'approval_rules_respected': {'const': True}, 'audit_log_emitted': {'const': True}}
...
OK = {'gitlab_version': '18.9', 'flows': [{'name': 'developer', 'trigger_label': 'agent:implement', 'task_type': 'issue-to-mr'}, {'name': 'code-review', 'trigger_label': 'agent:review', 'task_type': 'review'}, {'name': 'ci-cd', 'trigger_label': 'agent:ci-fix', 'task_type': 'build-fix'}], 'approval_rules_respected': True, 'audit_log_emitted': True}
BAD = {'gitlab_version': '18.5', 'flows': [{'name': 'developer', 'trigger_label': 'auto', 'task_type': 'issue-to-mr'}], 'approval_rules_respected': False, 'audit_log_emitted': False}
