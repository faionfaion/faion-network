#!/usr/bin/env python3
"""validate-uv-lockfile-floor.py

Validate the config artefact for the uv-lockfile-floor methodology against the schema
...
REQUIRED = ['pyproject_path', 'requires_python', 'uv_lock_present', 'agents_md_snippet_present', 'other_resolvers_absent']
SCHEMA_PROPS = {'pyproject_path': {'pattern': 'pyproject\\.toml$'}, 'requires_python': {'pattern': '^[<>=~,.\\d ]+$'}, 'uv_lock_present': {'const': True}, 'agents_md_snippet_present': {'const': True}, 'other_resolvers_absent': {'const': True}}
...
OK = {'pyproject_path': 'pyproject.toml', 'requires_python': '>=3.12,<3.13', 'uv_lock_present': True, 'agents_md_snippet_present': True, 'other_resolvers_absent': True}
BAD = {'pyproject_path': 'pyproject.toml', 'requires_python': '>=3.10', 'uv_lock_present': False, 'agents_md_snippet_present': False, 'other_resolvers_absent': False}
