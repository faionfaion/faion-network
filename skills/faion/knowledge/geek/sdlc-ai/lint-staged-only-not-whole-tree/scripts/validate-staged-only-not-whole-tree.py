#!/usr/bin/env python3
"""validate-staged-only-not-whole-tree.py — validate the config artefact for the lint-staged-only-not-whole-tree methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lint-staged-only-not-whole-tree.json", "type": "object", "required": ["framework", "staged_filter_native", "whole_tree_ci_job"], "properties": {"framework": {"enum": ["pre-commit", "lefthook", "husky+lint-staged"]}, "staged_filter_native": {"const": true}, "whole_tree_ci_job": {"const": true}, "linters_accept_paths": {"const": true}}}""")
VALID_FIXTURE = json.loads(r"""{"framework": "lefthook", "staged_filter_native": true, "whole_tree_ci_job": true, "linters_accept_paths": true}""")
INVALID_FIXTURE = json.loads(r"""{"framework": "custom-shell-script", "staged_filter_native": false, "whole_tree_ci_job": false}""")
