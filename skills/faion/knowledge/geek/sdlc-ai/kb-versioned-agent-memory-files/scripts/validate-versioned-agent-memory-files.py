#!/usr/bin/env python3
"""validate-versioned-agent-memory-files.py — validate the config artefact for the kb-versioned-agent-memory-files methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/kb-versioned-agent-memory-files.json", "type": "object", "required": ["bundle_path", "files", "load_on_start", "secret_scan_hook"], "properties": {"bundle_path": {"type": "string"}, "files": {"type": "array", "minItems": 4, "maxItems": 4, "items": {"enum": ["decisions.md", "patterns.md", "mistakes.md", "session.md"]}, "uniqueItems": true}, "load_on_start": {"const": true}, "secret_scan_hook": {"const": true}, "prune_strategy": {"enum": ["quarterly", "size-cap", "never"]}}}""")
VALID_FIXTURE = json.loads(r"""{"bundle_path": ".aidocs/memory/", "files": ["decisions.md", "patterns.md", "mistakes.md", "session.md"], "load_on_start": true, "secret_scan_hook": true, "prune_strategy": "quarterly"}""")
INVALID_FIXTURE = json.loads(r"""{"bundle_path": ".memory/", "files": ["notes.md"], "load_on_start": false, "secret_scan_hook": false}""")
