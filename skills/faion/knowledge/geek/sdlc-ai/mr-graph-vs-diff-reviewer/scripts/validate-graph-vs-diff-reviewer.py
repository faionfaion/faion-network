#!/usr/bin/env python3
"""validate-graph-vs-diff-reviewer.py — validate the decision-record artefact for the mr-graph-vs-diff-reviewer methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/mr-graph-vs-diff-reviewer.json", "type": "object", "required": ["decision_record_path", "candidates_evaluated", "eval_pr_count", "chosen_architecture", "scorecard"], "properties": {"decision_record_path": {"type": "string"}, "candidates_evaluated": {"type": "array", "minItems": 2, "items": {"type": "object", "required": ["name", "architecture"], "properties": {"name": {"type": "string"}, "architecture": {"enum": ["diff-only", "graph-indexed"]}}}}, "eval_pr_count": {"type": "integer", "minimum": 5}, "chosen_architecture": {"enum": ["diff-only", "graph-indexed"]}, "scorecard": {"type": "object", "required": ["precision", "recall", "coverage_pct"], "properties": {"precision": {"type": "number", "minimum": 0, "maximum": 1}, "recall": {"type": "number", "minimum": 0, "maximum": 1}, "coverage_pct": {"type": "number", "minimum": 0, "maximum": 100}}}}}""")
VALID_FIXTURE = json.loads(r"""{"decision_record_path": "docs/decisions/0007-ai-reviewer.md", "candidates_evaluated": [{"name": "CodeRabbit", "architecture": "diff-only"}, {"name": "Greptile", "architecture": "graph-indexed"}], "eval_pr_count": 8, "chosen_architecture": "graph-indexed", "scorecard": {"precision": 0.78, "recall": 0.62, "coverage_pct": 84}}""")
INVALID_FIXTURE = json.loads(r"""{"decision_record_path": "", "candidates_evaluated": [{"name": "X", "architecture": "diff-only"}], "eval_pr_count": 2, "chosen_architecture": "neither"}""")
