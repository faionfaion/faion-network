#!/usr/bin/env python3
"""validate-csharp-roslyn-analyzer-errors.py — validate the config artefact for the lang-csharp-roslyn-analyzer-errors methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lang-csharp-roslyn-analyzer-errors.json", "type": "object", "required": ["editorconfig_present", "escalated_ids", "treat_warnings_as_errors", "public_api_analyzer_libs", "ci_blocks_on_build"], "properties": {"editorconfig_present": {"const": true}, "escalated_ids": {"type": "array", "minItems": 5, "uniqueItems": true, "items": {"type": "string", "pattern": "^(CS|CA|VSTHRD|RS)[0-9]+$"}, "contains": {"const": "CS8602"}}, "treat_warnings_as_errors": {"const": true}, "public_api_analyzer_libs": {"type": "array", "items": {"type": "string"}}, "ci_blocks_on_build": {"const": true}, "pragma_justification_required": {"const": true}}}""")
VALID_FIXTURE = json.loads(r"""{"editorconfig_present": true, "escalated_ids": ["CS8602", "CS8618", "CS8625", "CA2007", "VSTHRD100", "VSTHRD002", "CA2016", "RS0016", "RS0017"], "treat_warnings_as_errors": true, "public_api_analyzer_libs": ["src/Foo.Lib", "src/Bar.Lib"], "ci_blocks_on_build": true, "pragma_justification_required": true}""")
INVALID_FIXTURE = json.loads(r"""{"editorconfig_present": false, "escalated_ids": ["CS001"], "treat_warnings_as_errors": false}""")
