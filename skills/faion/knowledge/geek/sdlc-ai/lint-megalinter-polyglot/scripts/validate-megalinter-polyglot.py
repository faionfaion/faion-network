#!/usr/bin/env python3
"""validate-megalinter-polyglot.py — validate the config artefact for the lint-megalinter-polyglot methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lint-megalinter-polyglot.json", "type": "object", "required": ["mega_linter_yml", "flavour", "image_sha", "sarif_upload", "languages_count"], "properties": {"mega_linter_yml": {"type": "string"}, "flavour": {"enum": ["all", "ci_light", "cupcake", "documentation", "dotnet", "go", "java", "javascript", "python", "ruby", "rust", "salesforce", "security", "swift", "terraform"]}, "image_sha": {"type": "string", "pattern": "^sha256:[a-f0-9]{64}$"}, "sarif_upload": {"const": true}, "languages_count": {"type": "integer", "minimum": 3}}}""")
VALID_FIXTURE = json.loads(r"""{"mega_linter_yml": ".mega-linter.yml", "flavour": "all", "image_sha": "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "sarif_upload": true, "languages_count": 5}""")
INVALID_FIXTURE = json.loads(r"""{"mega_linter_yml": ".mega-linter.yml", "flavour": "ci_light", "image_sha": "latest", "sarif_upload": false, "languages_count": 1}""")
