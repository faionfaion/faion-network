---
slug: rfc-template-product-dev-team
tier: geek
group: dev
domain: dev
version: 1.0.0
...
maintainers: [faion]
content_id: "5409169d3f4b096e"
summary: RFC Template Product DEV Team — pinned template for the product-dev team: fixed shape + named owner + evidence anchors + outcome review, so rfc → production: feature delivery (3-6 weeks) stops being folklore and starts being a reviewable operating tool.
tags: [dev, geek, template, rfc, product, team]
---
# RFC Template Product DEV Team

## Summary

**One-sentence:** RFC Template Product DEV Team — pinned template for the product-dev team: fixed shape + named owner + evidence anchors + outcome review, so rfc → production: feature delivery (3-6 weeks) stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the product-dev team runs rfc → production: feature delivery (3-6 weeks) on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Architecture methodologies cover patterns but there is no canonical RFC template tying spec → C4 → quality attributes → approval-token. Product teams reinvent it per repo. `rfc-template-product-dev-team` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the product-dev team starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored template committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p6-product-dev-team/RFC → Production: feature delivery (3-6 weeks)`
...
| Methodology | Why |
|-------------|-----|
| `geek/dev/<upstream-canon>` | Upstream concept; this methodology consumes its output without re-teaching it. |
