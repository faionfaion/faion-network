---
slug: pr-mentoring-session-protocol
tier: geek
group: dev
domain: dev
version: 1.0.0
...
maintainers: [faion]
content_id: "be6081a7019a120b"
summary: PR Mentoring Session Protocol — pinned protocol for the product-dev team: fixed shape + named owner + evidence anchors + outcome review, so pr review with junior present (mentoring loop) stops being folklore and starts being a reviewable operating tool.
tags: [dev, geek, protocol, mentoring, session]
---
# PR Mentoring Session Protocol

## Summary

**One-sentence:** PR Mentoring Session Protocol — pinned protocol for the product-dev team: fixed shape + named owner + evidence anchors + outcome review, so pr review with junior present (mentoring loop) stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the product-dev team runs pr review with junior present (mentoring loop) on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Existing code-review pages cover review mechanics, not the senior+junior live narration format that actually transfers judgment. P6 teams cite onboarding as a #1 pain. `pr-mentoring-session-protocol` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the product-dev team starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored protocol committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p6-product-dev-team/PR review with junior present (mentoring loop)`
