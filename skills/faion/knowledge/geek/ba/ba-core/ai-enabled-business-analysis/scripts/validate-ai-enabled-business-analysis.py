#!/usr/bin/env python3
"""validate-ai-enabled-business-analysis.py — validate a AI-Enabled Business Analysis artefact JSON.
...
ID_RE = re.compile(r"^aiba-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^aiba-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "aiba-self-test-001",
        "owner": "alex@acme.com",
