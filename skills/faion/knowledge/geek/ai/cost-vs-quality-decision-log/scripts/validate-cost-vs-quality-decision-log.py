#!/usr/bin/env python3
"""validate-cost-vs-quality-decision-log.py — validate a Cost-vs-Quality Decision Log artefact JSON.

Usage:
  validate-cost-vs-quality-decision-log.py <artefact.json>
...
ID_RE = re.compile(r"^cvql-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^cvql-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "cvql-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/cost-vs-quality-decision-log.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-cost-vs-quality-decision-log.py <artefact.json>\n")
        return 2
    try:
