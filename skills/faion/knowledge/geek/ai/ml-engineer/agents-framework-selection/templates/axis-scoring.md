<!--
purpose: 3-axis scoring rubric (graph-vs-role, observability, deployment)
consumes: Inputs declared in `AGENTS.md` Prerequisites.
produces: Filled artefact for `agents-framework-selection` matching `content/02-output-contract.xml`.
depends-on: `content/01-core-rules.xml`, `scripts/validate-agents-framework-selection.py`.
