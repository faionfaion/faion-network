#!/usr/bin/env python3
"""validate-drift-slo-thresholds-cookbook.py — validate a Drift SLO Thresholds Cookbook artefact JSON.

Usage:
  validate-drift-slo-thresholds-cookbook.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^dst-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "dst-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/drift-slo-thresholds-cookbook.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-drift-slo-thresholds-cookbook.py <artefact.json>\n")
        return 2
    try:
