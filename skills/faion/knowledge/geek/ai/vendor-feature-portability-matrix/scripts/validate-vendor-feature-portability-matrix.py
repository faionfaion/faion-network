#!/usr/bin/env python3
"""validate-vendor-feature-portability-matrix.py — validate a Vendor Feature Portability Matrix artefact JSON.

Usage:
  validate-vendor-feature-portability-matrix.py <artefact.json>
...
ID_RE = re.compile(r"^vfpm-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^vfpm-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "vfpm-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/vendor-feature-portability-matrix.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-vendor-feature-portability-matrix.py <artefact.json>\n")
        return 2
    try:
