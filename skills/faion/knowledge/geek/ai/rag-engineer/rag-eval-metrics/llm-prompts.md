...
### Design Pattern
```
Prompt: Design a rag-eval-metrics system that:
1. Handles {use_case}
...
### Code Generation
```
Prompt: Generate Python code for {specific_task} using rag-eval-metrics.
...
Using rag-eval-metrics, what's the issue and how to fix it?
```
...
### Test Generation
```
Prompt: Generate comprehensive tests for {function_name} that handles rag-eval-metrics.
...
### Tutorial Generation
```
Prompt: Create a tutorial for implementing rag-eval-metrics from scratch.
...
### Performance
```
Prompt: How can I improve the performance of {metric} in {language} for rag-eval-metrics?
...
### Cost Optimization
```
Prompt: How to reduce API costs while maintaining quality for rag-eval-metrics?
