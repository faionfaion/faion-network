#!/usr/bin/env python3
"""validate-weekly-llm-cost-review-template.py — validate a Weekly LLM Cost Review Template artefact JSON.

Usage:
  validate-weekly-llm-cost-review-template.py <artefact.json>
...
ID_RE = re.compile(r"^wlcr-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^wlcr-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "wlcr-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/weekly-llm-cost-review-template.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-weekly-llm-cost-review-template.py <artefact.json>\n")
        return 2
    try:
