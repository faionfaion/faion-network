#!/usr/bin/env python3
"""validate-verbatim-to-eval-row-recipe.py — validate a Verbatim To Eval Row Recipe artefact JSON.

Usage:
  validate-verbatim-to-eval-row-recipe.py <artefact.json>
...
ID_RE = re.compile(r"^vter-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^vter-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "vter-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/verbatim-to-eval-row-recipe.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-verbatim-to-eval-row-recipe.py <artefact.json>\n")
        return 2
    try:
