#!/usr/bin/env python3
"""validate-streaming-response-ux.py — validate a Streaming Response UX artefact JSON.

Usage:
  validate-streaming-response-ux.py <artefact.json>
...
ID_RE = re.compile(r"^srux-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^srux-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "srux-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/streaming-response-ux.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-streaming-response-ux.py <artefact.json>\n")
        return 2
    try:
