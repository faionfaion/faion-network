#!/usr/bin/env python3
"""validate-tool-call-schema-design-checklist.py — validate a Tool-Call Schema Design Checklist artefact JSON.

Usage:
  validate-tool-call-schema-design-checklist.py <artefact.json>
...
ID_RE = re.compile(r"^tcsd-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^tcsd-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "tcsd-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/tool-call-schema-design-checklist.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-tool-call-schema-design-checklist.py <artefact.json>\n")
        return 2
    try:
