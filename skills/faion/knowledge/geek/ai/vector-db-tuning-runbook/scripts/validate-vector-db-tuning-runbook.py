#!/usr/bin/env python3
"""validate-vector-db-tuning-runbook.py — validate a Vector DB Tuning Runbook artefact JSON.

Usage:
  validate-vector-db-tuning-runbook.py <artefact.json>
...
ID_RE = re.compile(r"^vdbt-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^vdbt-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "vdbt-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/vector-db-tuning-runbook.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-vector-db-tuning-runbook.py <artefact.json>\n")
        return 2
    try:
