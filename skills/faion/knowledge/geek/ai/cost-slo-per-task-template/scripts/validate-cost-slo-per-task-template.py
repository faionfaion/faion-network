#!/usr/bin/env python3
"""validate-cost-slo-per-task-template.py — validate a Cost-SLO Per-Task Template artefact JSON.

Usage:
  validate-cost-slo-per-task-template.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^cst-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "cst-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/cost-slo-per-task-template.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-cost-slo-per-task-template.py <artefact.json>\n")
        return 2
    try:
