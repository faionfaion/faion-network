...
summary: Declares finite answer sets as Literal/enum so constrained decoders mask non-enum tokens to zero probability at sampling time, eliminating hallucinated labels and giving the highest accuracy-per-token ratio of any structured-output trick.
