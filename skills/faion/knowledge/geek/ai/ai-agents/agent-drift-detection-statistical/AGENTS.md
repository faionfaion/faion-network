...
maintainers: [faion]
summary: Evaluation method for agent drift detection statistical that defines test set, scoring function, pass/fail thresholds, and regression rules so quality is comparable across runs.
...
**One-sentence:** Evaluation method for agent drift detection statistical that defines test set, scoring function, pass/fail thresholds, and regression rules so quality is comparable across runs.

**One-paragraph:** Evaluation method for agent drift detection statistical that defines test set, scoring function, pass/fail thresholds, and regression rules so quality is comparable across runs. Eval scores drift across model upgrades, retrieval-index updates, prompt edits. No methodology defines the statistical machinery: per-eval pass-rate control charts, KL-div on output distributions, judge-score regression with confidence intervals. Without it P7 ships and prays.

## Applies If (ALL must hold)

- You are stabilizing or comparing the AI feature behavior described by agent drift detection statistical across model, prompt, or retrieval versions.
