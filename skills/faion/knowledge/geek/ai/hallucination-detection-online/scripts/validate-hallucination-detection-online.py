#!/usr/bin/env python3
"""validate-hallucination-detection-online.py — validate a Hallucination Detection — Online artefact JSON.

Usage:
  validate-hallucination-detection-online.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^hdo-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "hdo-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/hallucination-detection-online.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-hallucination-detection-online.py <artefact.json>\n")
        return 2
    try:
