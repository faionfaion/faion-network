...
summary: Prompt Portability Audit — pinned audit for the ML engineer: fixed shape + named owner + evidence anchors + outcome review, so migrate llm provider / model generation stops being folklore and starts being a reviewable operating tool.
tags: [ai, geek, audit, prompt, portability]
---
# Prompt Portability Audit

## Summary

**One-sentence:** Prompt Portability Audit — pinned audit for the ML engineer: fixed shape + named owner + evidence anchors + outcome review, so migrate llm provider / model generation stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In AI / agent engineering, the ML engineer runs migrate llm provider / model generation on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Prompts often encode model-specific assumptions (Claude's XML, OpenAI's system messages). Need a methodology to audit a prompt suite for portability before cross-vendor migration. `prompt-portability-audit` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the ML engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored audit committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-ml-engineer/Migrate LLM provider / model generation`
