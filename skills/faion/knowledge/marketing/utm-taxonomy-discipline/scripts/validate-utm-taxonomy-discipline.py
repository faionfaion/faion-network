#!/usr/bin/env python3
"""validate-utm-taxonomy-discipline.py — validate a UTM Taxonomy Discipline artefact JSON against the output contract.

Usage:
  validate-utm-taxonomy-discipline.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'source_vocabulary', 'medium_vocabulary', 'campaign_id_pattern', 'validation_regex', 'link_builder_url', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['source_vocabulary count > 12 OR mixed-case values', 'medium_vocabulary count > 6 OR mixed-case values', 'campaign_id_pattern empty', 'validation_regex empty', 'link_builder_url empty']
...
def _self_test() -> int:
    fixture = {
        ...
        'source_vocabulary': [{'k': 'v'}],
        'medium_vocabulary': [{'k': 'v'}],
        'campaign_id_pattern': 'sample-' + 'campaign_id_pattern',
        'validation_regex': 'sample-' + 'validation_regex',
        'link_builder_url': 'sample-' + 'link_builder_url',
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-utm-taxonomy-discipline.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
