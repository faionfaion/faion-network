#!/usr/bin/env python3
"""validate-privacy-compliance.py

Validate the output artefact for the `privacy-compliance` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/privacy-compliance.json', 'type': 'object', 'required': ['cmp', 'default_state', 'consent_mode_version', 'gpc_handler', 'ip_policy', 'tag_audit', 'produced_at'], 'additionalProperties': True}
...
OK = {'cmp': 'cookiebot', 'default_state': 'deny', 'consent_mode_version': 'v2', 'gpc_handler': {'detected_in': 'server_middleware', 'treats_as': 'ccpa_opt_out'}, 'ip_policy': 'anonymized_pre_storage', 'tag_audit': [{'tag': 'ga4', 'category': 'analytics', 'fires_when': 'analytics_storage=granted'}], 'produced_at': '2026-05-23T11:00:00Z'}
BAD = {'default_state': 'allow', 'consent_mode_version': 'v1'}
