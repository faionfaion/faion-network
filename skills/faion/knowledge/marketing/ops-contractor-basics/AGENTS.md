...
**One-sentence:** Contractor hiring playbook-step: Identify/Find/Evaluate/Onboard/Manage with paid-test-task gate before any ongoing engagement; no autonomous hire commitments.
...
- The team operates the system the methodology targets (`ops-contractor-basics` scope).
...
- Соло-операторів, що вперше делегують роботу контракторам.
- Прийняття рішення hire-vs-DIY через delegation matrix + ROI калк.
- Скринінгу 5-15 кандидатів з paid test task ДО будь-якого довгого контракту.
- Захисту від «hire on vibes»: гейтер на human approval at every commitment point.
...
| File | Purpose |
|------|---------|
| `templates/ops-contractor-basics.md.j2` | Working skeleton for the `ops-contractor-basics` artefact with required fields and `not_applicable: <reason>` markers per row. |
| `templates/ops-contractor-basics.md` | Working skeleton for the `ops-contractor-basics` artefact with required fields and `not_applicable: <reason>` markers per row. Generated from `templates/ops-contractor-basics.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
| `templates/job-post.md.j2` | Contractor job post skeleton — role, outcome, requirements, rate band, application filter |
| `templates/job-post.md` | Contractor job post skeleton — role, outcome, requirements, rate band, application filter Generated from `templates/job-post.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |

## Scripts

| File | Purpose | When to call |
|------|---------|--------------|
| `scripts/validate-ops-contractor-basics.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
