#!/usr/bin/env python3
"""validate-ops-churn-prevention.py

Validate the output artefact for the `ops-churn-prevention` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/ops-churn-prevention.json', 'type': 'object', 'required': ['playbook_version', 'phases', 'reason_segments', 'offers', 'win_back_windows', 'stop_loss', 'owner', 'produced_at'], 'additionalProperties': True}
...
OK = {'playbook_version': '2026-Q2', 'phases': ['early_intervention', 'save_offer', 'win_back'], 'reason_segments': ['price', 'missing_feature', 'low_usage', 'competitor', 'lifecycle'], 'offers': [{'segment': 'price', 'offer': '20pct_3mo', 'max_cost_pct_ltv': 0.45}], 'win_back_windows': [30, 60, 90], 'stop_loss': {'metric': 'blended_margin', 'threshold': 0.3, 'window_days': 28}, 'owner': {'name': 'Tetiana Lytvyn', 'role': 'head of lifecycle'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'phases': 'all', 'offers': '20pct'}
