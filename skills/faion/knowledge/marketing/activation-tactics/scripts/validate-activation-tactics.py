...
Validates a playbook-step artefact produced by methodology 'activation-tactics' against the
...
def validate(obj) -> list:
    ...
    if "slug" in obj and obj["slug"] != "activation-tactics":
        errs.append(f"slug must equal 'activation-tactics'")
    if "steps" in obj:
...
def self_test() -> int:
    ...
    if errs:
        ...
        return 1
    errs2 = validate({"slug": "activation-tactics"})
    if not errs2:
