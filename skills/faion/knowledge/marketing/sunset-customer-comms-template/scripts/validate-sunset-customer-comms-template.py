#!/usr/bin/env python3
"""validate-sunset-customer-comms-template.py — validate a Sunset Customer Comms Template artefact JSON against the output contract.

Usage:
  validate-sunset-customer-comms-template.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('artefact_id', 'owner', 'cause_sentence', 'timeline', 'migration_path', 'communication_channels', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["cause_sentence matches r'(exciting|new chapter|evolving|journey)' (case-insensitive)", "owner = 'team' / 'we'", 'migration_path.primary missing', 'timeline.migration_window_days < 30', 'communication_channels empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-sunset-customer-comms-template.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
