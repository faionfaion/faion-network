#!/usr/bin/env python3
"""validate-growth-gtm-strategy.py

Validate the output artefact for the `growth-gtm-strategy` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/growth-gtm-strategy.json', 'type': 'object', 'required': ['icp', 'anti_icp', 'positioning_statement', 'channels', 'sales_motion', 'acv_band', 'assumptions_logged', 'owner', 'produced_at'], 'additionalProperties': True}
...
OK = {'icp': {'firmographic': 'B2B SaaS series-A', 'buying_authority': 'head of growth', 'trigger': 'churn >5% monthly'}, 'anti_icp': 'agencies + enterprise > 500 employees', 'positioning_statement': 'For B2B SaaS series-A heads of growth with churn pain, RetentionHQ is retention analytics that ships a cohort dashboard in 30 minutes; unlike spreadsheets and warehouse tools, no data engineer is required.', 'channels': ['seo_content', 'founder_led_outbound', 'partner_referrals'], 'sales_motion': 'inside_sales', 'acv_band': '$8k-$24k', 'assumptions_logged': [{'claim': 'seo CPL < $80', 'tag': 'ASSUMPTION', 'validate_by': '2026-Q3'}], 'owner': {'name': 'Tetiana Lytvyn', 'role': 'founder'}, 'produced_at': '2026-05-23T14:00:00Z'}
BAD = {'icp': 'everyone', 'channels': ['seo', 'ads', 'events', 'cold', 'influencers', 'tv']}
