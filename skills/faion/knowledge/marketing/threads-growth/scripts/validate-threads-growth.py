#!/usr/bin/env python3
"""validate-threads-growth.py — validate a Threads Growth artefact JSON against the output contract.

Usage:
  validate-threads-growth.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'daily_post_target', 'daily_reply_target', 'adaptation_log', 'voice_register', 'kpi_set', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['daily_post_target < 5', "voice_register == 'formal'", 'adaptation_log contains identical text from source (copy-paste)', "kpi_set headline == 'followers'"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-threads-growth.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
