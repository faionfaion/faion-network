#!/usr/bin/env python3
"""validate-social-proof-harvest.py — validate a Social Proof Harvest artefact JSON against the output contract.

Usage:
  validate-social-proof-harvest.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('quote_id', 'source_url', 'author_handle', 'author_display_name', 'verbatim_quote', 'captured_at', 'channel', 'consent', 'status', 'publish_surfaces', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['verbatim_quote differs from source text (paraphrase)', "status=='published' AND consent.granted_at missing", 'source_url empty', 'quote_id duplicated within the DB', 'publish_surfaces multiple BUT quote_id not the single source-of-truth']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-social-proof-harvest.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
