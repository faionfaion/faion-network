#!/usr/bin/env python3
"""validate-twitter-x-monetization-thread-to-product.py — validate a Twitter X Monetization Thread To Product artefact JSON against the output contract.

Usage:
  validate-twitter-x-monetization-thread-to-product.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'audience_signal', 'thread', 'email_gate_offer', 'welcome_sequence', 'paid_product', 'kpi_set', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['audience_signal.evidence_urls count < 3', 'thread shape != funnel-mode 7-tweet', 'email_gate_offer missing url', 'welcome_sequence count < 5', 'paid_product missing OR price == 0']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-twitter-x-monetization-thread-to-product.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
