#!/usr/bin/env python3
"""validate-substack-to-product-funnel.py — validate a Substack-to-Product Funnel artefact JSON against the output contract.

Usage:
  validate-substack-to-product-funnel.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'publication_url', 'paid_tier', 'recommendation_partners', 'cross_post_cadence', 'welcome_sequence', 'hard_product_url', 'hard_product_cta_text', 'kpi_set', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["paid_tier.concrete_benefit absent or 'support the publication'", 'recommendation_partners count < 3', 'welcome_sequence missing email 4 OR email 4 lacks hard_product_cta', 'cross_post_cadence == 0 per month', 'hard_product_url empty']
...
def _self_test() -> int:
    fixture = {
        ...
        'paid_tier': {'k': 'v'},
        'recommendation_partners': [{'k': 'v'}],
        'cross_post_cadence': {'k': 'v'},
        'welcome_sequence': [{'k': 'v'}],
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-substack-to-product-funnel.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
