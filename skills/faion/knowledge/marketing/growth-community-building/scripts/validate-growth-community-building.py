#!/usr/bin/env python3
"""validate-growth-community-building.py — validate a Growth Community Building artefact JSON against the output contract.

Usage:
  validate-growth-community-building.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('community_id', 'host', 'platform', 'seed_members', 'planted_conversations', 'rituals', 'guidelines', 'health_targets', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['platform array (multiple) — single platform only', 'seed_members count < 50', 'rituals count < 3', "host = 'team' / 'community manager' (unnamed)", 'planted_conversations count < 5']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-growth-community-building.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
