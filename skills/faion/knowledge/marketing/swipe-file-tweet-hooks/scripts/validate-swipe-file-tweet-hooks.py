#!/usr/bin/env python3
"""validate-swipe-file-tweet-hooks.py — validate a Swipe File Tweet Hooks artefact JSON against the output contract.

Usage:
  validate-swipe-file-tweet-hooks.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'entries', 'hook_shapes', 'attribution_policy', 'storage_path', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['entries count < 20', 'any entry missing source_url OR author_handle', 'hook_shapes count < 5', 'attribution_policy empty', "any entry verbatim-copied into operator's own posts log"]
...
def _self_test() -> int:
    fixture = {
        ...
        'entries': [{'k': 'v'}],
        'hook_shapes': [{'k': 'v'}],
        'attribution_policy': 'sample-' + 'attribution_policy',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-swipe-file-tweet-hooks.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
