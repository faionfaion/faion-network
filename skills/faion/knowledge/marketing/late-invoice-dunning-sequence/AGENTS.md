...
| File | Purpose |
|------|---------|
| `templates/late-invoice-dunning-sequence.md.j2` | Working skeleton for the `late-invoice-dunning-sequence` artefact with required fields and `not_applicable: <reason>` markers per row. |
| `templates/late-invoice-dunning-sequence.md` | Working skeleton for the `late-invoice-dunning-sequence` artefact with required fields and `not_applicable: <reason>` markers per row. Generated from `templates/late-invoice-dunning-sequence.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
