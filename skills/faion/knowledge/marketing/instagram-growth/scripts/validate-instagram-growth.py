#!/usr/bin/env python3
"""validate-instagram-growth.py — validate a Instagram Growth artefact JSON against the output contract.

Usage:
  validate-instagram-growth.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'pillar', 'reel_batch', 'stories_cadence', 'dm_trigger', 'engagement_block', 'kpi_set', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['pillar split into 3+ niches', 'reel_batch count < 28', 'DM trigger missing lead_magnet_url', 'engagement_block < 20 minutes', "kpi_set headline == 'followers'"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-instagram-growth.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
