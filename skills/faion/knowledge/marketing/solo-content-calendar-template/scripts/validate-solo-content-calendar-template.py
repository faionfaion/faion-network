#!/usr/bin/env python3
"""validate-solo-content-calendar-template.py — validate a Solo Content Calendar Template artefact JSON against the output contract.

Usage:
  validate-solo-content-calendar-template.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'rows', 'primary_channel', 'quarterly_review_date', 'friday_gate_enabled', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['rows count != 12', 'any row missing pain_ref OR channel OR target_keyword/hook', 'any row pinned to a pain not in current top-5', 'primary_channel list (multi-channel)', 'friday_gate_enabled == false']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-solo-content-calendar-template.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
