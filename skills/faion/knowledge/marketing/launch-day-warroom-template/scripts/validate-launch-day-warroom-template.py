#!/usr/bin/env python3
"""validate-launch-day-warroom-template.py

Validates a playbook-step artefact produced by methodology 'launch-day-warroom-template' against the
...
def validate(obj) -> list:
    ...
    if "slug" in obj and obj["slug"] != "launch-day-warroom-template":
        errs.append(f"slug must equal 'launch-day-warroom-template'")
    if "steps" in obj:
...
def self_test() -> int:
    ...
    if errs:
        ...
        return 1
    errs2 = validate({"slug": "launch-day-warroom-template"})
    if not errs2:
