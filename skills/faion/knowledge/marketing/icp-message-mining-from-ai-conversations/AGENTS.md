...
**One-sentence:** ICP message-mining report: extract JTBD phrases / objections / success quotes / churn signals / competitor mentions from support + sales + AI-agent transcripts, monthly top-5 synthesis driving ≥1 copy change per month.

**One-paragraph:** ICP Message Mining from AI Conversations pins the discipline that turns this workflow from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in `content/02-output-contract.xml` drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale. The artefact lives in the team's versioned space and is refreshed on a stated cadence.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`icp-message-mining-from-ai-conversations` scope).
...
- Маркетингових команд, що мають ≥100 transcripts на місяць (support + sales + AI agents).
- Витягання JTBD phrases / objections / success quotes / churn signals у fixed taxonomy.
- Monthly synthesis: top-5 по кожному типу, що йде в копі і roadmap.
- Feedback loop у copy: ≥1 landing / FAQ / ad change щомісяця.
...
| File | Purpose |
|------|---------|
| `templates/icp-message-mining-from-ai-conversations.md.j2` | Working skeleton for the `icp-message-mining-from-ai-conversations` artefact with required fields and `not_applicable: <reason>` markers per row. |
| `templates/icp-message-mining-from-ai-conversations.md` | Working skeleton for the `icp-message-mining-from-ai-conversations` artefact with required fields and `not_applicable: <reason>` markers per row. Generated from `templates/icp-message-mining-from-ai-conversations.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-icp-message-mining-from-ai-conversations.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
