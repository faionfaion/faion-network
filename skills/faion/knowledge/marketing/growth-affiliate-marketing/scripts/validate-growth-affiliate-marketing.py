#!/usr/bin/env python3
"""validate-growth-affiliate-marketing.py

Validate the output artefact for the `growth-affiliate-marketing` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/growth-affiliate-marketing.json', 'type': 'object', 'required': ['program_id', 'commission_structure', 'cookie_window_days', 'disclosure_policy', 'fraud_detection', 'owner', 'produced_at'], 'additionalProperties': True}
...
OK = {'program_id': 'affiliate-2026', 'commission_structure': {'type': 'recurring', 'pct_year_1': 0.2, 'pct_lifetime': 0.1, 'cap_pct_ltv': 0.25}, 'cookie_window_days': 60, 'disclosure_policy': {'required': True, 'audit_rate_pct': 0.1}, 'fraud_detection': ['cookie_stuffing_monitor', 'brand_keyword_serp_monitor', 'self_referral_block'], 'owner': {'name': 'Olha Riznyk', 'role': 'head of partnerships'}, 'produced_at': '2026-05-23T12:00:00Z'}
BAD = {'program_id': 'x', 'commission_structure': '50%', 'cookie_window_days': 365}
