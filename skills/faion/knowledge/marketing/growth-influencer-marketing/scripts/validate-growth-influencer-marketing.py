#!/usr/bin/env python3
"""validate-growth-influencer-marketing.py

Validate the output artefact for the `growth-influencer-marketing` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/growth-influencer-marketing.json', 'type': 'object', 'required': ['program_id', 'tier', 'partners', 'outreach_policy', 'disclosure_policy', 'tracking_per_partner', 'owner', 'produced_at'], 'additionalProperties': True}
...
OK = {'program_id': 'influencer-2026-Q2', 'tier': 'micro_10k_100k', 'partners': [{'handle': '@creator1', 'followers': 38000, 'engagement_pct': 0.041, 'bot_ratio_pct': 0.08, 'vetted': True}], 'outreach_policy': {'personalized': True, 'min_content_references': 1}, 'disclosure_policy': {'required': True, 'audit_rate_pct': 0.2}, 'tracking_per_partner': {'method': 'unique_promo_code', 'attribution_window_days': 90}, 'owner': {'name': 'Olha Riznyk', 'role': 'head of brand'}, 'produced_at': '2026-05-23T15:00:00Z'}
BAD = {'program_id': 'x', 'tier': 'mega_1M_plus', 'partners': []}
