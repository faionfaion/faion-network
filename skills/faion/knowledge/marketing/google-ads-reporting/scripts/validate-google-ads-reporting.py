#!/usr/bin/env python3
"""validate-google-ads-reporting.py — schema validator for the google-ads-reporting methodology artefact.
...
SCHEMA = { '$id': 'https://faion.net/schemas/google-ads-reporting.json',
  '$schema': 'https://json-schema.org/draft/2020-12/schema',
  'properties': { 'action_queue': { 'items': { 'required': [ 'priority',
                                                             'lever',
                                                             'expected_impact',
                                                             'owner'],
                                               'type': 'object'},
                                    ...
                                    'minItems': 1,
                                    'type': 'array'},
                  'alerts': { 'items': { 'required': ['metric', 'operator', 'threshold'],
                                         'type': 'object'},
                              'minItems': 4,
                              'type': 'array'},
                  'columns': {'items': {'type': 'string'}, 'minItems': 12, 'type': 'array'},
                  'segments': { 'items': { 'enum': [ 'device',
                                                     'network',
                                                     'location',
                                                     'time',
                                                     'audience']},
                                'minItems': 4,
                                'type': 'array'},
                  'views': { 'items': { 'enum': [ 'operator_weekly',
                                                  'executive_monthly',
                                                  'stakeholder_summary']},
                             'minItems': 3,
                             'type': 'array'}},
  'required': ['columns', 'views', 'alerts', 'segments', 'action_queue'],
  'type': 'object'}

OK = { 'action_queue': [ { 'expected_impact': 'CPA -8%',
                      'lever': 'negative_sweep',
                      'owner': 'ops',
                      'priority': 1}],
  'alerts': [ {'metric': 'CPA', 'operator': '>', 'threshold': 'target +20%'},
              {'metric': 'IS', 'operator': '<', 'threshold': '70%'},
              {'metric': 'conv_rate', 'operator': '<', 'threshold': 'target -15%'},
              {'metric': 'daily_spend', 'operator': '>', 'threshold': 'forecast +25%'}],
  'columns': [ 'Impressions',
               'Clicks',
               'CTR',
               'Avg CPC',
               ...
               'Impression share',
               'Quality Score'],
  'segments': ['device', 'network', 'location', 'time', 'audience'],
  'views': ['operator_weekly', 'executive_monthly', 'stakeholder_summary']}
BAD = { 'action_queue': [],
  'alerts': [],
  'columns': ['Clicks'],
  'segments': [],
  'views': ['operator_weekly']}
