#!/usr/bin/env python3
"""validate-tweet-thread-launch-template.py — validate a Tweet Thread Launch Template artefact JSON against the output contract.

Usage:
  validate-tweet-thread-launch-template.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('launch_id', 'operator', 'tweets', 'demo_gif_url', 'hook_variants_tested', 'social_proof_quote', 'scheduled_for', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['tweets count != 7', 'demo_gif_url empty OR static-image URL', 'social_proof_quote missing consent_logged_at', 'hook_variants_tested count < 3', 'tweet text exceeding platform char limit']
...
def _self_test() -> int:
    fixture = {
        ...
        'hook_variants_tested': [{'k': 'v'}],
        'social_proof_quote': {'k': 'v'},
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-tweet-thread-launch-template.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
