#!/usr/bin/env python3
"""validate-ops-contractor-management.py

Validate the output artefact for the `ops-contractor-management` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/ops-contractor-management.json', 'type': 'object', 'required': ['slug', 'title', 'owner', 'produced_at', 'evidence', 'version', 'last_reviewed', 'items'], 'properties': {'slug': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'}, 'title': {'type': 'string', 'minLength': 3, 'maxLength': 200}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string', 'minLength': 2}, 'role': {'type': 'string', 'minLength': 2}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}, 'evidence': {'type': 'array', 'items': {'type': 'object', 'required': ['claim', 'link'], 'properties': {'claim': {'type': 'string', 'minLength': 3}, 'link': {'type': 'string', 'format': 'uri'}}}, 'minItems': 1}, 'version': {'type': 'string', 'pattern': '^\\d+\\.\\d+\\.\\d+$'}, 'last_reviewed': {'type': 'string', 'format': 'date'}, 'status': {'enum': ['draft', 'in-review', 'active', 'deprecated']}, 'notes': {'type': 'string'}, 'items': {'type': 'array', 'minItems': 1, 'items': {'type': 'object', 'required': ['id', 'statement', 'executor', 'artefact'], 'properties': {'id': {'type': 'string', 'pattern': '^[a-z0-9-]+$'}, 'statement': {'type': 'string', 'minLength': 3}, 'executor': {'type': 'string', 'minLength': 2}, 'artefact': {'type': 'string', 'minLength': 2}, 'not_applicable': {'type': 'string'}}}}}, 'additionalProperties': True}
