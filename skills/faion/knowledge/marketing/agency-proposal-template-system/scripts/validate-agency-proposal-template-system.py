#!/usr/bin/env python3
"""validate-agency-proposal-template-system.py

Validate the output artefact for the `agency-proposal-template-system` methodology against the JSON Schema
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/agency-proposal-template-system.json', 'type': 'object', 'required': ['lead_id', 'tiers', 'send_due_at', 'send_sent_at', 'minimum_term_months', 'owner', 'produced_at'], 'additionalProperties': True}
...
OK = {'lead_id': 'acme-2026-04', 'tiers': [{'name': 'Good', 'price_monthly': 3500, 'scope': ['2 articles', 'quarterly SEO audit'], 'out_of_scope': ['video', 'design']}, {'name': 'Better', 'price_monthly': 5800, 'scope': ['4 articles', 'monthly SEO audit', 'newsletter + X'], 'out_of_scope': ['video', 'design']}, {'name': 'Best', 'price_monthly': 9500, 'scope': ['6 articles + 2 case studies', 'monthly SEO + competitive intel', 'all channels + paid'], 'out_of_scope': ['video']}], 'send_due_at': '2026-04-12T18:00:00Z', 'send_sent_at': '2026-04-12T16:00:00Z', 'minimum_term_months': 6, 'owner': {'name': 'Olha Riznyk', 'role': 'founder'}, 'produced_at': '2026-05-23T09:00:00Z'}
BAD = {'lead_id': 'x', 'tiers': [{'name': 'single', 'price_monthly': 5000}], 'minimum_term_months': 1}
