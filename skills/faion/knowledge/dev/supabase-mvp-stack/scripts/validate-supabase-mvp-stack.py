#!/usr/bin/env python3
"""validate-supabase-mvp-stack.py

Validate an artefact produced by the supabase-mvp-stack methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/supabase-mvp-stack.json", "type": "object", "required": ["tables", "buckets", "migrations_path", "realtime_channels", "owner"], "properties": {"tables": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["name", "rls_enabled", "policies"], "properties": {"name": {"type": "string"}, "rls_enabled": {"type": "boolean"}, "policies": {"type": "array", "minItems": 1, "items": {"type": "string"}}}}}, "buckets": {"type": "array", "items": {"type": "object", "required": ["name", "private"], "properties": {"name": {"type": "string"}, "private": {"type": "boolean"}}}}, "migrations_path": {"type": "string"}, "realtime_channels": {"type": "array", "items": {"type": "object", "required": ["name", "payload_columns_reviewed"], "properties": {"name": {"type": "string"}, "payload_columns_reviewed": {"type": "boolean"}}}}, "owner": {"type": "string"}}}')
