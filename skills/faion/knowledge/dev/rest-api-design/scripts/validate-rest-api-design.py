#!/usr/bin/env python3
"""validate-rest-api-design.py

Validate an artefact produced by the rest-api-design methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/rest-api-design.json", "type": "object", "required": ["resources", "version_strategy", "error_format", "pagination", "status_codes"], "properties": {"resources": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["name", "url"]}}, "version_strategy": {"type": "string", "enum": ["path", "header", "query"]}, "error_format": {"type": "string", "enum": ["problem_json", "custom"]}, "pagination": {"type": "string", "enum": ["cursor", "offset", "page_number"]}, "status_codes": {"type": "array", "items": {"type": "integer"}}, "auth_model": {"type": "string"}}}')
