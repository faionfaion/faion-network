...
**One-sentence:** Scores vendor lock-in (data portability, control-plane exit, hiring market, license) on a 0-5 rubric.

**One-paragraph:** Scores vendor lock-in (data portability, control-plane exit, hiring market, license) on a 0-5 rubric. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
| File | Purpose |
|------|---------|
| `templates/output.md.j2` | Rubric skeleton matching the schema in 02-output-contract.xml |
| `templates/output.md` | Rubric skeleton matching the schema in 02-output-contract.xml Generated from `templates/output.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-lock-in-risk-scoring-rubric.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
