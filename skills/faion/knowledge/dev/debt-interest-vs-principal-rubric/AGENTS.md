# Debt Interest Vs Principal Rubric

## Summary

**One-sentence:** Debt Interest Vs Principal Rubric: codified dev practice that turns the recurring 'role-software-architect/Weekly tech-debt prioritisation session' decision into a repeatable, auditable artefact.

**One-paragraph:** Debt Interest Vs Principal Rubric codifies a recurring "weekly tech-debt prioritisation session" decision into a rubric artefact with a typed input contract, a JSON-schema-checked output, and a decision tree that routes between the operational variants. It exists because adjacent methodologies cover the surrounding topic without pinning the precise output shape this task produces. The artefact carries owner, version, last-reviewed date, and citations to every input used, so downstream agents and human reviewers can consume it without re-deriving the rationale.

**Ефективно для:**

- A team that already runs the parent activity but has no canonical rubric shape.
...
- Task is an instance of "weekly tech-debt prioritisation session" or a closely-adjacent variant.
...
| File | Purpose |
|------|---------|
| `templates/debt-interest-vs-principal-rubric.json` | JSON Schema for the rubric output contract |
| `templates/debt-interest-vs-principal-rubric.md.j2` | Markdown skeleton with the required fields |
| `templates/debt-interest-vs-principal-rubric.md` | Markdown skeleton with the required fields Generated from `templates/debt-interest-vs-principal-rubric.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-debt-interest-vs-principal-rubric.py` | Enforce Debt Interest Vs Principal Rubric output contract against the JSON Schema | After subagent returns, before downstream consumer reads |
...
- upstream activity: `weekly tech-debt prioritisation session`
...
### `templates/debt-interest-vs-principal-rubric.json`

```json
{
  "$schema": "https://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/debt-interest-vs-principal-rubric.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
...
```json
{
  "artefact_id": "debt-interest-vs-principal-rubric-2026-05-23-001",
  "owner": "alice@example.com",
  "decision": "Adopt the canonical rubric shape per r1-bound-scope.",
