...
**One-sentence:** Matrix mapping SOC2/GDPR controls to system surfaces + owner + evidence

**One-paragraph:** Compliance Control Matrix SOC2 GDPR codifies a recurring "security-by-design audit + threat modelling" decision into a specification artefact with a typed input contract, a JSON-schema-checked output, and a decision tree that routes between the operational variants. It exists because adjacent methodologies cover the surrounding topic without pinning the precise output shape this task produces. The artefact carries owner, version, last-reviewed date, and citations to every input used, so downstream agents and human reviewers can consume it without re-deriving the rationale.
...
- Task is an instance of "security-by-design audit + threat modelling" or a closely-adjacent variant.
...
| File | Purpose |
|------|---------|
| `templates/compliance-control-matrix-soc2-gdpr.json` | JSON Schema for the specification output contract |
| `templates/compliance-control-matrix-soc2-gdpr.md.j2` | Markdown skeleton with the required fields |
| `templates/compliance-control-matrix-soc2-gdpr.md` | Markdown skeleton with the required fields Generated from `templates/compliance-control-matrix-soc2-gdpr.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-compliance-control-matrix-soc2-gdpr.py` | Enforce Compliance Control Matrix SOC2 GDPR output contract against the JSON Schema | After subagent returns, before downstream consumer reads |
...
- upstream activity: `security-by-design audit + threat modelling`
...
### `templates/compliance-control-matrix-soc2-gdpr.json`

```json
{
  "$schema": "https://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/compliance-control-matrix-soc2-gdpr.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
