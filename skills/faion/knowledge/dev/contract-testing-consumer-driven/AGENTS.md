...
**One-sentence:** Consumer-driven contract pacts with provider verification gate

**One-paragraph:** Contract Testing Consumer Driven codifies a recurring "modernize 2018-era QA suite into AI-augmented test ops" decision into a specification artefact with a typed input contract, a JSON-schema-checked output, and a decision tree that routes between the operational variants. It exists because adjacent methodologies cover the surrounding topic without pinning the precise output shape this task produces. The artefact carries owner, version, last-reviewed date, and citations to every input used, so downstream agents and human reviewers can consume it without re-deriving the rationale.
...
- Task is an instance of "modernize 2018-era QA suite into AI-augmented test ops" or a closely-adjacent variant.
...
| File | Purpose |
|------|---------|
| `templates/contract-testing-consumer-driven.json` | JSON Schema for the specification output contract |
| `templates/contract-testing-consumer-driven.md.j2` | Markdown skeleton with the required fields |
| `templates/contract-testing-consumer-driven.md` | Markdown skeleton with the required fields Generated from `templates/contract-testing-consumer-driven.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-contract-testing-consumer-driven.py` | Enforce Contract Testing Consumer Driven output contract against the JSON Schema | After subagent returns, before downstream consumer reads |
...
- upstream activity: `modernize 2018-era QA suite into AI-augmented test ops`
...
### `templates/contract-testing-consumer-driven.json`

```json
{
  "$schema": "https://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/contract-testing-consumer-driven.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
...
```json
{
  "artefact_id": "contract-testing-consumer-driven-2026-05-23-001",
