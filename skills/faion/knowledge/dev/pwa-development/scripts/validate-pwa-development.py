#!/usr/bin/env python3
"""validate-pwa-development.py

Validate an artefact produced by the pwa-development methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/pwa-development.json", "type": "object", "required": ["manifest", "service_worker", "cache_plan", "update_strategy"], "properties": {"manifest": {"type": "object", "required": ["name", "short_name", "start_url", "display", "icons"]}, "service_worker": {"type": "object", "required": ["scope", "skip_waiting", "clients_claim"]}, "cache_plan": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["route", "strategy", "ttl_seconds", "max_entries"]}}, "update_strategy": {"type": "object", "required": ["user_prompt", "auto_reload"]}, "offline_page": {"type": "string"}}}')
