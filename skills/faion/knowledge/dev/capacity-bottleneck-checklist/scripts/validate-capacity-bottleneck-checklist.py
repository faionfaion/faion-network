#!/usr/bin/env python3
"""validate-capacity-bottleneck-checklist.py

Validate the output artefact for the capacity-bottleneck-checklist methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/capacity-bottleneck-checklist.json", "type": "object", "required": ["artefact_id", "owner", "decision", "rationale", "inputs_used", "version", "last_reviewed", "headroom_pct", "bottlenecks"], "properties": {"artefact_id": {"type": "string", "minLength": 3}, "owner": {"type": "string", "pattern": "^(?!team$|we$|us$|engineering$|the (team|squad|group)$).+", "minLength": 2}, "decision": {"type": "string", "minLength": 10}, "rationale": {"type": "string", "minLength": 40}, "inputs_used": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "version": {"type": "string", "pattern": "^\\\\d+\\\\.\\\\d+\\\\.\\\\d+$"}, "last_reviewed": {"type": "string", "pattern": "^\\\\d{4}-\\\\d{2}-\\\\d{2}$"}, "headroom_pct": {"type": "integer", "minimum": 20, "maximum": 100}, "bottlenecks": {"type": "array", "items": {"type": "string"}, "minItems": 1}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "cap-2026-05-23", "owner": "@ruslan", "decision": "raise pgbouncer pool from 80 to 200; provision +2 web replicas", "rationale": "db pool saturation observed at 95% during last load test (load-test-2026-05-22.html); web CPU p95 at 78%; both inputs cited inline", "inputs_used": ["observability/load-test-2026-05-22.html", "observability/web-cpu-p95.json"], "version": "1.0.0", "last_reviewed": "2026-05-23", "headroom_pct": 22, "bottlenecks": ["db_pool", "web_cpu"]}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "x", "owner": "team", "decision": "tbd", "rationale": "obviously", "inputs_used": [], "version": "v1", "last_reviewed": "soon", "headroom_pct": 5, "bottlenecks": []}')
