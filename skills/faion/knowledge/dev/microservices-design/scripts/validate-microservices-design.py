#!/usr/bin/env python3
"""validate-microservices-design.py

Validate the output artefact for the microservices-design methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/microservices-design.json", "type": "object", "required": ["services", "no_shared_tables", "no_cross_service_imports", "circuit_breakers_present", "tracing_present"], "properties": {"services": {"type": "array", "items": {"type": "object", "required": ["name", "owner", "data_store", "transport"], "properties": {"name": {"type": "string"}, "owner": {"type": "string"}, "data_store": {"type": "string"}, "transport": {"enum": ["http", "grpc", "async", "mixed"]}}}, "minItems": 2}, "no_shared_tables": {"const": true}, "no_cross_service_imports": {"const": true}, "circuit_breakers_present": {"const": true}, "tracing_present": {"const": true}, "api_versioning_scheme": {"enum": ["url-path", "accept-header", "proto-package"]}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"services": [{"name": "orders", "owner": "@orders-team", "data_store": "postgres-orders", "transport": "mixed"}, {"name": "payments", "owner": "@payments-team", "data_store": "postgres-payments", "transport": "http"}], "no_shared_tables": true, "no_cross_service_imports": true, "circuit_breakers_present": true, "tracing_present": true, "api_versioning_scheme": "url-path"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"services": [{"name": "orders", "owner": "team", "data_store": "shared-db", "transport": "http"}], "no_shared_tables": false, "no_cross_service_imports": false, "circuit_breakers_present": false, "tracing_present": false}')
