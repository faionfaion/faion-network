# Flag Killswitch Decision Criteria

## Summary

**One-sentence:** Kill vs keep vs ramp decision criteria for flag review

**One-paragraph:** Flag Killswitch Decision Criteria codifies a recurring "feature flag rollout decision (weekly)" decision into a decision record artefact with a typed input contract, a JSON-schema-checked output, and a decision tree that routes between the operational variants. It exists because adjacent methodologies cover the surrounding topic without pinning the precise output shape this task produces. The artefact carries owner, version, last-reviewed date, and citations to every input used, so downstream agents and human reviewers can consume it without re-deriving the rationale.
...
- Task is an instance of "feature flag rollout decision (weekly)" or a closely-adjacent variant.
...
| File | Purpose |
|------|---------|
| `templates/flag-killswitch-decision-criteria.json` | JSON Schema for the decision record output contract |
| `templates/flag-killswitch-decision-criteria.md.j2` | Markdown skeleton with the required fields |
| `templates/flag-killswitch-decision-criteria.md` | Markdown skeleton with the required fields Generated from `templates/flag-killswitch-decision-criteria.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-flag-killswitch-decision-criteria.py` | Enforce Flag Killswitch Decision Criteria output contract against the JSON Schema | After subagent returns, before downstream consumer reads |
...
- upstream activity: `feature flag rollout decision (weekly)`
...
### `templates/flag-killswitch-decision-criteria.json`

```json
{
  "$schema": "https://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/flag-killswitch-decision-criteria.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
...
```json
{
  "artefact_id": "flag-killswitch-decision-criteria-2026-05-23-001",
