...
**One-sentence:** Failure modes as first-class design inputs spec

**One-paragraph:** Failure Mode As Design Input codifies a recurring "microservice extraction safety gate" decision into a specification artefact with a typed input contract, a JSON-schema-checked output, and a decision tree that routes between the operational variants. It exists because adjacent methodologies cover the surrounding topic without pinning the precise output shape this task produces. The artefact carries owner, version, last-reviewed date, and citations to every input used, so downstream agents and human reviewers can consume it without re-deriving the rationale.
...
- Task is an instance of "microservice extraction safety gate" or a closely-adjacent variant.
...
| File | Purpose |
|------|---------|
| `templates/failure-mode-as-design-input.json` | JSON Schema for the specification output contract |
| `templates/failure-mode-as-design-input.md.j2` | Markdown skeleton with the required fields |
| `templates/failure-mode-as-design-input.md` | Markdown skeleton with the required fields Generated from `templates/failure-mode-as-design-input.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-failure-mode-as-design-input.py` | Enforce Failure Mode As Design Input output contract against the JSON Schema | After subagent returns, before downstream consumer reads |

## Related

- parent skill: `pro/dev/software-developer/`
- upstream activity: `microservice extraction safety gate`
...
### `templates/failure-mode-as-design-input.json`

```json
{
  "$schema": "https://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/failure-mode-as-design-input.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
...
```json
{
  "artefact_id": "failure-mode-as-design-input-2026-05-23-001",
