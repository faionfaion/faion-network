...
**One-sentence:** Regime review matrix for SOC2/HIPAA/PCI client diffs

**One-paragraph:** Compliance Regime Review Matrix SOC2 HIPAA PCI codifies a recurring "audit-grade code review for compliance client" decision into a specification artefact with a typed input contract, a JSON-schema-checked output, and a decision tree that routes between the operational variants. It exists because adjacent methodologies cover the surrounding topic without pinning the precise output shape this task produces. The artefact carries owner, version, last-reviewed date, and citations to every input used, so downstream agents and human reviewers can consume it without re-deriving the rationale.

**Ефективно для:**

- A team that already runs the parent activity but has no canonical specification shape.
...
- Task is an instance of "audit-grade code review for compliance client" or a closely-adjacent variant.
...
| File | Purpose |
|------|---------|
| `templates/compliance-regime-review-matrix-soc2-hipaa-pci.json` | JSON Schema for the specification output contract |
| `templates/compliance-regime-review-matrix-soc2-hipaa-pci.md.j2` | Markdown skeleton with the required fields |
| `templates/compliance-regime-review-matrix-soc2-hipaa-pci.md` | Markdown skeleton with the required fields Generated from `templates/compliance-regime-review-matrix-soc2-hipaa-pci.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-compliance-regime-review-matrix-soc2-hipaa-pci.py` | Enforce Compliance Regime Review Matrix SOC2 HIPAA PCI output contract against the JSON Schema | After subagent returns, before downstream consumer reads |

## Related

- parent skill: `pro/dev/software-developer/`
- upstream activity: `audit-grade code review for compliance client`
...
### `templates/compliance-regime-review-matrix-soc2-hipaa-pci.json`

```json
{
  "$schema": "https://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/compliance-regime-review-matrix-soc2-hipaa-pci.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
...
```json
{
  "artefact_id": "compliance-regime-review-matrix-soc2-hipaa-pci-2026-05-23-001",
  "owner": "alice@example.com",
  "decision": "Adopt the canonical spec shape per r1-bound-scope.",
