#!/usr/bin/env python3
"""validate-react-component-architecture.py

Validate an artefact produced by the react-component-architecture methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/react-component-architecture.json", "type": "object", "required": ["routing", "folder_layout", "primitive_layer", "state_strategy", "memoisation_policy"], "properties": {"routing": {"type": "string", "enum": ["next_app_router", "next_pages", "vite_spa", "remix"]}, "folder_layout": {"type": "object", "required": ["features", "primitives"]}, "primitive_layer": {"type": "string"}, "state_strategy": {"type": "object", "required": ["global", "feature"]}, "memoisation_policy": {"type": "string", "enum": ["profiler_only", "memoised_child_only", "blanket"]}, "client_marker_required": {"type": "boolean"}}}')
