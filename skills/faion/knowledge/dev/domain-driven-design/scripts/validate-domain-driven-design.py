#!/usr/bin/env python3
"""validate-domain-driven-design.py

Validate the output artefact for the domain-driven-design methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/domain-driven-design.json", "type": "object", "required": ["bounded_context", "aggregate_roots", "value_objects", "ubiquitous_language_count", "framework_imports_in_domain"], "properties": {"bounded_context": {"type": "string", "minLength": 2}, "aggregate_roots": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "value_objects": {"type": "array", "items": {"type": "string"}}, "ubiquitous_language_count": {"type": "integer", "minimum": 5}, "framework_imports_in_domain": {"const": 0}, "context_map_path": {"type": "string"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"bounded_context": "ordering", "aggregate_roots": ["Order", "Customer"], "value_objects": ["Money", "Address"], "ubiquitous_language_count": 18, "framework_imports_in_domain": 0, "context_map_path": "docs/context-map.md"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"bounded_context": "x", "aggregate_roots": [], "value_objects": [], "ubiquitous_language_count": 1, "framework_imports_in_domain": 4}')
