#!/usr/bin/env python3
"""validate-security-testing.py

Validate an artefact produced by the security-testing methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/security-testing.json", "type": "object", "required": ["release", "sast_tools", "dep_audit", "dast", "secrets_scan", "asvs_coverage", "fix_windows"], "properties": {"release": {"type": "string"}, "sast_tools": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "dep_audit": {"type": "object", "required": ["sources", "transitive"]}, "dast": {"type": "object", "required": ["authenticated", "target"]}, "secrets_scan": {"type": "object", "required": ["pre_commit", "ci"]}, "asvs_coverage": {"type": "object", "required": ["level", "covered_pct"]}, "fix_windows": {"type": "object", "required": ["critical_hours", "high_days", "medium_days"]}}}')
