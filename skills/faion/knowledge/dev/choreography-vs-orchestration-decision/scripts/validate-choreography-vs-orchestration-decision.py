#!/usr/bin/env python3
"""validate-choreography-vs-orchestration-decision.py

Validate the output artefact for the choreography-vs-orchestration-decision methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/choreography-vs-orchestration-decision.json", "type": "object", "required": ["artefact_id", "owner", "decision", "rationale", "inputs_used", "version", "last_reviewed", "pattern", "workflow"], "properties": {"artefact_id": {"type": "string", "minLength": 3}, "owner": {"type": "string", "pattern": "^(?!team$|we$|us$|engineering$).+", "minLength": 2}, "decision": {"type": "string", "minLength": 10}, "rationale": {"type": "string", "minLength": 40}, "inputs_used": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "version": {"type": "string", "pattern": "^\\\\d+\\\\.\\\\d+\\\\.\\\\d+$"}, "last_reviewed": {"type": "string", "pattern": "^\\\\d{4}-\\\\d{2}-\\\\d{2}$"}, "pattern": {"enum": ["choreography", "orchestration"]}, "workflow": {"type": "string", "minLength": 3}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "co-checkout-2026-05-23", "owner": "@ruslan", "decision": "orchestration via temporal workflow \'checkout-saga\'", "rationale": "checkout-saga has 7 steps with compensations; choreography would spread debug ownership across 5 teams; SLO for retry recovery is 5 min; both inputs cited inline", "inputs_used": ["docs/checkout-workflow.md", "docs/service-map.md"], "version": "1.0.0", "last_reviewed": "2026-05-23", "pattern": "orchestration", "workflow": "checkout-saga"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "x", "owner": "team", "decision": "tbd", "rationale": "obviously", "inputs_used": [], "version": "v1", "last_reviewed": "soon", "pattern": "either", "workflow": ""}')
