#!/usr/bin/env python3
"""validate-tailwind-architecture.py

Validate an artefact produced by the tailwind-architecture methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/tailwind-architecture.json", "type": "object", "required": ["tailwind_version", "token_source", "cn_util", "extraction_threshold", "lint_rules"], "properties": {"tailwind_version": {"type": "string", "enum": ["v3", "v4"]}, "token_source": {"type": "string"}, "cn_util": {"type": "object", "required": ["path", "uses_tailwind_merge"]}, "extraction_threshold": {"type": "object", "required": ["min_utilities", "min_call_sites"]}, "lint_rules": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "dark_mode_strategy": {"type": "string", "enum": ["semantic_tokens", "concrete_shades"]}}}')
