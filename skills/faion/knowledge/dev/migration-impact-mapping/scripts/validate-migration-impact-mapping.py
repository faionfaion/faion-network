#!/usr/bin/env python3
"""validate-migration-impact-mapping.py

Validate a Migration Impact Mapping artefact against the JSON Schema (draft-07) declared in
...
REQUIRED = ['artefact_id', 'owner', 'last_touched', 'template_version', 'status', 'evidence', 'decision_id', 'context', 'decision', 'consequences']
...
NUM_FIELDS = []
BOOL_FIELDS = []
ARRAY_FIELDS = ['consequences']
OBJECT_FIELDS = []
STRING_FIELDS = ['decision_id', 'context', 'decision']
...
OK = {'artefact_id': 'migration-impact-mapping-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'decision_id': 'draft', 'context': 'draft', 'decision': 'draft', 'consequences': ['draft-consequence']}
BAD = {"foo": "bar"}
