#!/usr/bin/env python3
"""validate-go-concurrency-patterns.py — validate a Go Concurrency Patterns (Worker Pool, Fan-Out/Fan-In, errgroup, Pipeline) artefact JSON.

Usage:
  validate-go-concurrency-patterns.py <artefact.json>
...
ID_RE = re.compile(r"^gocp\-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^gocp\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "gocp-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/go-concurrency-patterns.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-go-concurrency-patterns.py <artefact.json>\n")
        return 2
    try:
