#!/usr/bin/env python3
"""validate-rust-testing-integration.py

Validate the output artefact for the rust-testing-integration methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/rust-testing-integration.json", "type": "object", "required": ["test_file", "uses_oneshot", "uses_testcontainers", "migrations_run_in_setup", "tokio_flavor"], "properties": {"test_file": {"type": "string", "pattern": "^tests/.*\\\\.rs$"}, "uses_oneshot": {"const": true}, "uses_testcontainers": {"type": "boolean"}, "migrations_run_in_setup": {"type": "boolean"}, "tokio_flavor": {"enum": ["multi_thread", "current_thread"]}, "snapshot_lib": {"enum": ["insta", "expect-test", "none"]}, "schema_isolation": {"enum": ["per-test-schema", "transaction-rollback", "shared"]}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"test_file": "tests/users_integration.rs", "uses_oneshot": true, "uses_testcontainers": true, "migrations_run_in_setup": true, "tokio_flavor": "multi_thread", "snapshot_lib": "insta", "schema_isolation": "per-test-schema"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"test_file": "src/main.rs", "uses_oneshot": false, "uses_testcontainers": true, "migrations_run_in_setup": false, "tokio_flavor": "current_thread", "snapshot_lib": "none", "schema_isolation": "shared"}')
