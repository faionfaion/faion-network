#!/usr/bin/env python3
"""validate-nginx-reverse-proxy.py — validate a nginx-reverse-proxy output JSON against the methodology schema.
...
REQUIRED = ["artefact_id", "version", "last_reviewed", "domain", "backend_upstream", "tls_mode", "websocket", "rate_limit_zone", "owner"]
ENUMS = {"tls_mode": ["cloudflare-origin", "letsencrypt", "self-signed"]}
SEMVER_FIELDS = ["version"]
DATE_FIELDS = ["last_reviewed"]
INT_FIELDS = []
ARRAY_FIELDS = ["security_headers"]
OBJECT_FIELDS = []
BOOL_FIELDS = ["websocket"]
STRING_FIELDS = ["artefact_id", "domain", "backend_upstream", "rate_limit_zone", "owner"]
...
VALID_FIXTURE = json.loads("{\"artefact_id\": \"nginx-app01\", \"version\": \"1.1.0\", \"last_reviewed\": \"2026-05-23\", \"domain\": \"app.example.com\", \"backend_upstream\": \"127.0.0.1:8100\", \"tls_mode\": \"cloudflare-origin\", \"websocket\": true, \"rate_limit_zone\": \"api_general\", \"security_headers\": [\"HSTS\", \"X-Content-Type-Options\", \"Referrer-Policy\"], \"owner\": \"@ruslan\"}")
INVALID_FIXTURE = json.loads("{\"domain\": \"app.example.com\", \"backend_upstream\": \"127.0.0.1:8100\", \"tls_mode\": \"wildcard\", \"websocket\": true, \"owner\": \"team\"}")
