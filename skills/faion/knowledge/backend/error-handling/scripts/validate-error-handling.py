#!/usr/bin/env python3
"""validate-error-handling.py — validate a Error Handling (RFC 7807 Problem Details) artefact JSON.

Usage:
  validate-error-handling.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^eh\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "eh-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/error-handling.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-error-handling.py <artefact.json>\n")
        return 2
    try:
