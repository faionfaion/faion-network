#!/usr/bin/env python3
"""validate-go-error-handling-patterns.py — validate a Go Error Handling Patterns (Wrap, Translate, Sentinel, Log-Once) artefact JSON.

Usage:
  validate-go-error-handling-patterns.py <artefact.json>
...
ID_RE = re.compile(r"^goep\-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^goep\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "goep-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/go-error-handling-patterns.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-go-error-handling-patterns.py <artefact.json>\n")
        return 2
    try:
