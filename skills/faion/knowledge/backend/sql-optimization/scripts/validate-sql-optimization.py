#!/usr/bin/env python3
"""validate-sql-optimization.py

Validate the output artefact for the sql-optimization methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/sql-optimization.json", "type": "object", "required": ["report_id", "queries", "explain_analyze_present", "index_changes"], "properties": {"report_id": {"type": "string", "minLength": 3}, "queries": {"type": "array", "items": {"type": "object", "required": ["name", "before_ms", "after_ms"], "properties": {"name": {"type": "string"}, "before_ms": {"type": "number", "minimum": 0}, "after_ms": {"type": "number", "minimum": 0}}}, "minItems": 1}, "explain_analyze_present": {"const": true}, "index_changes": {"type": "object", "required": ["added", "dropped"], "properties": {"added": {"type": "array"}, "dropped": {"type": "array"}}}, "net_p95_improvement_pct": {"type": "number"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"report_id": "sql-opt-2026-05-23", "queries": [{"name": "orders_by_user", "before_ms": 1280.0, "after_ms": 42.5}], "explain_analyze_present": true, "index_changes": {"added": ["idx_orders_user_created"], "dropped": []}, "net_p95_improvement_pct": 96.7}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"report_id": "x", "queries": [], "explain_analyze_present": false, "index_changes": {}, "net_p95_improvement_pct": 0}')
