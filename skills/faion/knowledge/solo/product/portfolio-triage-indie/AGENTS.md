...
maintainers: [faion]
content_id: "d22b9ab3183a3880"
summary: Portfolio Triage Indie — pinned method for the indie hacker: fixed shape + named owner + evidence anchors + outcome review, so weekly portfolio metric scan stops being folklore and starts being a reviewable operating tool.
tags: [product, solo, method, portfolio, triage, indie]
---
# Portfolio Triage Indie

## Summary

**One-sentence:** Portfolio Triage Indie — pinned method for the indie hacker: fixed shape + named owner + evidence anchors + outcome review, so weekly portfolio metric scan stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In product management, the indie hacker runs weekly portfolio metric scan on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Indie hackers run 3-8 side projects in parallel — but the corpus only supports single-product prioritisation (RICE/ICE on one backlog). Need cross-project triage criteria. `portfolio-triage-indie` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the indie hacker starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p2-indie-hacker/Weekly portfolio metric scan`
