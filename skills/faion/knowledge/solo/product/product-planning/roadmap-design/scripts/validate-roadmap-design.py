#!/usr/bin/env python3
"""validate-roadmap-design.py — validate a roadmap-design output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'horizon_quarters', 'uncertainty', 'format', 'internal_rows', 'external_themes', 'not_doing', 'owner']
ENUMS = {'uncertainty': ['low', 'medium', 'high'], 'format': ['timeline', 'now-next-later', 'outcome-themed']}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = ['horizon_quarters']
NUM_FIELDS = []
ARRAY_FIELDS = ['internal_rows', 'external_themes', 'not_doing']
OBJECT_FIELDS = []
BOOL_FIELDS = []
...
VALID_FIXTURE = json.loads("{\"artefact_id\": \"roadmap-design-example\", \"version\": \"1.0.0\", \"last_reviewed\": \"2026-05-23\", \"horizon_quarters\": 1, \"uncertainty\": \"low\", \"format\": \"timeline\", \"internal_rows\": [\"item-1\", \"item-2\", \"item-3\"], \"external_themes\": [\"item-1\", \"item-2\", \"item-3\"], \"not_doing\": [\"item-1\", \"item-2\", \"item-3\"], \"owner\": \"@solo-founder\"}")
