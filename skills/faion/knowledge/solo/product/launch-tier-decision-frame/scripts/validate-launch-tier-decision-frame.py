#!/usr/bin/env python3
"""validate-launch-tier-decision-frame.py — validate a Launch Tier Decision Frame artefact JSON against the output contract.

Usage:
  validate-launch-tier-decision-frame.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('launch_name', 'funnel_goal', 'selected_tier', 'readiness_score', 'retreat_cost', 'rationale', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["selected_tier outside 1-4", "rationale length < 200", "funnel_goal empty", "readiness_score missing capacity / support / rollback", "retreat_cost generic ('low' / 'small')"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'funnel_goal': 'sample-funnel_goal',
        'selected_tier': 1,
        'readiness_score': {'k': 'v'},
        'retreat_cost': 'sample-retreat_cost',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-launch-tier-decision-frame.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
