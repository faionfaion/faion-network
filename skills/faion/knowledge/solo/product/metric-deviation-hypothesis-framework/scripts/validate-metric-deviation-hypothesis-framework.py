#!/usr/bin/env python3
"""validate-metric-deviation-hypothesis-framework.py — validate a Metric Deviation Hypothesis Framework artefact JSON against the output contract.

Usage:
  validate-metric-deviation-hypothesis-framework.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('metric_name', 'baseline', 'deviation_magnitude', 'time_window', 'hypotheses', 'next_checks', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["hypotheses count < 3", "any hypothesis missing detector", "deviation_magnitude non-numeric", "next_checks unranked", "probability not in {high, medium, low}"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-metric-deviation-hypothesis-framework.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
