#!/usr/bin/env python3
"""validate-launch-comms-kit-template.py — validate a Launch Comms Kit Template artefact JSON against the output contract.

Usage:
  validate-launch-comms-kit-template.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('launch_name', 'positioning_sentence', 'launch_window', 'channels', 'publish_timeline', 'retro_at', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["positioning_sentence empty OR > 140 chars", "channels object missing \u22651 required channel", "publish_timeline empty", "retro_at empty or non-ISO", "channel-piece positioning_sentence overlap < 80% with canonical"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-launch-comms-kit-template.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
