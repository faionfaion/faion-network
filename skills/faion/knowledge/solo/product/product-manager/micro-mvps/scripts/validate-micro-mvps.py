#!/usr/bin/env python3
"""validate-micro-mvps.py — validate a Micro MVPs artefact JSON against the output contract.

Usage:
  validate-micro-mvps.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('hypothesis', 'gate_event', 'gate_threshold', 'build_window', 'manual_backend_plan', 'decision_at', 'decision', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["build_window > 7 days", "hypothesis missing numeric prediction", "gate_threshold non-numeric", "decision not in {go, kill, iterate}", "manual_backend_plan empty AND build_includes_back_end_code == false"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-micro-mvps.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
