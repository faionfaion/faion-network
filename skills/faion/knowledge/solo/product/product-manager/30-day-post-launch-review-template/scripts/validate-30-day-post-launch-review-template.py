#!/usr/bin/env python3
"""validate-30-day-post-launch-review-template.py — validate a 30 Day Post Launch Review Template artefact JSON against the output contract.

Usage:
  validate-30-day-post-launch-review-template.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('launch_id', 'launch_comms_kit_id', 'review_at', 'funnel_by_channel', 'hypothesis_verdicts', 'next_bets', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["funnel_by_channel empty", "any hypothesis_verdicts entry missing verdict", "next_bets count < 3", "any next_bet missing budget_usd or budget_hours", "launch_comms_kit_id empty"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'funnel_by_channel': {'k': 'v'},
        'hypothesis_verdicts': [{'k': 'v'}],
        'next_bets': [{'k': 'v'}],
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-30-day-post-launch-review-template.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
