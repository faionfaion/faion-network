#!/usr/bin/env python3
"""validate-feature-prioritization-moscow.py — validate a Feature Prioritization MoSCoW artefact JSON against the output contract.

Usage:
  validate-feature-prioritization-moscow.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('cycle_id', 'capacity', 'buckets', 'tiebreaker_rule', 'must_cap_pct', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["Must items consume > 60% of capacity", "buckets count != 4", "tiebreaker_rule empty", "wont_items count == 0", "any Must item without dod"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'capacity': 1,
        'buckets': {'k': 'v'},
        'tiebreaker_rule': 'sample-tiebreaker_rule',
        'must_cap_pct': 1,
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-feature-prioritization-moscow.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
