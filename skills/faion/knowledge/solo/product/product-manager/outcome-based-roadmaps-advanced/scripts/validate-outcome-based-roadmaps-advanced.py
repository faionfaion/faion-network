#!/usr/bin/env python3
"""validate-outcome-based-roadmaps-advanced.py — validate a Outcome Based Roadmaps Advanced artefact JSON against the output contract.

Usage:
  validate-outcome-based-roadmaps-advanced.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('horizon', 'swim_lanes', 'outcomes_by_quarter', 'dependency_edges', 'confidence_decay_applied', 'quarter_reviews', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["swim_lanes count < 2", "any dependency edge with type empty", "any Q+3 solution with confidence=high AND no cited evidence", "outcomes_by_quarter covers only 1 quarter", "quarter_reviews missing on closed quarter"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        'horizon': {'k': 'v'},
        'swim_lanes': [{'k': 'v'}],
        'outcomes_by_quarter': {'k': 'v'},
        'dependency_edges': [{'k': 'v'}],
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-outcome-based-roadmaps-advanced.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
