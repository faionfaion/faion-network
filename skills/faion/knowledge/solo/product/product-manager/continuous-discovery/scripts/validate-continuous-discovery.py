#!/usr/bin/env python3
"""validate-continuous-discovery.py — validate a Continuous Discovery artefact JSON against the output contract.

Usage:
  validate-continuous-discovery.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('outcome', 'touchpoints', 'opportunity_tree', 'assumption_tests', 'decision_log', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["touchpoints per week < 1", "opportunity_tree depth < 2", "any opportunity without assumption_test", "decision_log empty", "outcome empty"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'touchpoints': [{'k': 'v'}],
        'opportunity_tree': {'k': 'v'},
        'assumption_tests': [{'k': 'v'}],
        'decision_log': [{'k': 'v'}],
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-continuous-discovery.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
