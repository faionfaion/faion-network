#!/usr/bin/env python3
"""validate-product-launch.py — validate a product-launch output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'launch_id', 'audience_segments', 'stage_gates', 'success_metrics', 'rollback_plan', 'post_launch_review_date', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed', 'post_launch_review_date']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['audience_segments', 'stage_gates', 'success_metrics']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'launch_id', 'rollback_plan', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "product-launch-example", "version": "1.0.0", "last_reviewed": "2026-05-23", "launch_id": "launch_id value", "audience_segments": ["item-1", "item-2", "item-3"], "stage_gates": ["item-1", "item-2", "item-3"], "success_metrics": ["item-1", "item-2", "item-3"], "rollback_plan": "rollback_plan value", "post_launch_review_date": "2026-05-23", "owner": "@solo-founder"}')
