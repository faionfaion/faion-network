#!/usr/bin/env python3
"""validate-backlog-management.py — validate a Backlog Management artefact JSON against the output contract.

Usage:
  validate-backlog-management.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('backlog_url', 'buckets', 'ready_items', 'type_distribution', 'grooming_cadence', 'prioritisation_method', 'archive_proposal_count', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["any Ready item with empty acceptance_criteria", "buckets count != 4", "any item without type tag", "grooming_cadence empty", "ready bucket count > 25"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        'backlog_url': 'sample-backlog_url',
        'buckets': {'k': 'v'},
        'ready_items': [{'k': 'v'}],
        'type_distribution': {'k': 'v'},
        'grooming_cadence': {'k': 'v'},
        'prioritisation_method': 'sample-prioritisation_method',
        'archive_proposal_count': 1,
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-backlog-management.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
