#!/usr/bin/env python3
"""validate-mvp-scoping.py — validate a MVP Scoping artefact JSON against the output contract.

Usage:
  validate-mvp-scoping.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('product_name', 'aha_moment', 'must_have_journey', 'build_window', 'cut_list', 'readiness_checklist', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["build_window > 4 weeks", "aha_moment empty", "cut_list count < 3", "must_have_journey count != 1", "any readiness_checklist subcomponent false at launch_at"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'must_have_journey': [{'k': 'v'}],
        'build_window': {'k': 'v'},
        'cut_list': [{'k': 'v'}],
        'readiness_checklist': {'k': 'v'},
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-mvp-scoping.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
