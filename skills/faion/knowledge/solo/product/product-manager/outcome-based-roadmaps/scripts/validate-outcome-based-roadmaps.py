#!/usr/bin/env python3
"""validate-outcome-based-roadmaps.py — validate a Outcome Based Roadmaps artefact JSON against the output contract.

Usage:
  validate-outcome-based-roadmaps.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('quarter', 'outcomes', 'solutions', 'public_url', 'quarter_review', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["outcomes count > 3", "any row title is a feature verb", "any solution with exact-date promise", "any solution with empty confidence", "quarter_review missing on closed quarter"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-outcome-based-roadmaps.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
