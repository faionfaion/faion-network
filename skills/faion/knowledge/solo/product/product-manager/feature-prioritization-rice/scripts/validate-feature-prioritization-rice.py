#!/usr/bin/env python3
"""validate-feature-prioritization-rice.py — validate a Feature Prioritization RICE artefact JSON against the output contract.

Usage:
  validate-feature-prioritization-rice.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('scoring_round_id', 'reach_unit', 'effort_unit', 'impact_anchors', 'rows', 'cut_line', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["any row missing source citation", "reach_unit empty", "effort_unit mixed across rows", "impact_anchors count != 3", "cut_line empty"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        'scoring_round_id': 'sample-scoring_round_id',
        'reach_unit': 'sample-reach_unit',
        'effort_unit': 'sample-effort_unit',
        'impact_anchors': [{'k': 'v'}],
        'rows': [{'k': 'v'}],
        'cut_line': 1,
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-feature-prioritization-rice.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
