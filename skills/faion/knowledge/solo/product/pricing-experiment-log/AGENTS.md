...
maintainers: [faion]
content_id: "4e00c3552c96e0f6"
summary: Pricing Experiment Log — pinned log for the indie hacker: fixed shape + named owner + evidence anchors + outcome review, so pricing experiment toggle stops being folklore and starts being a reviewable operating tool.
tags: [product, solo, log, pricing, experiment]
---
# Pricing Experiment Log

## Summary

**One-sentence:** Pricing Experiment Log — pinned log for the indie hacker: fixed shape + named owner + evidence anchors + outcome review, so pricing experiment toggle stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In product management, the indie hacker runs pricing experiment toggle on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. pricing-experiments playbook exists but no living log/template to track hypotheses → result → decision across a year of flips. `pricing-experiment-log` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the indie hacker starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored log committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p2-indie-hacker/Pricing experiment toggle`
