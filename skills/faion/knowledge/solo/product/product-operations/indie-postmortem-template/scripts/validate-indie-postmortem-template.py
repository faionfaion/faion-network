#!/usr/bin/env python3
"""validate-indie-postmortem-template.py — validate a indie-postmortem-template output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'event', 'timeline', 'contributing_factors', 'blast_radius', 'lessons', 'follow_up_actions', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['timeline', 'contributing_factors', 'lessons', 'follow_up_actions']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'event', 'blast_radius', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "indie-postmortem-template-example", "version": "1.0.0", "last_reviewed": "2026-05-23", "event": "event value", "timeline": ["item-1", "item-2", "item-3"], "contributing_factors": ["item-1", "item-2", "item-3"], "blast_radius": "blast_radius value", "lessons": ["item-1", "item-2", "item-3"], "follow_up_actions": ["item-1", "item-2", "item-3"], "owner": "@solo-founder"}')
