#!/usr/bin/env python3
"""validate-single-operator-funnel-rubric.py — validate a Single Operator Funnel Rubric artefact JSON against the output contract.

Usage:
  validate-single-operator-funnel-rubric.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('week_iso', 'operator', 'stages', 'broken_stage', 'next_action', 'time_spent_min', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['stages count != 4', 'more than one evidence source per stage', 'broken_stage missing or multi-valued', 'time_spent_min > 20', 'next_action absent on a week with any negative delta']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-single-operator-funnel-rubric.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
