#!/usr/bin/env python3
"""validate-solo-x-analytics-review.py — validate a Solo X Analytics Review artefact JSON against the output contract.

Usage:
  validate-solo-x-analytics-review.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'week_iso', 'metrics', 'trailing_4w_median', 'outliers', 'top_post', 'bottom_post', 'qualified_follower_pct', 'next_week_experiment', 'time_spent_min', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['metrics keys != exact 5-key set', 'next_week_experiment missing hypothesis OR variable OR success_metric', 'outliers includes posts <3x median', 'qualified_follower_pct missing when net_followers > 0', 'time_spent_min > 20']
...
def _self_test() -> int:
    fixture = {
        ...
        'week_iso': 'sample-' + 'week_iso',
        'metrics': {'k': 'v'},
        'trailing_4w_median': {'k': 'v'},
        'outliers': [{'k': 'v'}],
        'top_post': {'k': 'v'},
        'bottom_post': {'k': 'v'},
        'qualified_follower_pct': 0.55,
        'next_week_experiment': {'k': 'v'},
        'time_spent_min': 5,
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-solo-x-analytics-review.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
