#!/usr/bin/env python3
"""validate-serp-intent-classification-rubric.py — validate a SERP Intent Classification Rubric artefact JSON against the output contract.

Usage:
  validate-serp-intent-classification-rubric.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('query', 'primary_intent', 'primary_subtype', 'secondary_intent', 'serp_evidence', 'recommendation', 'classifier', 'classified_at')
FORBIDDEN_HINTS: tuple[str, ...] = ['primary_intent outside {I,C,T,N}', 'serp_evidence array has <2 entries', 'secondary_intent set when <30% of top-10 serves a different class', 'recommendation=SINGLE_BRIEF when classifier flagged AMBIGUOUS']
...
def _self_test() -> int:
    fixture = {
        ...
        'primary_subtype': 'sample-' + 'primary_subtype',
        'secondary_intent': 'sample-' + 'secondary_intent',
        'serp_evidence': [{'k': 'v'}],
        'recommendation': 'value',
        'classifier': 'sample-' + 'classifier',
        'classified_at': '2026-05-23T12:00:00Z',
    }
    errs = _check(fixture)
    if errs:
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-serp-intent-classification-rubric.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
