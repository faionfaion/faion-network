#!/usr/bin/env python3
"""validate-growth-twitter-x-growth.py — validate a Growth Twitter X Growth artefact JSON against the output contract.

Usage:
  validate-growth-twitter-x-growth.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'unique_angle', 'daily_post_target', 'weekly_thread_target', 'daily_reply_target', 'reply_target_list', 'kpi_set', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['daily_post_target outside 3..5 band', 'weekly_thread_target == 0', "reply_target_list contains accounts smaller than operator's account", "kpi_set headline == 'followers'", "unique_angle generic ('helping people grow on X')"]
...
def _self_test() -> int:
    fixture = {
        ...
        'daily_post_target': 5,
        'weekly_thread_target': 5,
        'daily_reply_target': 5,
        'reply_target_list': [{'k': 'v'}],
        'kpi_set': {'k': 'v'},
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-growth-twitter-x-growth.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
