#!/usr/bin/env python3
"""validate-growth-social-media-strategy.py — validate a Growth Social Media Strategy artefact JSON against the output contract.

Usage:
  validate-growth-social-media-strategy.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'primary_platforms', 'atomization_loop', 'weekly_time_budget_hours', 'engagement_quota', 'kpi_set', 'review_cadence_weeks', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['primary_platforms count > 2', 'weekly_time_budget_hours > 10 (not solo-realistic)', 'atomization_loop.weekly_cuts < 3', "kpi_set headline == 'followers'"]
...
def _self_test() -> int:
    fixture = {
        ...
        'primary_platforms': [{'k': 'v'}],
        'atomization_loop': {'k': 'v'},
        'weekly_time_budget_hours': 5,
        'engagement_quota': {'k': 'v'},
        'kpi_set': {'k': 'v'},
        'review_cadence_weeks': 5,
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-growth-social-media-strategy.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
