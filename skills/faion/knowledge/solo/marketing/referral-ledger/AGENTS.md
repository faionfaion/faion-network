...
maintainers: [faion]
content_id: "a008b03e6ee73fc6"
summary: Referral Ledger — pinned ledger for the indie hacker: fixed shape + named owner + evidence anchors + outcome review, so affiliate / referral check + payout stops being folklore and starts being a reviewable operating tool.
tags: [marketing, solo, ledger, referral]
---
# Referral Ledger

## Summary

**One-sentence:** Referral Ledger — pinned ledger for the indie hacker: fixed shape + named owner + evidence anchors + outcome review, so affiliate / referral check + payout stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In growth and marketing, the indie hacker runs affiliate / referral check + payout on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Monthly affiliate ritual needs a paper-trail template; ops-financial-planning is too generic. `referral-ledger` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the indie hacker starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored ledger committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p2-indie-hacker/Affiliate / referral check + payout`
