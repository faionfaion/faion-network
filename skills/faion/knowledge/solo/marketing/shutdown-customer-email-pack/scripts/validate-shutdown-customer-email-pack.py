#!/usr/bin/env python3
"""validate-shutdown-customer-email-pack.py — validate a Shutdown Customer Email Pack artefact JSON against the output contract.

Usage:
  validate-shutdown-customer-email-pack.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('artefact_id', 'owner', 'service_off_date', 'announce_email', 'refund_email', 'final_thanks_email', 'real_reason_sentence', 'refund_or_migration', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['from-line set to noreply@ or generic team alias', 'announce_email.send_date within 30 days of service_off_date', 'refund_or_migration missing (no remedy = abandonment)', "real_reason_sentence containing corporate-speak ('exciting new chapter', 'evolving in a new direction')", 'sequence count != 3']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-shutdown-customer-email-pack.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
