#!/usr/bin/env python3
"""validate-audience-to-paid-conversion-loop.py — validate a audience-to-paid-conversion-loop output JSON against the methodology schema.
...
REQUIRED = ["artefact_id", "version", "last_reviewed", "loop_id", "cohort_size", "window_days", "variant", "conversion_rate", "decision", "owner"]
ENUMS = {"decision": ["keep", "drop", "iterate", "log-only"]}
SEMVER_FIELDS = ["version"]
DATE_FIELDS = ["last_reviewed"]
INT_FIELDS = ["cohort_size", "window_days"]
ARRAY_FIELDS = []
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ["artefact_id", "loop_id", "variant", "conversion_rate", "owner"]
...
VALID_FIXTURE = json.loads("{\"artefact_id\": \"loop-2026-w20\", \"version\": \"1.1.0\", \"last_reviewed\": \"2026-05-23\", \"loop_id\": \"LOOP-2026-W20\", \"cohort_size\": 312, \"window_days\": 7, \"variant\": \"subject-line-emoji\", \"conversion_rate\": \"4.8%\", \"decision\": \"keep\", \"owner\": \"@ruslan\"}")
INVALID_FIXTURE = json.loads("{\"loop_id\": \"LOOP-X\", \"cohort_size\": 30, \"window_days\": 7, \"variant\": \"subject + price + cta\", \"decision\": \"keep\", \"owner\": \"team\"}")
