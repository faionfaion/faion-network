#!/usr/bin/env python3
"""validate-zero-click-search-adaptation.py — validate a Zero-Click Search Adaptation artefact JSON against the output contract.

Usage:
  validate-zero-click-search-adaptation.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('page_url', 'target_query', 'owner', 'jsonld_article', 'jsonld_faq', 'lede_rewrite', 'original_data_points', 'kpi_set', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["owner is 'team' / 'we' / 'us' / empty", "kpi_set contains only 'sessions'", 'FAQ JSON-LD references questions not visible in rendered DOM', 'lede outside 40-60 word band', 'data points without year + source']
...
def _self_test() -> int:
    fixture = {
        ...
        'jsonld_article': {'k': 'v'},
        'jsonld_faq': {'k': 'v'},
        'lede_rewrite': 'sample-' + 'lede_rewrite',
        'original_data_points': [{'k': 'v'}],
        'kpi_set': {'k': 'v'},
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-zero-click-search-adaptation.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
