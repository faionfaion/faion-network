#!/usr/bin/env python3
"""validate-solo-niche-disqualifier-checklist.py — validate a Solo Niche Disqualifier Checklist artefact JSON against the output contract.

Usage:
  validate-solo-niche-disqualifier-checklist.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'specialisation_statement', 'floor_rate', 'hard_no_list', 'disqualifier_signals', 'rejection_reply_template', 'max_eval_minutes', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['disqualifier_signals count != 5', 'hard_no_list count < 5', 'rejection_reply_template > 80 words', 'max_eval_minutes > 5', "rejection_reply contains counter-offer phrases ('but if you bump to', 'happy to do smaller scope')"]
...
def _self_test() -> int:
    fixture = {
        ...
        'specialisation_statement': 'sample-' + 'specialisation_statement',
        'floor_rate': {'k': 'v'},
        'hard_no_list': [{'k': 'v'}],
        'disqualifier_signals': [{'k': 'v'}],
        'rejection_reply_template': 'sample-' + 'rejection_reply_template',
        'max_eval_minutes': 5,
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-solo-niche-disqualifier-checklist.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
