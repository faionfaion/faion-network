#!/usr/bin/env python3
"""validate-weekly-growth-review-rhythm.py — validate a weekly-growth-review-rhythm output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'metrics', 'hypothesis_log_url', 'decision_template', 'review_slot', 'quarterly_revisit_date', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed', 'quarterly_revisit_date']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['metrics']
OBJECT_FIELDS = ['review_slot']
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'hypothesis_log_url', 'decision_template', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "weekly-growth-review-2026-q2", "version": "1.0.0", "last_reviewed": "2026-05-23", "metrics": [{"name": "MRR", "type": "nsm", "definition": "monthly recurring revenue"}, {"name": "signups", "type": "leading", "definition": "free signups / week"}, {"name": "trial\\u2192paid", "type": "leading", "definition": "trial conversion %"}, {"name": "CAC", "type": "efficiency", "definition": "ad spend / new paid"}, {"name": "churn", "type": "health", "definition": "monthly logo churn %"}], "hypothesis_log_url": "notion://workspace/hypotheses", "decision_template": "what changed | which metric | hypothesis | decision (continue/kill/double/redesign)", "review_slot": {"day_of_week": "Monday", "time": "09:00", "duration_min": 45}, "quarterly_revisit_date": "2026-08-15", "owner": "@ruslan"}')
INVALID_FIXTURE = json.loads('{"metrics": [{"name": "MRR"}], "hypothesis_log_url": "", "decision_template": "", "owner": "team"}')
