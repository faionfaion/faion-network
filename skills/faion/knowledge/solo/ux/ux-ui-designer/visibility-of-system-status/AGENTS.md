...
summary: Produce a Status Audit report (Nielsen Heuristic #1) — per-action three-state coverage (loading/success/error), feedback latency thresholds (≤100ms response, spinner 1-3s, progress bar ≥3s, percentage+cancel ≥10s), and aria-live correctness.
