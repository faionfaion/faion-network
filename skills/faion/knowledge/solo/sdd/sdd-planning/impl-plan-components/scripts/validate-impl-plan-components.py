#!/usr/bin/env python3
"""validate-impl-plan-components.py — validate a impl-plan-components output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'components', 'tasks', 'graph', 'cycles_detected', 'orphan_components', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['components', 'tasks', 'cycles_detected', 'orphan_components']
OBJECT_FIELDS = ['graph']
BOOL_FIELDS = []
...
VALID_FIXTURE = json.loads('{"artefact_id": "impl-plan-components-example", "version": "1.0.0", "last_reviewed": "2026-05-23", "components": ["item-1", "item-2", "item-3"], "tasks": ["item-1", "item-2", "item-3"], "graph": {"key": "value"}, "cycles_detected": ["item-1", "item-2", "item-3"], "orphan_components": ["item-1", "item-2", "item-3"], "owner": "@solo-founder"}')
