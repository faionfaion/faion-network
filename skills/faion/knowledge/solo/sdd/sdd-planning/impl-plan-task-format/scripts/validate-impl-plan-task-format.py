#!/usr/bin/env python3
"""validate-impl-plan-task-format.py — validate a impl-plan-task-format output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'task_id', 'task_slug', 'est_tokens', 'status', 'component', 'depends_on', 'acceptance', 'owner']
ENUMS = {'status': ['todo', 'in-progress', 'done', 'blocked']}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = ['est_tokens']
NUM_FIELDS = []
ARRAY_FIELDS = ['depends_on', 'acceptance']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'task_id', 'task_slug', 'component', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "impl-plan-task-format-example", "version": "1.0.0", "last_reviewed": "2026-05-23", "task_id": "task_id value", "task_slug": "task_slug value", "est_tokens": 1, "status": "todo", "component": "component value", "depends_on": ["item-1", "item-2", "item-3"], "acceptance": ["item-1", "item-2", "item-3"], "owner": "@solo-founder"}')
