#!/usr/bin/env python3
"""validate-engagement-pattern-memory.py — validate a Engagement Pattern Memory artefact JSON against the output contract.

Usage:
  validate-engagement-pattern-memory.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('engagement_id', 'memory_file_path', 'repo_conventions', 'reviewer_preferences', 'deploy_quirks', 'recurring_traps', 'glossary', 'resolved_questions', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['engagement_id shared across files', 'memory_file_path missing', 'any section absent', 'client PII beyond engagement_id']
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'repo_conventions': {'k': 'v'},
        'reviewer_preferences': {'k': 'v'},
        'deploy_quirks': [{'k': 'v'}],
        'recurring_traps': [{'k': 'v'}],
        'glossary': {'k': 'v'},
        'resolved_questions': [{'k': 'v'}],
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-engagement-pattern-memory.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
