#!/usr/bin/env python3
"""validate-api-first-development.py — validate a API-First Development artefact JSON against the output contract.

Usage:
  validate-api-first-development.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('openapi_spec_path', 'openapi_version', 'endpoints', 'mock_server_url', 'contract_test_config', 'version', 'owner', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["openapi_version not starting with '3.1'", 'endpoints empty', 'contract_test_config missing', 'mock_server_url empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-api-first-development.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
