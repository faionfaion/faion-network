#!/usr/bin/env python3
"""validate-mistake-memory.py — validate a Mistake Memory artefact JSON against the output contract.

Usage:
  validate-mistake-memory.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('mistake_id', 'severity', 'what_happened', 'five_whys', 'prevention', 'ci_rule_created', 'occurrence_count', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['five_whys length < 3', 'prevention matches /be careful|pay attention|remember/i', 'occurrence_count > 1 AND ci_rule_created == false', 'severity not in allowed set']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-mistake-memory.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
