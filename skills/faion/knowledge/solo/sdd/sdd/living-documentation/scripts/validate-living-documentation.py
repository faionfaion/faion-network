#!/usr/bin/env python3
"""validate-living-documentation.py — validate a Living Documentation artefact JSON against the output contract.

Usage:
  validate-living-documentation.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('docs_root', 'docs_framework', 'auto_generated_sections', 'hand_authored_sections', 'ci_checks', 'changelog_source', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['docs_root outside source repo', 'any auto-generated section without AUTO-GENERATED marker', "changelog_source == 'manual'", 'ci_checks.link_validation != true']
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'auto_generated_sections': [{'k': 'v'}],
        'hand_authored_sections': [{'k': 'v'}],
        'ci_checks': {'k': 'v'},
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-living-documentation.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
