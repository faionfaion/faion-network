#!/usr/bin/env python3
"""validate-code-review-cycle.py — validate a Code Review Cycle artefact JSON against the output contract.

Usage:
  validate-code-review-cycle.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('pr_id', 'prescreen_findings', 'reviewer_findings', 'merged_findings', 'block_count', 'reflexion_writeback', 'verdict', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['verdict == merge-ready AND block_count > 0', 'any finding without severity OR line ref', 'reflexion_writeback empty when novel findings exist']
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        'pr_id': 'sample-pr_id',
        'prescreen_findings': [{'k': 'v'}],
        'reviewer_findings': [{'k': 'v'}],
        'merged_findings': [{'k': 'v'}],
        'block_count': 3,
        'reflexion_writeback': [{'k': 'v'}],
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-code-review-cycle.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
