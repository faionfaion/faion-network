#!/usr/bin/env python3
"""validate-pattern-memory.py — validate a Pattern Memory artefact JSON against the output contract.

Usage:
  validate-pattern-memory.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('pattern_id', 'title', 'confidence', 'contexts_used', 'rationale', 'synced_to_claude_md', 'occurrence_count', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['contexts_used length < 2', 'confidence < 0.5 OR confidence > 0.95', 'confidence >= 0.8 AND synced_to_claude_md == false', 'rationale empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-pattern-memory.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
