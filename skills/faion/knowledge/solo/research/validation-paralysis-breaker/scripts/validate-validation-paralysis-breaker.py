#!/usr/bin/env python3
"""validate-validation-paralysis-breaker.py — validate a Validation Paralysis Breaker artefact JSON against the output contract.

Usage:
  validate-validation-paralysis-breaker.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('hypothesis', 'falsification_trigger', 'budget_start', 'budget_end', 'verdict', 'named_risk', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['budget_end > budget_start + 72h (unless extend-once verdict logged)', "verdict ∈ {'continue', 'tbd', ''}", 'falsification_trigger empty', 'named_risk empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-validation-paralysis-breaker.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
