#!/usr/bin/env python3
"""validate-research-repo-file-naming-convention.py — validate a Research Repo File-Naming Convention artefact JSON against the output contract.

Usage:
  validate-research-repo-file-naming-convention.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('repo_root', 'folder_tree', 'filename_grammar', 'manifest_schema', 'anonymisation_policy', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['filename_grammar regex matches names containing real PII', 'folder_tree missing any of {recruit, run, tag, synthesise, archive}', 'manifest_schema empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-research-repo-file-naming-convention.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
