#!/usr/bin/env python3
"""validate-success-metrics-definition.py — validate a Success Metrics Definition artefact JSON against the output contract.

Usage:
  validate-success-metrics-definition.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('north_star', 'aarrr_kpis', 'baselines', 'targets', 'vanity_excluded', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['aarrr_kpis length > 5', 'north_star is array', 'any KPI missing baseline or target', 'vanity_excluded empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-success-metrics-definition.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
