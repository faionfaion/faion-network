#!/usr/bin/env python3
"""validate-user-interviews.py — validate a User Interviews artefact JSON against the output contract.

Usage:
  validate-user-interviews.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('session_id', 'respondent', 'script_used', 'transcript_path', 'behavioural_ask_outcome', 'insights', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['any question in script verb ∈ {would, could, might}', 'behavioural_ask_outcome empty', 'transcript_path missing OR not diarized', 'any insight without frequency_count']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-user-interviews.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
