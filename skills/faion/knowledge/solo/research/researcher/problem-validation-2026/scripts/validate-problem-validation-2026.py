#!/usr/bin/env python3
"""validate-problem-validation-2026.py — validate a Problem Validation 2026 artefact JSON against the output contract.

Usage:
  validate-problem-validation-2026.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('hypothesis', 'evidence_ledger', 'cold_respondent_count', 'verdict', 'next_revalidation_due', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["verdict == 'validated' AND cold_tier12_count < 3", 'evidence_ledger length < 10', 'any ledger entry missing tier or citation', 'next_revalidation_due > last_reviewed + 90 days']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-problem-validation-2026.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
