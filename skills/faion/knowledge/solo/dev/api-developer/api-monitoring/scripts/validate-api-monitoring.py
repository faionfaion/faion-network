#!/usr/bin/env python3
"""validate-api-monitoring.py

Validate a API Monitoring artefact against the JSON Schema (draft-07) declared in
...
NUM_FIELDS = []
BOOL_FIELDS = []
ARRAY_FIELDS = ["structured_log_fields", "slos", "alert_rules", "evidence"]
OBJECT_FIELDS = ["red_metrics"]
STRING_FIELDS = ["api_id", "metrics_backend", "liveness_probe", "readiness_probe", "status"]
...
OK = {'artefact_id': 'api-monitoring-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'api_id': 'draft', 'metrics_backend': 'draft', 'red_metrics': {'key': 'value'}, 'structured_log_fields': ['draft-item'], 'liveness_probe': 'draft', 'readiness_probe': 'draft', 'slos': ['draft-item'], 'alert_rules': ['draft-item']}
BAD = {"foo": "bar"}
