#!/usr/bin/env python3
"""validate-api-testing.py

Validate a API Testing artefact against the JSON Schema (draft-07) declared in
...
NUM_FIELDS = []
BOOL_FIELDS = []
ARRAY_FIELDS = ["frameworks", "ci_jobs", "evidence"]
OBJECT_FIELDS = ["pyramid", "negative_coverage", "contract_tests"]
STRING_FIELDS = ["api_id", "spec_conformance_tool", "status"]
...
OK = {'artefact_id': 'api-testing-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'api_id': 'draft', 'pyramid': {'key': 'value'}, 'frameworks': ['draft-item'], 'negative_coverage': {'key': 'value'}, 'contract_tests': {'key': 'value'}, 'spec_conformance_tool': 'draft', 'ci_jobs': ['draft-item']}
BAD = {"foo": "bar"}
