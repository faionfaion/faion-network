#!/usr/bin/env python3
"""validate-api-rest-design.py

Validate a API REST Design artefact against the JSON Schema (draft-07) declared in
...
NUM_FIELDS = []
BOOL_FIELDS = ["hateoas"]
ARRAY_FIELDS = ["resources", "evidence"]
OBJECT_FIELDS = ["verb_semantics", "status_code_map"]
STRING_FIELDS = ["api_id", "base_url", "idempotency_header", "status"]
...
OK = {'artefact_id': 'api-rest-design-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'api_id': 'draft', 'base_url': 'draft', 'resources': ['draft-item'], 'verb_semantics': {'key': 'value'}, 'status_code_map': {'key': 'value'}, 'idempotency_header': 'draft', 'hateoas': True}
BAD = {"foo": "bar"}
