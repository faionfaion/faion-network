#!/usr/bin/env python3
"""validate-a11y-no-regression-pr-gate.py

Validate an artefact produced by the a11y-no-regression-pr-gate methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/a11y-no-regression-pr-gate.json", "type": "object", "required": ["artefact_id", "owner", "baseline_path", "waiver_path", "route_map_path", "severity_block", "waiver_max_days", "version", "last_reviewed"], "properties": {"artefact_id": {"type": "string", "minLength": 1}, "owner": {"type": "string", "minLength": 1}, "baseline_path": {"type": "string", "minLength": 1}, "waiver_path": {"type": "string", "minLength": 1}, "route_map_path": {"type": "string", "minLength": 1}, "severity_block": {"type": "array", "minItems": 1, "items": {"type": "string", "enum": ["critical", "serious", "moderate", "minor"]}}, "severity_ack": {"type": "array", "items": {"type": "string", "enum": ["critical", "serious", "moderate", "minor"]}}, "waiver_max_days": {"type": "integer", "minimum": 1, "maximum": 90}, "nightly_full_scan": {"type": "boolean"}, "version": {"type": "string", "minLength": 1}, "last_reviewed": {"type": "string", "minLength": 1}}}')
