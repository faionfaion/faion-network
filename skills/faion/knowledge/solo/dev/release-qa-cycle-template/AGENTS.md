---
slug: release-qa-cycle-template
tier: solo
group: dev
domain: dev
version: 1.0.0
...
maintainers: [faion]
content_id: "9d4c83815882d2c9"
summary: Release QA Cycle Template — pinned template for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so major release qa cycle: regression + smoke + uat stops being folklore and starts being a reviewable operating tool.
tags: [dev, solo, template, release, cycle]
---
# Release QA Cycle Template

## Summary

**One-sentence:** Release QA Cycle Template — pinned template for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so major release qa cycle: regression + smoke + uat stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs major release qa cycle: regression + smoke + uat on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Release planning exists on product side; QA-side checklist for cut → regression → UAT → sign-off → release is missing. `release-qa-cycle-template` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored template committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Major release QA cycle: regression + smoke + UAT`
