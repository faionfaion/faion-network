#!/usr/bin/env python3
"""validate-api-contract-pattern-selection.py

Validate a API Contract Pattern Selection artefact against the JSON Schema (draft-07) declared in
...
INT_FIELDS = ["consumer_count"]
NUM_FIELDS = []
BOOL_FIELDS = []
ARRAY_FIELDS = ["tooling_needs"]
OBJECT_FIELDS = []
STRING_FIELDS = ["api_id", "evolution_rate", "team_boundary", "verdict", "rationale"]
...
OK = {'artefact_id': 'api-contract-pattern-selection-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'api_id': 'draft', 'consumer_count': 1, 'tooling_needs': ['draft-item'], 'evolution_rate': 'draft', 'team_boundary': 'draft', 'verdict': 'draft', 'rationale': 'draft'}
BAD = {"foo": "bar"}
