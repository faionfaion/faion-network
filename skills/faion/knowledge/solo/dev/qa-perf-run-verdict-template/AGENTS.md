...
summary: QA Perf Run Verdict Template — pinned template for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so performance test run + analysis stops being folklore and starts being a reviewable operating tool.
tags: [dev, solo, template, perf, run, verdict]
---
# QA Perf Run Verdict Template

## Summary

**One-sentence:** QA Perf Run Verdict Template — pinned template for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so performance test run + analysis stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs performance test run + analysis on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. perf-test-basics covers running a test; nothing covers the verdict artifact (ship/hold/investigate + chart + next experiment) that closes the QA loop. `qa-perf-run-verdict-template` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored template committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Performance test run + analysis`
