#!/usr/bin/env python3
"""validate-tech-debt-budget-for-solos.py

Validate an artefact produced by the tech-debt-budget-for-solos methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/tech-debt-budget-for-solos.json", "type": "object", "required": ["cycle_length_days", "cap_percent", "items", "owner", "last_signed_utc"], "properties": {"cycle_length_days": {"type": "integer", "minimum": 1, "maximum": 90}, "cap_percent": {"type": "integer", "minimum": 0, "maximum": 20}, "items": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["id", "title", "impact", "interest", "decision"], "properties": {"id": {"type": "string"}, "title": {"type": "string"}, "impact": {"type": "integer", "minimum": 1, "maximum": 5}, "interest": {"type": "integer", "minimum": 1, "maximum": 5}, "decision": {"type": "string", "enum": ["ship", "pay", "monitor"]}}}}, "owner": {"type": "string"}, "last_signed_utc": {"type": "string"}}}')
