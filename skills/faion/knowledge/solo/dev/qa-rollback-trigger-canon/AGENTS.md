---
slug: qa-rollback-trigger-canon
tier: solo
group: dev
domain: dev
version: 1.0.0
...
maintainers: [faion]
content_id: "2ab7f8ca00321ca9"
summary: QA Rollback Trigger Canon — pinned canon for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so release-candidate smoke pack stops being folklore and starts being a reviewable operating tool.
tags: [dev, solo, canon, rollback, trigger]
---
# QA Rollback Trigger Canon

## Summary

**One-sentence:** QA Rollback Trigger Canon — pinned canon for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so release-candidate smoke pack stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs release-candidate smoke pack on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Companion to the RC smoke: which metric thresholds trigger an automatic rollback decision. Belongs with QA, not just SRE, because QA defines the user-visible signals. `qa-rollback-trigger-canon` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored canon committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Release-candidate smoke pack`
