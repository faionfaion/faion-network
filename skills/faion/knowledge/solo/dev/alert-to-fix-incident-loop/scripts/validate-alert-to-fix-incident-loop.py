#!/usr/bin/env python3
"""validate-alert-to-fix-incident-loop.py

Validate a Alert-to-Fix Incident Loop artefact against the JSON Schema (draft-07) declared in
...
OBJECT_FIELDS = []
STRING_FIELDS = ["incident_id", "alert_id", "severity", "triage", "mitigation", "root_cause", "fix_pr", "postmortem_url", "alert_quality_patch", "status"]
...
OK = {'artefact_id': 'alert-to-fix-incident-loop-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'incident_id': 'draft', 'alert_id': 'draft', 'severity': 'draft', 'triage': 'draft', 'mitigation': 'draft', 'root_cause': 'draft', 'fix_pr': 'draft', 'postmortem_url': 'draft', 'alert_quality_patch': 'draft'}
BAD = {"foo": "bar"}
