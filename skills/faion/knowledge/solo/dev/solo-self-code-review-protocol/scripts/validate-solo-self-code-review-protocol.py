#!/usr/bin/env python3
"""validate-solo-self-code-review-protocol.py

Validate an artefact produced by the solo-self-code-review-protocol methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/solo-self-code-review-protocol.json", "type": "object", "required": ["pr_id", "checklist_passed", "ai_review_present", "risk_flags_hit", "ci_status"], "properties": {"pr_id": {"type": "string"}, "checklist_passed": {"type": "object", "required": ["items_total", "items_passed"]}, "ai_review_present": {"type": "boolean"}, "risk_flags_hit": {"type": "array", "items": {"type": "string"}}, "cooloff_start": {"type": "string"}, "cooloff_end": {"type": "string"}, "ci_status": {"type": "string", "enum": ["green", "red", "yellow"]}, "merge_decision": {"type": "string", "enum": ["merge", "revert", "hold"]}}}')
