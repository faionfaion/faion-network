...
maintainers: [faion]
content_id: "76c0375ce97779d8"
summary: QA Test Strategy Template — pinned template for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so test strategy from scratch for a new product stops being folklore and starts being a reviewable operating tool.
tags: [dev, solo, template, test, strategy]
---
# QA Test Strategy Template

## Summary

**One-sentence:** QA Test Strategy Template — pinned template for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so test strategy from scratch for a new product stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs test strategy from scratch for a new product on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. There is no canonical 'write a test strategy doc' methodology — only per-layer testing recipes. QAs starting on new products have to invent the structure each time. `qa-test-strategy-template` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored template committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Test strategy from scratch for a new product`
