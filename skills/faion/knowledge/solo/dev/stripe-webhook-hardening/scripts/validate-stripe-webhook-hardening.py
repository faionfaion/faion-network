#!/usr/bin/env python3
"""validate-stripe-webhook-hardening.py

Validate an artefact produced by the stripe-webhook-hardening methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/stripe-webhook-hardening.json", "type": "object", "required": ["timestamp_tolerance_sec", "rotation_cadence_days", "raw_body_verified", "edge_rate_limit_rpm", "audit_log_destination", "owner"], "properties": {"timestamp_tolerance_sec": {"type": "integer", "minimum": 60, "maximum": 300}, "rotation_cadence_days": {"type": "integer", "minimum": 1, "maximum": 90}, "raw_body_verified": {"type": "boolean"}, "edge_rate_limit_rpm": {"type": "integer", "minimum": 1}, "audit_log_destination": {"type": "string"}, "owner": {"type": "string"}, "ip_allowlist": {"type": "array", "items": {"type": "string"}}}}')
