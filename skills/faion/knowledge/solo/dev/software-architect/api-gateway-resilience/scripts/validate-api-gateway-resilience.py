#!/usr/bin/env python3
"""validate-api-gateway-resilience.py — stdlib-only validator for the api-gateway-resilience output artefact.
...
REQUIRED = ['artefact_id', 'owner', 'version', 'last_reviewed', 'rate_limits', 'circuit_breakers', 'retries', 'timeouts']
...
OK_JSON = '{"artefact_id": "api-gateway-resilience-acme-2026-05-23", "owner": "Ruslan Faion <ruslan@faion.net>", "version": "1.0.0", "last_reviewed": "2026-05-23", "rate_limits": [{"scope": "consumer", "requests_per_minute": 600}], "circuit_breakers": [{"upstream": "payment-service", "failure_threshold_pct": 50}], "retries": [{"method": "POST", "max_attempts": 2, "require_idempotency_key": true}], "timeouts": {"default_ms": 5000}}'
BAD_JSON = '{"owner": "team", "rate_limits": [], "retries": [{"method": "POST", "max_attempts": 5}], "timeouts": {"default_ms": 60000}}'
