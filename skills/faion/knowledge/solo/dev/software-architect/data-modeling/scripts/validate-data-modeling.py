#!/usr/bin/env python3
"""validate-data-modeling.py — stdlib-only validator for the data-modeling output artefact.
...
REQUIRED = ['artefact_id', 'owner', 'version', 'last_reviewed', 'engine', 'conceptual', 'logical', 'physical', 'migration_plan']
...
OK_JSON = '{"artefact_id": "schema-orders-2026-05-23", "owner": "Ruslan Faion <ruslan@faion.net>", "version": "1.0.0", "last_reviewed": "2026-05-23", "engine": "postgres 16", "conceptual": [{"entity": "Order"}, {"entity": "LineItem"}], "logical": [{"entity": "Order", "attrs": ["id", "customer_id", "total"]}], "physical": [{"table": "orders", "ddl": "CREATE TABLE orders ..."}], "migration_plan": {"ddl_path": "migrations/2026-05-23-orders.sql", "rollback": "drop + restore"}}'
BAD_JSON = '{"owner": "team", "conceptual": [], "physical": [{"table": "orders"}], "migration_plan": {}}'
