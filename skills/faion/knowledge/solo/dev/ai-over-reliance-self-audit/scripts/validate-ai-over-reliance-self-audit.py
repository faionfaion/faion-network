#!/usr/bin/env python3
"""validate-ai-over-reliance-self-audit.py

Validate a AI Over-Reliance Self-Audit artefact against the JSON Schema (draft-07) declared in
...
INT_FIELDS = []
NUM_FIELDS = ["score", "threshold"]
BOOL_FIELDS = []
ARRAY_FIELDS = ["checklist_items", "remediation"]
OBJECT_FIELDS = []
STRING_FIELDS = ["audit_period", "next_audit_date"]
...
OK = {'artefact_id': 'ai-over-reliance-self-audit-2026-05-23', 'owner': 'ruslan@faion.net', 'last_touched': '2026-05-23T12:00:00Z', 'template_version': '1.1.0', 'status': 'ready_for_review', 'evidence': [{'source': 'https://example.com/source-1', 'citation': 'verbatim quote from source'}], 'audit_period': 'draft', 'checklist_items': ['draft-item'], 'score': 1.0, 'threshold': 1.0, 'remediation': ['draft-item'], 'next_audit_date': 'draft'}
BAD = {"foo": "bar"}
