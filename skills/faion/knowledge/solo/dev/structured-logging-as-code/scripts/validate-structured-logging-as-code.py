#!/usr/bin/env python3
"""validate-structured-logging-as-code.py

Validate an artefact produced by the structured-logging-as-code methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/structured-logging-as-code.json", "type": "object", "required": ["required_fields", "redaction_rules", "trace_correlation", "per_env_levels", "ci_parser_fixture", "owner"], "properties": {"required_fields": {"type": "array", "minItems": 4, "items": {"type": "string"}}, "redaction_rules": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["field_or_regex", "policy"], "properties": {"field_or_regex": {"type": "string"}, "policy": {"type": "string", "enum": ["drop", "hash", "mask"]}}}}, "trace_correlation": {"type": "string", "enum": ["otel", "vendor", "manual"]}, "per_env_levels": {"type": "object", "required": ["dev", "staging", "prod"], "properties": {"dev": {"type": "string"}, "staging": {"type": "string"}, "prod": {"type": "string"}}}, "ci_parser_fixture": {"type": "string"}, "owner": {"type": "string"}}}')
        ...
        if k in props:
            ...
            if t == "array" and isinstance(v, list):
                ...
                if "maxItems" in spec and len(v) > spec["maxItems"]:
                    errs.append(f"field {k} must have maxItems={spec['maxItems']}")
...
def _placeholder(spec):
    ...
    if t == "array":
        ...
        return [_placeholder(items) for _ in range(max(min_items, 1))]
    if t == "object":
        out = {}
        for rk in spec.get("required", []):
