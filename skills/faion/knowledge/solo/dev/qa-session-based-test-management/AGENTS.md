---
slug: qa-session-based-test-management
tier: solo
group: dev
domain: dev
version: 1.0.0
...
maintainers: [faion]
content_id: "2d01be1846c5bd9d"
summary: QA Session Based Test Management — pinned method for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so exploratory testing session on new feature stops being folklore and starts being a reviewable operating tool.
tags: [dev, solo, method, session, based, test, management]
---
# QA Session Based Test Management

## Summary

**One-sentence:** QA Session Based Test Management — pinned method for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so exploratory testing session on new feature stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs exploratory testing session on new feature on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Companion to charters — how to log, debrief, and turn session findings into a feedback loop. Standard SBTM framework, currently uncovered. `qa-session-based-test-management` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Exploratory testing session on new feature`
