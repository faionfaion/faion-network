#!/usr/bin/env python3
"""validate-supply-chain-risk-checklist-spike.py

Validate an artefact produced by the supply-chain-risk-checklist-spike methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/supply-chain-risk-checklist-spike.json", "type": "object", "required": ["library", "ecosystem", "signals", "aggregate_score", "verdict", "remediation", "decay_date"], "properties": {"library": {"type": "string"}, "ecosystem": {"type": "string", "enum": ["npm", "pypi", "crates", "maven", "rubygems", "go"]}, "signals": {"type": "object", "required": ["license", "maintainer_count", "last_release_age", "cve_history_12mo", "stars_to_downloads", "transitive_depth", "vendor_lock_in"]}, "aggregate_score": {"type": "integer", "minimum": 0, "maximum": 21}, "verdict": {"type": "string", "enum": ["green", "yellow", "red"]}, "remediation": {"type": "string", "enum": ["adopt", "pin-and-monitor", "fork", "replace", "decline"]}, "decay_date": {"type": "string"}, "data_freshness": {"type": "string", "enum": ["fresh", "stale"]}}}')
