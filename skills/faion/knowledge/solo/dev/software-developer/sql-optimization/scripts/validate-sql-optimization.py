...
Validate an artefact produced by the sql-optimization methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/sql-optimization.json", "type": "object", "required": ["query_id", "baseline_plan", "applied_change", "after_plan", "wall_clock_delta_ms"], "properties": {"query_id": {"type": "string"}, "baseline_plan": {"type": "object", "required": ["plan_text", "actual_rows", "buffers"]}, "applied_change": {"type": "string", "enum": ["index_added", "index_dropped", "query_rewrite", "analyze", "materialized_view", "vacuum"]}, "after_plan": {"type": "object", "required": ["plan_text", "actual_rows"]}, "wall_clock_delta_ms": {"type": "integer"}, "sample_size": {"type": "integer", "minimum": 1}}}')
