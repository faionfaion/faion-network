#!/usr/bin/env python3
"""validate-performance-testing.py

Validate an artefact produced by the performance-testing methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/performance-testing.json", "type": "object", "required": ["test_type", "slo", "baseline_sha", "results", "verdict"], "properties": {"test_type": {"type": "string", "enum": ["load", "stress", "spike", "endurance", "scalability"]}, "slo": {"type": "object", "required": ["p95_ms", "rps", "error_rate_ceiling"]}, "baseline_sha": {"type": "string"}, "results": {"type": "object", "required": ["p50_ms", "p95_ms", "p99_ms", "rps", "error_rate"]}, "verdict": {"type": "string", "enum": ["pass", "fail", "regression"]}, "regression_pct": {"type": "number"}}}')
