...
maintainers: [faion]
summary: End-to-end playbook for ai pair coding prompt patterns that walks an operator from trigger to closed outcome with named artefacts at each step.
content_id: "fc280734bab34023"
tags: [ai, dev, playbook]
---
# AI Pair Coding Prompt Patterns

## Summary

**One-sentence:** End-to-end playbook for ai pair coding prompt patterns that walks an operator from trigger to closed outcome with named artefacts at each step.

**One-paragraph:** End-to-end playbook for ai pair coding prompt patterns that walks an operator from trigger to closed outcome with named artefacts at each step. Alex codes through Claude/Cursor every day. faion has ZERO methodology for prompting a coding agent — what a good task prompt looks like, when to start a new chat, how to keep diffs surgical. All Claude Code / MCP content lives at geek tier, which is wrong: this is the core solo workflow.

## Applies If (ALL must hold)

- You are executing the cross-cutting workflow addressed by ai pair coding prompt patterns end to end.
