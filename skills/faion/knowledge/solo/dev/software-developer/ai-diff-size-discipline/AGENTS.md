...
maintainers: [faion]
summary: End-to-end playbook for ai diff size discipline that walks an operator from trigger to closed outcome with named artefacts at each step.
content_id: "0d808dad13fe2a67"
...
**One-sentence:** End-to-end playbook for ai diff size discipline that walks an operator from trigger to closed outcome with named artefacts at each step.

**One-paragraph:** End-to-end playbook for ai diff size discipline that walks an operator from trigger to closed outcome with named artefacts at each step. Solo dev review bandwidth = one human reading the diff. Need an explicit rule set for capping AI diff size, splitting changes, and what to reject without reading. Missing entirely.

## Applies If (ALL must hold)

- You are executing the cross-cutting workflow addressed by ai diff size discipline end to end.
