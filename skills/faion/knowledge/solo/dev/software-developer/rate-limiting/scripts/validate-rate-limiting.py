#!/usr/bin/env python3
"""validate-rate-limiting.py

Validate an artefact produced by the rate-limiting methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/rate-limiting.json", "type": "object", "required": ["classes", "key_strategy", "storage", "headers", "fail_mode"], "properties": {"classes": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["name", "algorithm", "rps", "burst"]}}, "key_strategy": {"type": "string", "enum": ["user_id", "tenant_id", "ip", "api_key", "composite"]}, "storage": {"type": "object", "required": ["backend", "ttl_seconds"]}, "headers": {"type": "object", "required": ["retry_after", "ratelimit_remaining"]}, "fail_mode": {"type": "string", "enum": ["open", "closed"]}, "bypass_paths": {"type": "array", "items": {"type": "string"}}}}')
