#!/usr/bin/env python3
"""validate-python-async-patterns.py

Validate an artefact produced by the python-async-patterns methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/python-async-patterns.json", "type": "object", "required": ["runtime", "async_drivers", "timeout_policy", "fanout_caps", "sync_offload_pattern"], "properties": {"runtime": {"type": "string", "enum": ["asyncio", "asyncio+uvloop"]}, "async_drivers": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "timeout_policy": {"type": "object", "required": ["default_ms"]}, "fanout_caps": {"type": "object", "required": ["max_parallel", "mechanism"]}, "sync_offload_pattern": {"type": "string", "enum": ["to_thread", "executor_pool", "none"]}, "lint_rules": {"type": "array", "items": {"type": "string"}}}}')
