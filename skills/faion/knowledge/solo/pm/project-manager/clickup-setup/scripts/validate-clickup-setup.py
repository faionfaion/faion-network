#!/usr/bin/env python3
"""validate-clickup-setup.py — validate a clickup-setup output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'workspace_url', 'spaces', 'statuses', 'automations_count', 'clickapps', 'views_per_list', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = ['automations_count', 'views_per_list']
NUM_FIELDS = []
ARRAY_FIELDS = ['spaces', 'statuses', 'clickapps']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'workspace_url', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "clickup-setup-2026-q2", "version": "1.0.0", "last_reviewed": "2026-05-23", "workspace_url": "https://app.clickup.com/123", "spaces": ["eng", "ops", "sales"], "statuses": ["backlog", "todo", "in-progress", "blocked", "review", "done", "cancelled"], "automations_count": 7, "clickapps": ["time-tracking", "custom-fields", "dependencies", "sprints"], "views_per_list": 4, "owner": "@ruslan"}')
INVALID_FIXTURE = json.loads('{"workspace_url": "", "spaces": ["a", "b", "c", "d", "e", "f"], "statuses": ["s1", "s2", "s3", "s4", "s5", "s6", "s7", "s8", "s9"], "automations_count": 22, "clickapps": ["mind-maps", "whiteboards", "chat", "docs", "screen-recording"], "views_per_list": 9, "owner": "team"}')
