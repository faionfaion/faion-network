#!/usr/bin/env python3
"""validate-nomad-low-energy-mode.py — validate a nomad-low-energy-mode output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'activation_date', 'expected_return', 'allowed_tasks', 'auto_reply_text', 'escalation_contact', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed', 'activation_date', 'expected_return']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['allowed_tasks']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'auto_reply_text', 'escalation_contact', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "low-energy-mode-2026-w22", "version": "1.0.0", "last_reviewed": "2026-05-23", "activation_date": "2026-05-25", "expected_return": "2026-06-08", "allowed_tasks": ["async-standup-read", "contractor-block-escalation", "customer-emergency-reply"], "auto_reply_text": "Limited availability until June 8; for emergencies contact @co-founder; daily check-in by 18:00 UTC.", "escalation_contact": "@co-founder", "owner": "@ruslan"}')
INVALID_FIXTURE = json.loads('{"activation_date": "2026-05-25", "expected_return": "", "allowed_tasks": [], "auto_reply_text": "", "escalation_contact": "", "owner": "team"}')
