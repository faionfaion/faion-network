#!/usr/bin/env python3
"""validate-linear-issue-tracking.py — validate a linear-issue-tracking output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'workspace_url', 'cycle_length_days', 'states', 'labels', 'integrations', 'estimation_method', 'owner']
ENUMS = {'cycle_length_days': [7, 14], 'estimation_method': ['points', 'tshirt', 'hours']}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = ['cycle_length_days']
NUM_FIELDS = []
ARRAY_FIELDS = ['states', 'labels']
OBJECT_FIELDS = ['integrations']
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'workspace_url', 'estimation_method', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "linear-setup-2026-q2", "version": "1.0.0", "last_reviewed": "2026-05-23", "workspace_url": "https://linear.app/faion", "cycle_length_days": 14, "states": ["Backlog", "Todo", "In Progress", "In Review", "Done", "Cancelled"], "labels": ["bug", "feat", "chore", "spike", "docs", "infra"], "integrations": {"github": true, "slack": true, "calendar": true}, "estimation_method": "points", "owner": "@ruslan"}')
INVALID_FIXTURE = json.loads('{"workspace_url": "https://linear.app/x", "cycle_length_days": 30, "states": ["a", "b", "c", "d", "e", "f", "g", "h"], "labels": ["x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x", "x"], "integrations": {}, "estimation_method": "vibes", "owner": "team"}')
