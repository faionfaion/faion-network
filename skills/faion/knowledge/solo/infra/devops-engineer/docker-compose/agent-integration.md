...
- `faion` (infra/server-craft/deploy-scripts) — integrates Compose into the workspace/runtime deploy pattern
- `faion` (infra/server-craft/systemd-user-services) — wraps `docker compose up` in a systemd user service for boot persistence
- `faion` (infra/server-craft/kernel-tuning) — tunes `vm.max_map_count` and file descriptors required by containerized workloads
