...
- `faion` (infra/server-craft/kernel-tuning) — `vm.swappiness`, `vm.overcommit_memory` are set via sysctl
- `faion` (infra/server-craft/systemd-user-services) — `MemoryMax`, `MemoryHigh`, `OOMScoreAdjust` in unit files
- `faion` (infra/server-craft/health-checks-autoheal) — memory pressure alerts feed into autoheal decisions
