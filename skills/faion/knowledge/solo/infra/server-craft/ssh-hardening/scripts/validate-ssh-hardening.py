#!/usr/bin/env python3
"""validate-ssh-hardening.py — validate a ssh-hardening output JSON against the methodology schema.
...
REQUIRED = ["artefact_id", "version", "last_reviewed", "host", "port", "allow_users", "password_auth", "owner"]
ENUMS = {}
SEMVER_FIELDS = ["version"]
DATE_FIELDS = ["last_reviewed"]
INT_FIELDS = ["port"]
ARRAY_FIELDS = ["allow_users"]
OBJECT_FIELDS = []
BOOL_FIELDS = ["password_auth", "permit_root_login", "second_terminal_verified"]
STRING_FIELDS = ["artefact_id", "host", "owner"]
