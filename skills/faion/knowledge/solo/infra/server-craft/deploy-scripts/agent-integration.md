...
- `faion` (infra/server-craft/git-server-workflow) — git checkout + redeploy as rollback mechanism
- `faion` (infra/server-craft/systemd-user-services) — the `systemctl --user restart` step and health check logic
- `faion` (infra/server-craft/health-checks-autoheal) — post-deploy validation patterns
