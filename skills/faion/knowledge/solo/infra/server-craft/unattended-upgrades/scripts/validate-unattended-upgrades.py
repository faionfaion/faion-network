#!/usr/bin/env python3
"""validate-unattended-upgrades.py — validate a unattended-upgrades output JSON against the methodology schema.
...
REQUIRED = ["artefact_id", "version", "last_reviewed", "allowed_origins", "auto_reboot", "reboot_time", "mail_to", "owner"]
ENUMS = {}
SEMVER_FIELDS = ["version"]
DATE_FIELDS = ["last_reviewed"]
INT_FIELDS = []
ARRAY_FIELDS = ["allowed_origins"]
OBJECT_FIELDS = []
BOOL_FIELDS = ["auto_reboot"]
STRING_FIELDS = ["artefact_id", "reboot_time", "mail_to", "owner"]
...
VALID_FIXTURE = json.loads("{\"artefact_id\": \"upgrades-faion-net-01\", \"version\": \"1.1.0\", \"last_reviewed\": \"2026-05-23\", \"allowed_origins\": [\"${distro_id}:${distro_codename}-security\", \"${distro_id}ESMApps:${distro_codename}-apps-security\"], \"auto_reboot\": true, \"reboot_time\": \"04:00\", \"mail_to\": \"ops@faion.net\", \"owner\": \"@ruslan\"}")
INVALID_FIXTURE = json.loads("{\"allowed_origins\": [\"${distro_id}:${distro_codename}-updates\"], \"auto_reboot\": true, \"reboot_time\": \"now\", \"owner\": \"team\"}")
