#!/usr/bin/env python3
"""validate-systemd-user-services.py — validate a systemd-user-services output JSON against the methodology schema.
...
REQUIRED = ["artefact_id", "version", "last_reviewed", "unit_name", "exec_start", "environment_file", "restart", "owner"]
ENUMS = {"restart": ["on-failure", "no", "on-abnormal"]}
SEMVER_FIELDS = ["version"]
DATE_FIELDS = ["last_reviewed"]
INT_FIELDS = []
ARRAY_FIELDS = []
OBJECT_FIELDS = []
BOOL_FIELDS = ["linger_enabled"]
STRING_FIELDS = ["artefact_id", "unit_name", "exec_start", "environment_file", "memory_high", "memory_max", "owner"]
