...
- `faion` (infra/server-craft/deploy-scripts) — deploy script invokes `systemctl --user restart` after rsync
- `faion` (infra/server-craft/swap-memory-management) — `MemoryMax` and `OOMScoreAdjust` directives
- `faion` (infra/server-craft/monitoring-logging) — `journalctl --user` log access and retention config
- `faion` (infra/server-craft/health-checks-autoheal) — post-start health check + autoheal integration
