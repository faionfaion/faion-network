#!/usr/bin/env python3
"""validate-solo-incident-triage-checklist.py — validate a solo-incident-triage-checklist output JSON against the methodology schema.
...
REQUIRED = ["artefact_id", "version", "last_reviewed", "incident_id", "detected_at", "owner", "impact", "containment_action", "comms_sent_at", "root_cause", "corrective_actions"]
ENUMS = {"containment_action": ["rollback", "feature-flag", "forward-fix", "none"]}
SEMVER_FIELDS = ["version"]
DATE_FIELDS = ["last_reviewed"]
INT_FIELDS = []
ARRAY_FIELDS = ["corrective_actions"]
OBJECT_FIELDS = []
BOOL_FIELDS = ["blameless"]
STRING_FIELDS = ["artefact_id", "incident_id", "detected_at", "owner", "impact", "comms_sent_at", "root_cause"]
...
VALID_FIXTURE = json.loads("{\"artefact_id\": \"incident-2026-05-23-01\", \"version\": \"1.1.0\", \"last_reviewed\": \"2026-05-23\", \"incident_id\": \"INC-2026-05-23-01\", \"detected_at\": \"2026-05-23T11:02Z\", \"owner\": \"@ruslan\", \"impact\": \"checkout flow 5xx ~12% requests\", \"containment_action\": \"rollback\", \"comms_sent_at\": \"2026-05-23T11:14Z\", \"root_cause\": \"Migration 0042 dropped index used by checkout query.\", \"corrective_actions\": [\"restore index\", \"add migration-impact CI check\"], \"blameless\": true}")
INVALID_FIXTURE = json.loads("{\"incident_id\": \"INC-x\", \"owner\": \"team\", \"containment_action\": \"forward-fix\", \"comms_sent_at\": \"\"}")
