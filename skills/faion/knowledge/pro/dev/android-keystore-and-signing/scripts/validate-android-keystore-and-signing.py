#!/usr/bin/env python3
"""validate-android-keystore-and-signing.py — validate android-keystore-and-signing output against the 02-output-contract schema.
...
REQUIRED = ['artefact_id', 'owner', 'config_kind', 'values', 'rules_checked', 'version', 'last_reviewed']
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        prog="validate-android-keystore-and-signing.py",
        description="Validate android-keystore-and-signing output against 02-output-contract schema. Exit 0 on pass, 1 on fail, 2 on usage error.",
    )
