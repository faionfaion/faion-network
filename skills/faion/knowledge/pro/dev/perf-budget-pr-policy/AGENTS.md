...
summary: PR-policy playbook step: budget regression blocks merge unless owner signs a waiver tied to a follow-up issue.
content_id: "3fdd84f69ce70bd1"
complexity: medium
produces: playbook-step
est_tokens: 4100
tags: [perf-budget, pr-policy, ci-gate, review]
---
...
**One-sentence:** PR-policy playbook step: budget regression blocks merge unless owner signs a waiver tied to a follow-up issue.

**One-paragraph:** PR-policy playbook step: budget regression blocks merge unless owner signs a waiver tied to a follow-up issue. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
| File | Purpose |
|------|---------|
| `templates/output.md` | Playbook-step skeleton matching the schema in 02-output-contract.xml |
...
|------|---------|--------------|
| `scripts/validate-perf-budget-pr-policy.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
