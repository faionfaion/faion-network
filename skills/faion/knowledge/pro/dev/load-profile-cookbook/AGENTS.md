...
summary: Documents steady / spike / soak / stress load profiles with k6 + locust starter templates.
content_id: "d05e6a953a342b80"
complexity: medium
produces: playbook-step
est_tokens: 4100
tags: [load-test, k6, locust, profiles, qa]
---
...
**One-sentence:** Documents steady / spike / soak / stress load profiles with k6 + locust starter templates.

**One-paragraph:** Documents steady / spike / soak / stress load profiles with k6 + locust starter templates. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
|------|---------|--------------|
| `scripts/validate-load-profile-cookbook.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
