#!/usr/bin/env python3
"""validate-arch-health-weekly-report-template.py — validate arch-health-weekly-report-template output against the 02-output-contract schema.
...
REQUIRED = ['artefact_id', 'owner', 'findings', 'recommendations', 'version', 'last_reviewed']
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        prog="validate-arch-health-weekly-report-template.py",
        description="Validate arch-health-weekly-report-template output against 02-output-contract schema. Exit 0 on pass, 1 on fail, 2 on usage error.",
    )
