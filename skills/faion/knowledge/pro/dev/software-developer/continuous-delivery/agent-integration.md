...
- `/faion` (sdd-batch-orchestrator workflow) skill — sequential mode for a feature whose deploy spans migration → app deploy → flag flip → bake → flag remove; out-of-order = outage.
