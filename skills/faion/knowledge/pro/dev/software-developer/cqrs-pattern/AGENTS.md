...
summary: Command Query Responsibility Segregation (CQRS) separates read and write operations into different models; commands return void/ID, queries never mutate.
...
**One-sentence:** Command Query Responsibility Segregation (CQRS) separates read and write operations into different models; commands return void/ID, queries never mutate.

**One-paragraph:** Command Query Responsibility Segregation (CQRS) separates read and write operations into different models; commands return void/ID, queries never mutate. This methodology pins the testable rules, output contract, and procedure that turn the abstract pattern into a reviewable artefact. Apply when the preconditions hold; otherwise the decision tree routes to `skip-this-methodology`. Output is the artefact described in `content/02-output-contract.xml`, validated by the bundled script.

**Ефективно для:**

- Team that needs a reusable, reviewable take on cqrs pattern for production code or operations.
...
| `draft-cqrs-pattern` | sonnet | Output drafting needs structure + light judgement. |
...
|------|---------|--------------|
| `scripts/validate-cqrs-pattern.py` | Validate output against the schema in `content/02-output-contract.xml` | CI on each artefact change; pre-commit; `--self-test` in unit run |
