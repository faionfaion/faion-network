#!/usr/bin/env python3
"""validate-event-sourcing-basics.py

Validate the output artefact for the event-sourcing-basics methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/event-sourcing-basics.json", "type": "object", "required": ["aggregate_name", "events", "applies_via_dispatch", "uses_optimistic_concurrency"], "properties": {"aggregate_name": {"type": "string", "minLength": 2}, "events": {"type": "array", "items": {"type": "string", "pattern": "^[A-Z][A-Za-z0-9]*(ed|en|d|t)$|^[A-Z][A-Za-z0-9]*(V[0-9]+)?$"}, "minItems": 1}, "applies_via_dispatch": {"const": true}, "uses_optimistic_concurrency": {"const": true}, "schema_evolution_plan_doc": {"type": "string"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"aggregate_name": "Order", "events": ["OrderPlaced", "ItemAdded", "OrderShipped"], "applies_via_dispatch": true, "uses_optimistic_concurrency": true, "schema_evolution_plan_doc": "docs/event-schema-evolution.md"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"aggregate_name": "x", "events": ["PlaceOrder"], "applies_via_dispatch": false, "uses_optimistic_concurrency": false}')
