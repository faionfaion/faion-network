#!/usr/bin/env python3
"""validate-cqrs-pattern.py

Validate the output artefact for the cqrs-pattern methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/cqrs-pattern.json", "type": "object", "required": ["module_root", "command_handlers", "query_handlers", "mediator_path", "no_mixed_handlers"], "properties": {"module_root": {"type": "string", "minLength": 1}, "command_handlers": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "query_handlers": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "mediator_path": {"type": "string", "minLength": 3}, "no_mixed_handlers": {"const": true}, "documented_projection_lag_ms_p95": {"type": "integer"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"module_root": "myapp", "command_handlers": ["myapp.commands.PlaceOrderHandler"], "query_handlers": ["myapp.queries.GetOrderHandler"], "mediator_path": "myapp.mediator.Mediator", "no_mixed_handlers": true, "documented_projection_lag_ms_p95": 250}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"module_root": "myapp", "command_handlers": [], "query_handlers": [], "mediator_path": "", "no_mixed_handlers": false}')
