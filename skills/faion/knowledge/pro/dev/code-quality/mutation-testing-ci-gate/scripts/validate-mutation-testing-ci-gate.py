#!/usr/bin/env python3
"""validate-mutation-testing-ci-gate.py

Validate the output artefact for the mutation-testing-ci-gate methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/mutation-testing-ci-gate.json", "type": "object", "required": ["tool", "config_path", "incremental_only", "shadow_mode_days", "module_floors", "exception_label"], "properties": {"tool": {"enum": ["stryker", "stryker-net", "mutmut", "pitest", "infection", "cosmic-ray"]}, "config_path": {"type": "string", "minLength": 3}, "incremental_only": {"const": true}, "shadow_mode_days": {"type": "integer", "minimum": 14, "maximum": 60}, "module_floors": {"type": "array", "items": {"type": "object", "required": ["module", "floor_pct"], "properties": {"module": {"type": "string"}, "floor_pct": {"type": "integer", "minimum": 20, "maximum": 90}}}, "minItems": 1}, "exception_label": {"type": "string"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"tool": "stryker", "config_path": "stryker.conf.json", "incremental_only": true, "shadow_mode_days": 21, "module_floors": [{"module": "src/billing", "floor_pct": 65}, {"module": "src/auth", "floor_pct": 70}], "exception_label": "mutation-exception"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"tool": "stryker", "config_path": "", "incremental_only": false, "shadow_mode_days": 1, "module_floors": []}')
