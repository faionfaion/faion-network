#!/usr/bin/env python3
"""validate-event-sourcing-implementation.py

Validate the output artefact for the event-sourcing-implementation methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/event-sourcing-implementation.json", "type": "object", "required": ["event_store_kind", "schema_path", "uses_expected_version_check", "append_only_at_grant_level", "snapshot_threshold"], "properties": {"event_store_kind": {"enum": ["postgresql", "eventstoredb", "kafka", "custom"]}, "schema_path": {"type": "string", "minLength": 3}, "uses_expected_version_check": {"const": true}, "append_only_at_grant_level": {"const": true}, "snapshot_threshold": {"type": "integer", "minimum": 10}, "projection_via_event_bus": {"type": "boolean"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"event_store_kind": "postgresql", "schema_path": "db/event-store-schema.sql", "uses_expected_version_check": true, "append_only_at_grant_level": true, "snapshot_threshold": 50, "projection_via_event_bus": true}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"event_store_kind": "custom", "schema_path": "", "uses_expected_version_check": false, "append_only_at_grant_level": false, "snapshot_threshold": 1}')
