...
maintainers: [faion]
content_id: "80a017ee6ad02b39"
summary: QA Metrics Slos — pinned method for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so modernize a 2018-era qa suite into ai-augmented test ops stops being folklore and starts being a reviewable operating tool.
tags: [dev, pro, method, metrics, slos]
---
# QA Metrics Slos

## Summary

**One-sentence:** QA Metrics Slos — pinned method for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so modernize a 2018-era qa suite into ai-augmented test ops stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs modernize a 2018-era qa suite into ai-augmented test ops on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Faion has zero coverage of QA-as-a-product metrics: escape rate, MTTD/MTTR for bugs, flake rate, mutation score, eval-coverage SLO for AI features, CI lead time. Without these, modernization regresses. Belongs alongside DevOps DORA but framed for QA. `qa-metrics-slos` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.
