...
summary: Pipeline spec for an Expo EAS Build + Submit + Update flow: channels, profiles, OTA cutoffs.
content_id: "3b3161a221b3e606"
complexity: medium
produces: spec
est_tokens: 5000
tags: [expo, eas, react-native, pipeline, release]
---
...
**One-sentence:** Pipeline spec for an Expo EAS Build + Submit + Update flow: channels, profiles, OTA cutoffs.

**One-paragraph:** Pipeline spec for an Expo EAS Build + Submit + Update flow: channels, profiles, OTA cutoffs. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
|------|---------|--------------|
| `scripts/validate-mobile-expo-eas-pipeline.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
