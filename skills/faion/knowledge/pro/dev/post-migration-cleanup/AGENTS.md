...
maintainers: [faion]
content_id: "37db5c41829d1ac6"
summary: Post Migration Cleanup — pinned method for the software developer: fixed shape + named owner + evidence anchors + outcome review, so survive a major framework upgrade without a big-bang branch stops being folklore and starts being a reviewable operating tool.
tags: [dev, pro, method, post, migration, cleanup]
---
# Post Migration Cleanup

## Summary

**One-sentence:** Post Migration Cleanup — pinned method for the software developer: fixed shape + named owner + evidence anchors + outcome review, so survive a major framework upgrade without a big-bang branch stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the software developer runs survive a major framework upgrade without a big-bang branch on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Removing the strangler scaffolding, killing feature flags, deleting dead dual-runtime code. Tech-debt methodology exists but doesn't address the specific shape of migration debt. `post-migration-cleanup` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the software developer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-software-developer/Survive a Major Framework Upgrade Without a Big-Bang Branch`
