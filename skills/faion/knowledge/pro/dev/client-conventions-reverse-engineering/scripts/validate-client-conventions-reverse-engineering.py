#!/usr/bin/env python3
"""validate-client-conventions-reverse-engineering.py

Validate the output artefact for the client-conventions-reverse-engineering methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/client-conventions-reverse-engineering.json", "type": "object", "required": ["artefact_id", "owner", "client_repo", "lint_tool", "lint_config_path", "naming", "branching_model", "version", "last_reviewed"], "properties": {"artefact_id": {"type": "string", "minLength": 3}, "owner": {"type": "string", "pattern": "^(?!team$|we$|us$|engineering$).+", "minLength": 2}, "client_repo": {"type": "string", "minLength": 5}, "lint_tool": {"type": "string", "minLength": 2}, "lint_config_path": {"type": "string", "minLength": 3}, "naming": {"type": "object", "required": ["files", "classes", "functions"]}, "branching_model": {"enum": ["trunk", "github-flow", "gitflow", "release-flow"]}, "commit_format": {"type": "string"}, "layering_rules": {"type": "array", "items": {"type": "string"}}, "version": {"type": "string", "pattern": "^\\\\d+\\\\.\\\\d+\\\\.\\\\d+$"}, "last_reviewed": {"type": "string", "pattern": "^\\\\d{4}-\\\\d{2}-\\\\d{2}$"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "rrev-2026-05-23", "owner": "@ruslan", "client_repo": "github.com/acme/web", "lint_tool": "eslint", "lint_config_path": ".eslintrc.json", "naming": {"files": "kebab-case", "classes": "PascalCase", "functions": "camelCase"}, "branching_model": "github-flow", "commit_format": "conventional", "layering_rules": ["no DB calls from controllers"], "version": "1.0.0", "last_reviewed": "2026-05-23"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "x", "owner": "team", "client_repo": "", "lint_tool": "", "lint_config_path": "", "naming": {}, "branching_model": "ad-hoc", "version": "v1", "last_reviewed": "soon"}')
