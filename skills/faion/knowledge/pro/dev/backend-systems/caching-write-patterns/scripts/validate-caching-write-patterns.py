#!/usr/bin/env python3
"""validate-caching-write-patterns.py — validate a Cache Write Patterns (Cache-Aside, Write-Through, Write-Behind) artefact JSON.

Usage:
  validate-caching-write-patterns.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^cwp\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "cwp-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/caching-write-patterns.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-caching-write-patterns.py <artefact.json>\n")
        return 2
    try:
