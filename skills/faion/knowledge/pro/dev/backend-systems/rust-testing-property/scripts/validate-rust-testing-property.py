#!/usr/bin/env python3
"""validate-rust-testing-property.py

Validate the output artefact for the rust-testing-property methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/rust-testing-property.json", "type": "object", "required": ["test_file", "invariant_name", "cases", "persists_seeds", "uses_prop_assert"], "properties": {"test_file": {"type": "string", "pattern": "\\\\.rs$"}, "invariant_name": {"type": "string", "minLength": 3}, "cases": {"type": "integer", "minimum": 16, "maximum": 100000}, "persists_seeds": {"const": true}, "uses_prop_assert": {"const": true}, "approx_for_floats": {"type": "boolean"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"test_file": "src/codec.rs", "invariant_name": "decode_encode_roundtrip", "cases": 256, "persists_seeds": true, "uses_prop_assert": true, "approx_for_floats": true}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"test_file": "src/codec.rs", "invariant_name": "x", "cases": 10, "persists_seeds": false, "uses_prop_assert": false, "approx_for_floats": false}')
