#!/usr/bin/env python3
"""validate-caching-invalidation.py — validate a Cache Invalidation (TTL, Event-Based, Tag-Based, Version-Based) artefact JSON.

Usage:
  validate-caching-invalidation.py <artefact.json>
...
ID_RE = re.compile(r"^cinv\-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^cinv\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "cinv-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/caching-invalidation.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-caching-invalidation.py <artefact.json>\n")
        return 2
    try:
