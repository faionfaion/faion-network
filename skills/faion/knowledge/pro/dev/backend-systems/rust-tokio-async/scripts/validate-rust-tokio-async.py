#!/usr/bin/env python3
"""validate-rust-tokio-async.py

Validate the output artefact for the rust-tokio-async methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/rust-tokio-async.json", "type": "object", "required": ["pattern", "concurrency_bound", "all_io_timed_out", "no_lock_across_await"], "properties": {"pattern": {"enum": ["try_join", "spawn_blocking", "buffer_unordered_semaphore", "join_set", "select"]}, "concurrency_bound": {"type": "integer", "minimum": 1}, "all_io_timed_out": {"const": true}, "no_lock_across_await": {"const": true}, "uses_join_set_for_dynamic": {"type": "boolean"}, "select_branches_documented": {"type": "boolean"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"pattern": "buffer_unordered_semaphore", "concurrency_bound": 16, "all_io_timed_out": true, "no_lock_across_await": true, "uses_join_set_for_dynamic": true, "select_branches_documented": true}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"pattern": "try_join", "concurrency_bound": 10000, "all_io_timed_out": false, "no_lock_across_await": false, "uses_join_set_for_dynamic": false, "select_branches_documented": false}')
