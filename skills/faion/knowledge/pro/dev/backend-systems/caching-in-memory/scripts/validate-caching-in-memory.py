#!/usr/bin/env python3
"""validate-caching-in-memory.py — validate a In-Memory Application Cache (L1 with lru_cache + TTLCache + WarmableCache) artefact JSON.

Usage:
  validate-caching-in-memory.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^l1c\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "l1c-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/caching-in-memory.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-caching-in-memory.py <artefact.json>\n")
        return 2
    try:
