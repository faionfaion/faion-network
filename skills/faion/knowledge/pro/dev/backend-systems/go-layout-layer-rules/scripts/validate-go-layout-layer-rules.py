#!/usr/bin/env python3
"""validate-go-layout-layer-rules.py — validate a Go Standard Layout — Layer Dependency Rules artefact JSON.

Usage:
  validate-go-layout-layer-rules.py <artefact.json>
...
ID_RE = re.compile(r"^goll\-[a-z0-9-]+$")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "goll-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/go-layout-layer-rules.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-go-layout-layer-rules.py <artefact.json>\n")
        return 2
    try:
