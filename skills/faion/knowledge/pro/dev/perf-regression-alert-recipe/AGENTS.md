...
summary: Recipe for a perf-regression alert: baseline diff, % delta thresholds, paging policy, owner rotation.
content_id: "2000a17f1b785c8d"
complexity: medium
produces: config
est_tokens: 4100
tags: [perf, regression, alert, recipe, ci]
---
...
**One-sentence:** Recipe for a perf-regression alert: baseline diff, % delta thresholds, paging policy, owner rotation.

**One-paragraph:** Recipe for a perf-regression alert: baseline diff, % delta thresholds, paging policy, owner rotation. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
|------|---------|--------------|
| `scripts/validate-perf-regression-alert-recipe.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
