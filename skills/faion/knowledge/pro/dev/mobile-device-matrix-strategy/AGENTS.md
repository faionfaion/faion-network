...
summary: Defines a mobile device + OS test matrix gated by audience share, crash-pareto, and store policy minimums.
content_id: "5eab7c37eeef6a21"
complexity: medium
produces: spec
est_tokens: 5000
tags: [mobile, device-matrix, testing, qa, coverage]
---
...
**One-sentence:** Defines a mobile device + OS test matrix gated by audience share, crash-pareto, and store policy minimums.

**One-paragraph:** Defines a mobile device + OS test matrix gated by audience share, crash-pareto, and store policy minimums. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
|------|---------|--------------|
| `scripts/validate-mobile-device-matrix-strategy.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
