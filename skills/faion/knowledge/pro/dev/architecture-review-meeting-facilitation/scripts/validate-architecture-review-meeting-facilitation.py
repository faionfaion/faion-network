#!/usr/bin/env python3
"""validate-architecture-review-meeting-facilitation.py — validate architecture-review-meeting-facilitation output against the 02-output-contract schema.
...
REQUIRED = ['artefact_id', 'owner', 'step_name', 'inputs', 'actions', 'outputs', 'exit_criteria', 'version', 'last_reviewed']
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        prog="validate-architecture-review-meeting-facilitation.py",
        description="Validate architecture-review-meeting-facilitation output against 02-output-contract schema. Exit 0 on pass, 1 on fail, 2 on usage error.",
    )
