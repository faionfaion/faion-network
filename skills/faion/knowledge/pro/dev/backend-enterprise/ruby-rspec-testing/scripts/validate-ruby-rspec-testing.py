#!/usr/bin/env python3
"""validate-ruby-rspec-testing.py — validate a RSpec Testing for Rails Applications artefact JSON.

Usage:
  validate-ruby-rspec-testing.py <artefact.json>
...
ID_RE = re.compile(r"^rspec\-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^rspec\-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "rspec-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/ruby-rspec-testing.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-ruby-rspec-testing.py <artefact.json>\n")
        return 2
    try:
