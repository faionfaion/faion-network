#!/usr/bin/env python3
"""validate-cap-pacelc-walkthrough.py

Validate the output artefact for the cap-pacelc-walkthrough methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/cap-pacelc-walkthrough.json", "type": "object", "required": ["artefact_id", "owner", "decision", "rationale", "inputs_used", "version", "last_reviewed", "cap_mode", "pacelc_mode"], "properties": {"artefact_id": {"type": "string", "minLength": 3}, "owner": {"type": "string", "pattern": "^(?!team$|we$|us$|engineering$).+", "minLength": 2}, "decision": {"type": "string", "minLength": 10}, "rationale": {"type": "string", "minLength": 40}, "inputs_used": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "version": {"type": "string", "pattern": "^\\\\d+\\\\.\\\\d+\\\\.\\\\d+$"}, "last_reviewed": {"type": "string", "pattern": "^\\\\d{4}-\\\\d{2}-\\\\d{2}$"}, "cap_mode": {"enum": ["AP", "CP"]}, "pacelc_mode": {"enum": ["EL", "EC"]}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "cap-pacelc-orders-2026-05-23", "owner": "@ruslan", "decision": "orders-service: AP under partition (DynamoDB), EL when healthy", "rationale": "RPO=30s and revenue impact of read unavailability dominates; PACELC choice EL because checkout p95 budget is 80ms; both inputs cited inline", "inputs_used": ["policy/rpo-rto.md", "observability/orders-read-write-split.json"], "version": "1.0.0", "last_reviewed": "2026-05-23", "cap_mode": "AP", "pacelc_mode": "EL"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "x", "owner": "team", "decision": "tbd", "rationale": "obviously", "inputs_used": [], "version": "v1", "last_reviewed": "soon", "cap_mode": "AC", "pacelc_mode": "EE"}')
