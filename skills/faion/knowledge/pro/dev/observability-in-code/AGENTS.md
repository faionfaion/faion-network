...
summary: Spec for in-code observability: structured logging, OTel span boundaries, sampling, redaction.
content_id: "3ccbd59c6935bcf4"
complexity: medium
produces: spec
est_tokens: 5000
tags: [observability, code, logging, tracing, spans]
---
...
**One-sentence:** Spec for in-code observability: structured logging, OTel span boundaries, sampling, redaction.

**One-paragraph:** Spec for in-code observability: structured logging, OTel span boundaries, sampling, redaction. Mechanism: typed input → bounded transformation → contract-checked output. The artefact carries owner + version + last_reviewed so downstream consumers can verify freshness without re-deriving the rationale.
...
|------|---------|--------------|
| `scripts/validate-observability-in-code.py` | Validate output against 02-output-contract JSON Schema; exit 0 on pass, 1 on fail with violation list | After subagent returns, before downstream consumer reads; pre-commit |
