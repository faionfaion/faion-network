...
maintainers: [faion]
content_id: "80788faff23c3f37"
summary: QA Security Triage Runbook — pinned runbook for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so security scan triage (weekly) stops being folklore and starts being a reviewable operating tool.
tags: [dev, pro, runbook, security, triage]
---
# QA Security Triage Runbook

## Summary

**One-sentence:** QA Security Triage Runbook — pinned runbook for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so security scan triage (weekly) stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In software development, the QA engineer runs security scan triage (weekly) on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Faion has scanning methodologies (SAST/DAST/container/supply-chain) but no triage-and-prioritisation playbook from the QA side. Without it, the scan output becomes noise. `qa-security-triage-runbook` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored runbook committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Security scan triage (weekly)`
