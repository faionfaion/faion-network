#!/usr/bin/env python3
"""validate-codemod-recipe-library.py

Validate the output artefact for the codemod-recipe-library methodology against the embedded
...
SCHEMA: dict[str, Any] = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/codemod-recipe-library.json", "type": "object", "required": ["artefact_id", "owner", "tool", "target_framework", "recipes", "version", "last_reviewed"], "properties": {"artefact_id": {"type": "string", "minLength": 3}, "owner": {"type": "string", "pattern": "^(?!team$|we$|us$|engineering$).+", "minLength": 2}, "tool": {"enum": ["jscodeshift", "ts-morph", "ast-grep", "libcst", "Bowler", "comby"]}, "target_framework": {"type": "string", "minLength": 3}, "recipes": {"type": "array", "items": {"type": "object", "required": ["name", "fixture_before", "fixture_after"], "properties": {"name": {"type": "string", "pattern": "^[a-z][a-z0-9-]+$"}, "fixture_before": {"type": "string"}, "fixture_after": {"type": "string"}}}, "minItems": 1}, "version": {"type": "string", "pattern": "^\\\\d+\\\\.\\\\d+\\\\.\\\\d+$"}, "last_reviewed": {"type": "string", "pattern": "^\\\\d{4}-\\\\d{2}-\\\\d{2}$"}}}')
VALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "codemod-react-18-2026-05-23", "owner": "@ruslan", "tool": "jscodeshift", "target_framework": "react@18", "recipes": [{"name": "render-to-create-root", "fixture_before": "fixtures/render.before.js", "fixture_after": "fixtures/render.after.js"}], "version": "1.0.0", "last_reviewed": "2026-05-23"}')
INVALID_FIXTURE: dict[str, Any] = json.loads('{"artefact_id": "x", "owner": "team", "tool": "regex", "target_framework": "", "recipes": [], "version": "v1", "last_reviewed": "soon"}')
