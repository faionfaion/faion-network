#!/usr/bin/env python3
"""validate-devops-aws-three-tier.py

Validate the config artefact for the devops-aws-three-tier methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/devops-aws-three-tier.json", "type": "object", "required": ["project", "az_count", "tiers", "nat_gateway_per_az_prod", "sg_reference_by_id", "aurora_serverless_v2", "encryption_at_rest"], "properties": {"project": {"type": "string", "minLength": 1}, "az_count": {"type": "integer", "minimum": 2, "maximum": 6}, "tiers": {"type": "object", "required": ["public", "private", "database"], "properties": {"public": {"type": "array", "minItems": 1}, "private": {"type": "array", "minItems": 1}, "database": {"type": "array", "minItems": 1}}}, "nat_gateway_per_az_prod": {"type": "boolean", "const": true}, "sg_reference_by_id": {"type": "boolean", "const": true}, "aurora_serverless_v2": {"type": "boolean"}, "encryption_at_rest": {"type": "boolean", "const": true}}}')
REQUIRED = ['project', 'az_count', 'tiers', 'nat_gateway_per_az_prod', 'sg_reference_by_id', 'aurora_serverless_v2', 'encryption_at_rest']
...
OK = json.loads('{"project": "checkout-app", "az_count": 2, "tiers": {"public": ["alb"], "private": ["app"], "database": ["aurora"]}, "nat_gateway_per_az_prod": true, "sg_reference_by_id": true, "aurora_serverless_v2": true, "encryption_at_rest": true}')
BAD = json.loads('{"project": "checkout-app", "az_count": 1, "tiers": {"public": ["alb"], "private": ["app"], "database": []}, "nat_gateway_per_az_prod": false, "sg_reference_by_id": false, "aurora_serverless_v2": false, "encryption_at_rest": false}')
