#!/usr/bin/env python3
"""validate-devops-elk-index-management.py

Validate the config artefact for the devops-elk-index-management methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/devops-elk-index-management.json", "type": "object", "required": ["index_pattern", "policy_name", "phases", "rollover", "shard_count", "write_alias"], "properties": {"index_pattern": {"type": "string", "minLength": 1}, "policy_name": {"type": "string", "minLength": 1}, "phases": {"type": "array", "items": {"enum": ["hot", "warm", "cold", "frozen", "delete"]}, "contains": {"const": "hot"}, "minItems": 2}, "rollover": {"type": "object", "required": ["max_primary_shard_size", "max_age_days"], "properties": {"max_primary_shard_size": {"type": "string"}, "max_age_days": {"type": "integer", "minimum": 1}}}, "shard_count": {"type": "integer", "minimum": 1, "maximum": 3}, "replica_count": {"type": "integer", "minimum": 0}, "write_alias": {"type": "string", "minLength": 1}}}')
REQUIRED = ['index_pattern', 'policy_name', 'phases', 'rollover', 'shard_count', 'write_alias']
...
OK = json.loads('{"index_pattern": "logs-*", "policy_name": "logs-policy", "phases": ["hot", "warm", "cold", "delete"], "rollover": {"max_primary_shard_size": "50gb", "max_age_days": 7}, "shard_count": 1, "replica_count": 1, "write_alias": "logs-write"}')
BAD = json.loads('{"index_pattern": "logs-*", "policy_name": "logs-policy", "phases": ["hot"], "rollover": {"max_primary_shard_size": "", "max_age_days": 0}, "shard_count": 5, "write_alias": ""}')
