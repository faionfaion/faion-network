#!/usr/bin/env python3
"""validate-devops-lb-kubernetes.py

Validate the artefact produced by the `devops-lb-kubernetes` methodology against the
...
REQUIRED = ['services', 'ingress_controller', 'tls_strategy', 'probes_present', 'owner', 'last_reviewed']
...
OK = json.loads('{"services": [{"name": "api", "type": "ClusterIP", "ports": [{"name": "http", "port": 80, "targetPort": 8080}]}], "ingress_controller": "nginx", "tls_strategy": "cert_manager", "probes_present": {"liveness": true, "readiness": true}, "owner": "sre@acme.io", "last_reviewed": "2026-05-23"}')
BAD = json.loads('{"services": [], "owner": "team"}')
