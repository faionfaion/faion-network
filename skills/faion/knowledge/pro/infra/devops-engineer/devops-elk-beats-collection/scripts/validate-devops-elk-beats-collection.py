#!/usr/bin/env python3
"""validate-devops-elk-beats-collection.py

Validate the config artefact for the devops-elk-beats-collection methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/devops-elk-beats-collection.json", "type": "object", "required": ["deployment", "inputs", "output"], "properties": {"deployment": {"enum": ["daemonset", "host-agent", "sidecar"]}, "inputs": {"type": "array", "minItems": 1, "items": {"type": "object", "required": ["type", "fields_under_root"], "properties": {"type": {"type": "string"}, "fields_under_root": {"type": "boolean", "const": true}, "fields": {"type": "object", "required": ["service", "environment"]}, "multiline_pattern": {"type": "string"}}}}, "autodiscover_k8s": {"type": "boolean"}, "output": {"type": "object", "required": ["target", "tls"], "properties": {"target": {"enum": ["elasticsearch", "logstash"]}, "tls": {"type": "boolean", "const": true}, "bulk_max_size": {"type": "integer", "minimum": 100}}}}}')
REQUIRED = ['deployment', 'inputs', 'output']
...
OK = json.loads('{"deployment": "daemonset", "inputs": [{"type": "log", "fields_under_root": true, "fields": {"service": "checkout-api", "environment": "prod"}, "multiline_pattern": "^[[:space:]]"}], "autodiscover_k8s": true, "output": {"target": "elasticsearch", "tls": true, "bulk_max_size": 1024}}')
BAD = json.loads('{"deployment": "sidecar", "inputs": [{"type": "log", "fields_under_root": false, "fields": {}}], "output": {"target": "elasticsearch", "tls": false}}')
