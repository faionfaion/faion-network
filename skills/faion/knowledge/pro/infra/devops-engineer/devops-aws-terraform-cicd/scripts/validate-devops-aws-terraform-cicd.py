#!/usr/bin/env python3
"""validate-devops-aws-terraform-cicd.py

Validate the config artefact for the devops-aws-terraform-cicd methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/devops-aws-terraform-cicd.json", "type": "object", "required": ["layout", "envs", "backend", "auth", "default_tags", "sequential_apply", "prod_approval"], "properties": {"layout": {"const": "per-env-dirs"}, "envs": {"type": "array", "minItems": 2, "items": {"type": "string"}}, "backend": {"type": "object", "required": ["type", "lock_table", "encrypt"], "properties": {"type": {"const": "s3"}, "lock_table": {"type": "string", "minLength": 1}, "encrypt": {"type": "boolean", "const": true}}}, "auth": {"const": "oidc"}, "default_tags": {"type": "array", "items": {"type": "string"}, "contains": {"const": "Project"}, "minItems": 3}, "sequential_apply": {"type": "boolean", "const": true}, "prod_approval": {"type": "boolean", "const": true}}}')
REQUIRED = ['layout', 'envs', 'backend', 'auth', 'default_tags', 'sequential_apply', 'prod_approval']
...
OK = json.loads('{"layout": "per-env-dirs", "envs": ["dev", "staging", "prod"], "backend": {"type": "s3", "lock_table": "acme-tfstate-lock", "encrypt": true}, "auth": "oidc", "default_tags": ["Project", "Environment", "ManagedBy"], "sequential_apply": true, "prod_approval": true}')
BAD = json.loads('{"layout": "single-dir", "envs": ["prod"], "backend": {"type": "local", "lock_table": "", "encrypt": false}, "auth": "static-keys", "default_tags": [], "sequential_apply": false, "prod_approval": false}')
