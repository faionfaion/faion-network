#!/usr/bin/env python3
"""validate-devops-aws-serverless-api.py

Validate the config artefact for the devops-aws-serverless-api methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/devops-aws-serverless-api.json", "type": "object", "required": ["project", "lambda", "api_gateway", "dynamodb"], "properties": {"project": {"type": "string", "minLength": 1}, "lambda": {"type": "object", "required": ["architecture", "runtime", "tracing_mode"], "properties": {"architecture": {"const": "arm64"}, "runtime": {"type": "string"}, "tracing_mode": {"const": "Active"}, "timeout_s": {"type": "integer", "maximum": 900}}}, "api_gateway": {"type": "object", "required": ["protocol_type", "cors_origins", "throttling_burst", "throttling_rate", "custom_domain"], "properties": {"protocol_type": {"const": "HTTP"}, "cors_origins": {"type": "array", "items": {"type": "string", "not": {"const": "*"}}, "minItems": 1}, "throttling_burst": {"type": "integer", "minimum": 1}, "throttling_rate": {"type": "integer", "minimum": 1}, "custom_domain": {"type": "string", "minLength": 1}}}, "dynamodb": {"type": "object", "required": ["billing_mode"], "properties": {"billing_mode": {"enum": ["PAY_PER_REQUEST", "PROVISIONED"]}, "pitr": {"type": "boolean"}}}, "dlq": {"type": "boolean"}, "step_functions": {"type": "boolean"}}}')
REQUIRED = ['project', 'lambda', 'api_gateway', 'dynamodb']
...
OK = json.loads('{"project": "checkout-api", "lambda": {"architecture": "arm64", "runtime": "python3.12", "tracing_mode": "Active", "timeout_s": 30}, "api_gateway": {"protocol_type": "HTTP", "cors_origins": ["https://app.example.com"], "throttling_burst": 5000, "throttling_rate": 1000, "custom_domain": "api.example.com"}, "dynamodb": {"billing_mode": "PAY_PER_REQUEST", "pitr": true}, "dlq": true}')
BAD = json.loads('{"project": "checkout-api", "lambda": {"architecture": "x86_64", "runtime": "python3.10", "tracing_mode": "PassThrough"}, "api_gateway": {"protocol_type": "REST", "cors_origins": ["*"], "throttling_burst": 100, "throttling_rate": 50, "custom_domain": ""}, "dynamodb": {"billing_mode": "PROVISIONED"}}')
