#!/usr/bin/env python3
"""validate-devops-elk-architecture.py

Validate the spec artefact for the devops-elk-architecture methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/devops-elk-architecture.json", "type": "object", "required": ["cluster", "variant", "volume_gb_per_day", "retention_days", "node_roles", "tiers", "deployment"], "properties": {"cluster": {"type": "string", "minLength": 1}, "variant": {"enum": ["elk", "efk", "elastic-stack"]}, "volume_gb_per_day": {"type": "number", "minimum": 0}, "retention_days": {"type": "integer", "minimum": 1}, "node_roles": {"type": "object", "required": ["master", "data_hot"], "properties": {"master": {"type": "integer", "minimum": 3}, "data_hot": {"type": "integer", "minimum": 2}, "data_warm": {"type": "integer", "minimum": 0}, "coordinating": {"type": "integer", "minimum": 0}}}, "tiers": {"type": "object", "required": ["hot_days", "warm_days", "cold_days"]}, "deployment": {"enum": ["eck", "elastic-cloud", "opensearch-service", "docker-compose"]}}}')
REQUIRED = ['cluster', 'variant', 'volume_gb_per_day', 'retention_days', 'node_roles', 'tiers', 'deployment']
...
OK = json.loads('{"cluster": "prod-logs", "variant": "elk", "volume_gb_per_day": 200, "retention_days": 90, "node_roles": {"master": 3, "data_hot": 3, "data_warm": 2, "coordinating": 2}, "tiers": {"hot_days": 7, "warm_days": 30, "cold_days": 53}, "deployment": "eck"}')
BAD = json.loads('{"cluster": "prod-logs", "variant": "elk", "volume_gb_per_day": 200, "retention_days": 90, "node_roles": {"master": 2, "data_hot": 1}, "tiers": {"hot_days": 90, "warm_days": 0, "cold_days": 0}, "deployment": "docker-compose"}')
