#!/usr/bin/env python3
"""validate-k8s-canary-progressive.py

Validate the output artefact for the `k8s-canary-progressive` methodology against the JSON
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/k8s-canary-progressive.json', 'type': 'object', 'required': ['service', 'engine', 'strategy', 'steps', 'analysis', 'rollback_seconds', 'owner', 'produced_at'], 'properties': {'service': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'}, 'engine': {'enum': ['argo-rollouts', 'flagger']}, 'strategy': {'enum': ['canary', 'blue-green']}, 'steps': {'type': 'array', 'items': {'type': 'object', 'required': ['weight', 'pause_seconds'], 'properties': {'weight': {'type': 'integer', 'minimum': 1, 'maximum': 100}, 'pause_seconds': {'type': 'integer', 'minimum': 30}}}, 'minItems': 3}, 'analysis': {'type': 'object', 'required': ['template', 'success_condition', 'failure_condition'], 'properties': {'template': {'type': 'string'}, 'success_condition': {'type': 'string'}, 'failure_condition': {'type': 'string'}}}, 'rollback_seconds': {'type': 'integer', 'maximum': 300}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string'}, 'role': {'type': 'string'}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}}, 'additionalProperties': True}
...
OK = {'service': 'acme-api', 'engine': 'argo-rollouts', 'strategy': 'canary', 'steps': [{'weight': 5, 'pause_seconds': 300}, {'weight': 25, 'pause_seconds': 600}, {'weight': 50, 'pause_seconds': 600}, {'weight': 100, 'pause_seconds': 60}], 'analysis': {'template': 'http-slo-v1', 'success_condition': 'result[0] < 0.01', 'failure_condition': 'result[0] >= 0.02'}, 'rollback_seconds': 180, 'owner': {'name': 'Olena Boyko', 'role': 'platform lead'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'service': 'acme-api', 'strategy': 'canary', 'steps': [{'weight': 100, 'pause_seconds': 0}]}
