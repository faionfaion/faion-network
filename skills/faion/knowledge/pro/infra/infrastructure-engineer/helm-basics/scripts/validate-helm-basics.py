#!/usr/bin/env python3
"""validate-helm-basics.py

Validate the output artefact for the `helm-basics` methodology against the JSON
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/helm-basics.json', 'type': 'object', 'required': ['chart_name', 'chart_version', 'api_version', 'templates', 'values_files', 'owner', 'produced_at'], 'properties': {'chart_name': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'}, 'chart_version': {'type': 'string', 'pattern': '^\\d+\\.\\d+\\.\\d+(-[a-z0-9]+)?$'}, 'api_version': {'const': 'v2'}, 'templates': {'type': 'array', 'items': {'type': 'string'}, 'minItems': 1}, 'values_files': {'type': 'array', 'items': {'type': 'string'}, 'minItems': 1}, 'dependencies': {'type': 'array', 'items': {'type': 'object', 'required': ['name', 'version', 'repository'], 'properties': {'name': {'type': 'string'}, 'version': {'type': 'string'}, 'repository': {'type': 'string'}}}}, 'lockfile_committed': {'type': 'boolean'}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string'}, 'role': {'type': 'string'}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}}, 'additionalProperties': True}
...
OK = {'chart_name': 'acme-app', 'chart_version': '0.3.1', 'api_version': 'v2', 'templates': ['deployment.yaml', 'service.yaml', 'ingress.yaml', 'configmap.yaml'], 'values_files': ['values.yaml', 'values.dev.yaml', 'values.staging.yaml', 'values.prod.yaml'], 'dependencies': [{'name': 'postgresql', 'version': '15.5.0', 'repository': 'oci://registry-1.docker.io/bitnamicharts'}], 'lockfile_committed': True, 'owner': {'name': 'Olena Boyko', 'role': 'platform lead'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'chart_name': 'App', 'chart_version': 'v1'}
