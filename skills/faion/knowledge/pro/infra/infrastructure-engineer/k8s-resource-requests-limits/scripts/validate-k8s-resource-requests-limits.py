#!/usr/bin/env python3
"""validate-k8s-resource-requests-limits.py

Validate the output artefact for the `k8s-resource-requests-limits` methodology against the JSON
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/k8s-resource-requests-limits.json', 'type': 'object', 'required': ['service', 'container', 'qos_target', 'requests', 'limits', 'observed', 'owner', 'produced_at'], 'properties': {'service': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'}, 'container': {'type': 'string'}, 'qos_target': {'enum': ['Guaranteed', 'Burstable']}, 'requests': {'type': 'object', 'required': ['cpu', 'memory'], 'properties': {'cpu': {'type': 'string'}, 'memory': {'type': 'string'}}}, 'limits': {'type': 'object', 'required': ['memory'], 'properties': {'cpu': {'type': 'string'}, 'memory': {'type': 'string'}}}, 'observed': {'type': 'object', 'required': ['cpu_p95', 'memory_p99'], 'properties': {'cpu_p95': {'type': 'string'}, 'memory_p99': {'type': 'string'}}}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string'}, 'role': {'type': 'string'}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}}, 'additionalProperties': True}
...
OK = {'service': 'acme-api', 'container': 'api', 'qos_target': 'Burstable', 'requests': {'cpu': '250m', 'memory': '256Mi'}, 'limits': {'memory': '256Mi'}, 'observed': {'cpu_p95': '200m', 'memory_p99': '240Mi'}, 'owner': {'name': 'Tetiana Lytvyn', 'role': 'platform engineer'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'service': 'api', 'requests': {'cpu': '0', 'memory': '0'}}
