#!/usr/bin/env python3
"""validate-iac-patterns-testing.py

Validate the output artefact for the `iac-patterns-testing` methodology against the JSON
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/iac-patterns-testing.json', 'type': 'object', 'required': ['module_name', 'test_layers', 'policy_gates', 'owner', 'produced_at'], 'properties': {'module_name': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'}, 'test_layers': {'type': 'object', 'required': ['unit', 'component', 'integration'], 'properties': {'unit': {'type': 'array', 'items': {'type': 'string'}, 'minItems': 1}, 'component': {'type': 'array', 'items': {'type': 'string'}}, 'integration': {'type': 'array', 'items': {'type': 'string'}}}}, 'policy_gates': {'type': 'array', 'items': {'type': 'object', 'required': ['engine', 'policies'], 'properties': {'engine': {'enum': ['opa', 'sentinel', 'checkov', 'terrascan']}, 'policies': {'type': 'array', 'items': {'type': 'string'}, 'minItems': 1}}}, 'minItems': 1}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string'}, 'role': {'type': 'string'}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}}, 'additionalProperties': True}
...
OK = {'module_name': 'vpc-network', 'test_layers': {'unit': ['terraform fmt -check', 'terraform validate', 'tflint'], 'component': ['terraform plan -out plan.bin', 'json-assert: resource_count==12'], 'integration': ['terratest examples/simple', 'terratest examples/multi-region']}, 'policy_gates': [{'engine': 'opa', 'policies': ['no-public-buckets', 'cmek-required', 'no-primitive-roles']}], 'owner': {'name': 'Olha Petrenko', 'role': 'platform lead'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'module_name': 'vpc', 'test_layers': {}}
