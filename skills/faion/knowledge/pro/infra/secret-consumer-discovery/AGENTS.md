...
summary: Secret consumer discovery: enumerates every consumer (repo, service, CI job, scheduler, runtime config) of a given secret before rotation so no callsite is missed and no rollback is improvised.
content_id: "30ae82a782f998b9"
complexity: medium
produces: report
est_tokens: 4600
tags: [infra, pro, secrets, discovery, rotation]
---
# Secret Consumer Discovery

## Summary

**One-sentence:** Secret consumer discovery: enumerates every consumer (repo, service, CI job, scheduler, runtime config) of a given secret before rotation so no callsite is missed and no rollback is improvised.

**One-paragraph:** Secret Consumer Discovery pins the discipline that turns consumer discovery from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in 02-output-contract drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`secret-consumer-discovery` scope).
...
- Команд, де consumer discovery жив досі у головах SRE / DevOps, а не в репозиторії.
...
| File | Purpose |
|------|---------|
| `templates/secret-consumer-discovery.md` | Working skeleton for the `secret-consumer-discovery` artefact with required fields and `not_applicable: <reason>` markers per row. |
...
|------|---------|--------------|
| `scripts/validate-secret-consumer-discovery.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
