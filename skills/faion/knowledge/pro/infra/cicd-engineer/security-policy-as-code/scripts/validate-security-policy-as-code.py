#!/usr/bin/env python3
"""validate-security-policy-as-code.py

Validate the config artefact for the security-policy-as-code methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/security-policy-as-code.json", "type": "object", "required": ["engine", "engine_version", "policies", "rollout_phase", "unit_tests_present", "delivery_via_gitops"], "properties": {"engine": {"enum": ["kyverno", "opa-gatekeeper", "polaris"]}, "engine_version": {"type": "string", "minLength": 1}, "policies": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "rollout_phase": {"enum": ["audit", "enforce"]}, "unit_tests_present": {"type": "boolean", "const": true}, "delivery_via_gitops": {"type": "boolean", "const": true}, "trusted_registries": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "verify_images": {"type": "boolean"}}}')
REQUIRED = ['engine', 'engine_version', 'policies', 'rollout_phase', 'unit_tests_present', 'delivery_via_gitops']
...
OK = json.loads('{"engine": "kyverno", "engine_version": "1.12.0", "policies": ["baseline-security", "verify-image-signatures"], "rollout_phase": "audit", "unit_tests_present": true, "delivery_via_gitops": true, "trusted_registries": ["ghcr.io/acme/*", "quay.io/acme/*"], "verify_images": true}')
BAD = json.loads('{"engine": "kyverno", "engine_version": "1.12.0", "policies": ["baseline-security"], "rollout_phase": "enforce", "unit_tests_present": false, "delivery_via_gitops": false, "trusted_registries": [], "verify_images": false}')
