#!/usr/bin/env python3
"""validate-nginx-configuration.py

Validate the config artefact for the nginx-configuration methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/nginx-configuration.json", "type": "object", "required": ["domain", "upstream", "tls", "headers", "http2"], "properties": {"domain": {"type": "string", "minLength": 3}, "upstream": {"type": "string", "minLength": 1}, "tls": {"type": "object", "required": ["protocols", "hsts_max_age"], "properties": {"protocols": {"type": "array", "items": {"enum": ["TLSv1.2", "TLSv1.3"]}, "minItems": 1}, "hsts_max_age": {"type": "integer", "minimum": 15768000}, "ocsp_stapling": {"type": "boolean"}}}, "headers": {"type": "array", "items": {"type": "string"}, "minItems": 4}, "rate_limit_zones": {"type": "array", "items": {"type": "string"}}, "http2": {"type": "boolean", "const": true}, "websocket_upgrade": {"type": "boolean"}}}')
REQUIRED = ['domain', 'upstream', 'tls', 'headers', 'http2']
...
OK = json.loads('{"domain": "example.com", "upstream": "backend:8000", "tls": {"protocols": ["TLSv1.2", "TLSv1.3"], "hsts_max_age": 63072000, "ocsp_stapling": true}, "headers": ["Strict-Transport-Security", "X-Content-Type-Options", "X-Frame-Options", "Content-Security-Policy"], "rate_limit_zones": ["auth=5r/s"], "http2": true, "websocket_upgrade": true}')
BAD = json.loads('{"domain": "example.com", "upstream": "backend:8000", "tls": {"protocols": ["TLSv1.0", "TLSv1.1", "TLSv1.2"], "hsts_max_age": 3600}, "headers": ["X-Frame-Options"], "http2": false}')
