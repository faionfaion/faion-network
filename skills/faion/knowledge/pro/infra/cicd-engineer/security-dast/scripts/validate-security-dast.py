#!/usr/bin/env python3
"""validate-security-dast.py

Validate the config artefact for the security-dast methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/security-dast.json", "type": "object", "required": ["tool", "tool_version", "target_env", "target_url", "scope_file", "cadence", "sarif_uploaded"], "properties": {"tool": {"enum": ["zap", "nuclei", "burp"]}, "tool_version": {"type": "string", "minLength": 1}, "target_env": {"enum": ["staging", "preprod", "dev"]}, "target_url": {"type": "string", "pattern": "^https://"}, "scope_file": {"type": "string", "minLength": 1}, "cadence": {"type": "array", "items": {"enum": ["post-deploy", "nightly", "weekly", "on-demand"]}, "minItems": 1}, "templates_pinned": {"type": "boolean"}, "sarif_uploaded": {"type": "boolean", "const": true}}}')
REQUIRED = ['tool', 'tool_version', 'target_env', 'target_url', 'scope_file', 'cadence', 'sarif_uploaded']
...
OK = json.loads('{"tool": "zap", "tool_version": "v0.13.0", "target_env": "staging", "target_url": "https://staging.example.com", "scope_file": ".zap/rules.tsv", "cadence": ["post-deploy", "nightly"], "templates_pinned": true, "sarif_uploaded": true}')
BAD = json.loads('{"tool": "zap", "tool_version": "v0.13.0", "target_env": "staging", "target_url": "https://app.example.com", "scope_file": "", "cadence": [], "templates_pinned": false, "sarif_uploaded": false}')
