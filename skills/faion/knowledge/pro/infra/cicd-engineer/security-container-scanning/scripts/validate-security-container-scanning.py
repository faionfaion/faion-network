#!/usr/bin/env python3
"""validate-security-container-scanning.py

Validate the config artefact for the security-container-scanning methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/security-container-scanning.json", "type": "object", "required": ["scanner", "scanner_version", "severity_block", "sbom_format", "signing", "scan_before_push"], "properties": {"scanner": {"enum": ["trivy", "grype", "clair"]}, "scanner_version": {"type": "string", "minLength": 1, "not": {"enum": ["master", "latest", "main"]}}, "severity_block": {"type": "array", "items": {"enum": ["CRITICAL", "HIGH", "MEDIUM", "LOW"]}, "contains": {"const": "CRITICAL"}}, "ignore_unfixed": {"type": "boolean"}, "sbom_format": {"enum": ["cyclonedx", "spdx"]}, "signing": {"type": "object", "required": ["tool", "mode", "key_in_repo"], "properties": {"tool": {"enum": ["cosign", "notary", "none"]}, "mode": {"enum": ["keyless-oidc", "keyed", "none"]}, "key_in_repo": {"type": "boolean", "const": false}}}, "scan_before_push": {"type": "boolean", "const": true}}}')
REQUIRED = ['scanner', 'scanner_version', 'severity_block', 'sbom_format', 'signing', 'scan_before_push']
...
OK = json.loads('{"scanner": "trivy", "scanner_version": "0.50.0", "severity_block": ["CRITICAL", "HIGH"], "ignore_unfixed": true, "sbom_format": "cyclonedx", "signing": {"tool": "cosign", "mode": "keyless-oidc", "key_in_repo": false}, "scan_before_push": true}')
BAD = json.loads('{"scanner": "trivy", "scanner_version": "master", "severity_block": ["LOW"], "sbom_format": "cyclonedx", "signing": {"tool": "cosign", "mode": "keyed", "key_in_repo": true}, "scan_before_push": false}')
