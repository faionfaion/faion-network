...
maintainers: [faion]
content_id: "cd6a40514ad20ead"
summary: Restore Drill Template — pinned template for the DevOps engineer: fixed shape + named owner + evidence anchors + outcome review, so backup verification (weekly) stops being folklore and starts being a reviewable operating tool.
tags: [infra, pro, template, restore, drill]
---
# Restore Drill Template

## Summary

**One-sentence:** Restore Drill Template — pinned template for the DevOps engineer: fixed shape + named owner + evidence anchors + outcome review, so backup verification (weekly) stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In infrastructure and DevOps, the DevOps engineer runs backup verification (weekly) on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. backup-verification-dr exists but stops short of a structured drill template with RTO targets per data class. `restore-drill-template` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the DevOps engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored template committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-devops-engineer/Backup verification (weekly)`
