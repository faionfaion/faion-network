<!-- purpose: current-state documentation skeleton (lanes + steps + sources) -->
<!-- consumes: SOPs, transcripts, ticket exports -->
<!-- produces: current_state section of the 02-output-contract.xml report -->
<!-- depends-on: content/01-core-rules.xml -->
<!-- token-budget-impact: ~350 tokens loaded as template context -->

# Process Documentation: [Process Name]

**Version:** [X.X]
**Date:** [Date]
**Process Owner:** [Name]
**Analyst:** [Name]
**Stage:** 2 / 5 — Current State

## Process Overview

**Purpose:** [Why this process exists]
**Trigger:** [What starts the process]
**Outcome:** [What the process produces]
**Frequency:** [How often it runs — cite source]
**Scope note:** This documents the actual process, including workarounds. Not the ideal process.

## Scope

**In Scope:** [activities included]
**Out of Scope:** [explicit exclusions]

## Actors

| Actor | Role | Department |
|-------|------|------------|
| [Actor 1] | [Role] | [Dept] |

## Process Steps (cite source for every row)

| Step | Activity | Actor | System | Input | Output | Rules | Source |
|------|----------|-------|--------|-------|--------|-------|--------|
| 1 | [Activity] | [Who] | [Named system only] | [Input] | [Output] | [Rules] | [file:line or ticket-ID] |

_Any step with blank Source is marked unverified and excluded from Stage 3 analysis._

## Contradictions

[List any step described differently by two or more sources — verbatim quotes, do not smooth over]

## Workarounds and Exceptions

[Any step where the actual behaviour differs from what stakeholders say should happen — verbatim]

## Systems Used

| System | Steps | Purpose |
|--------|-------|---------|
| [Named system only] | [Steps] | [Purpose] |

## Metrics (cite source or write "Data not available")

| Metric | Value | Source |
|--------|-------|--------|
| Cycle time | [Value or Data not available] | [Source] |
| Error rate | [Value or Data not available] | [Source] |
