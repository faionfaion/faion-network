#!/usr/bin/env python3
"""validate-stakeholder-conflict-facilitation-script.py — stdlib-only validator for the Stakeholder Conflict Facilitation Script playbook-step artefact.
...
REQUIRED = ['artefact_id', 'owner', 'version', 'last_reviewed', 'steps']
...
OK_JSON = """{"artefact_id": "stakeholder-conflict-facilitation-script-acme-2026-05-23", "owner": "Ruslan Faion <ruslan@faion.net>", "version": "1.0.0", "last_reviewed": "2026-05-23", "steps": [{"n": 1, "name": "intake", "action": "Capture the request in the firewall channel.", "output": "ticket created"}, {"n": 2, "name": "classify", "action": "Apply the 3-question classifier within 5 minutes.", "output": "in-scope / change-order / no-op / backlog", "decision_gate": "If classifier ambiguous, escalate to client clarification."}, {"n": 3, "name": "respond", "action": "Send the corresponding language-pack phrasing.", "output": "email sent + send log entry"}]}"""
BAD = {"owner": "team"}
