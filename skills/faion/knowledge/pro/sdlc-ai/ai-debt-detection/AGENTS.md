...
maintainers: [faion]
summary: Detection protocol for ai debt detection — explicit signals, thresholds, and remediation paths so the failure mode is caught before it ships.
content_id: "3ce9a59ecfd8745a"
...
**One-sentence:** Detection protocol for ai debt detection — explicit signals, thresholds, and remediation paths so the failure mode is caught before it ships.

**One-paragraph:** Detection protocol for ai debt detection — explicit signals, thresholds, and remediation paths so the failure mode is caught before it ships. AI agents generate compilable but conventionally-wrong code that becomes silent debt. A methodology for using AI to detect AI-introduced debt closes the loop.

## Applies If (ALL must hold)

- You instrument or audit for the specific failure mode addressed by ai debt detection.
