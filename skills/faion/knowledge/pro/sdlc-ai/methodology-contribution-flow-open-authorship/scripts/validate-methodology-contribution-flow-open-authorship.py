#!/usr/bin/env python3
"""validate-methodology-contribution-flow-open-authorship.py — validate a methodology-contribution-flow-open-authorship output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'pr_url', 'contributor', 'tier', 'prior_art_done', 'reviewers', 'attribution', 'revenue_share', 'decision']
ENUMS = {'tier': ['free', 'solo', 'pro', 'geek'], 'decision': ['accept', 'request-changes', 'reject']}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = []
ARRAY_FIELDS = ['reviewers']
OBJECT_FIELDS = ['contributor', 'attribution', 'revenue_share']
BOOL_FIELDS = ['prior_art_done']
STRING_FIELDS = ['artefact_id', 'pr_url']
...
VALID_FIXTURE = json.loads('{"artefact_id": "mcf-2026-0001", "version": "1.1.0", "last_reviewed": "2026-05-23", "pr_url": "https://github.com/faionfaion/faion-network/pull/482", "contributor": {"name": "J. Doe", "github": "jdoe", "cla_signed": true}, "tier": "pro", "prior_art_done": true, "reviewers": [{"handle": "alice", "axis": "shape"}, {"handle": "bob", "axis": "evidence"}, {"handle": "carol", "axis": "duplication"}], "attribution": {"maintainers_listed": true, "changelog_entry": "v1.1.0 added by J. Doe"}, "revenue_share": {"enrolled": false, "count_so_far": 1, "trigger_count": 5, "share_pct": 10}, "decision": "accept"}')
INVALID_FIXTURE = json.loads('{"pr_url": "x", "tier": "free", "decision": "approve", "reviewers": [{"handle": "alice", "axis": "shape"}]}')
