#!/usr/bin/env python3
"""validate-paid-test-task-scoring-rubric.py — validate paid-test-task-scoring-rubric output against the 02-output-contract schema.
...
REQUIRED = ['artefact_id', 'owner', 'dimensions', 'calibrated', 'version', 'last_reviewed']
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        prog="validate-paid-test-task-scoring-rubric.py",
        description="Validate paid-test-task-scoring-rubric output against 02-output-contract schema. Exit 0 on pass, 1 on fail, 2 on usage error.",
    )
