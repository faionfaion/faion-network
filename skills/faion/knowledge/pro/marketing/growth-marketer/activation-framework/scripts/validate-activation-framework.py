...
Validates a spec artefact produced by methodology 'activation-framework' against the
...
def validate(obj) -> list:
    ...
    if "slug" in obj and obj["slug"] != "activation-framework":
        errs.append(f"slug must equal 'activation-framework'")
    if "sections" in obj:
...
FIXTURE_OK = {"slug": "activation-framework", "owner": "growth lead", "review_deadline": "2026-06-15", "summary": "Spec for test covering preconditions, procedure, output, and review gate.", "sections": [{"name": "preconditions", "content": "All Applies If items confirmed in writing."}, {"name": "procedure", "content": "Steps 1..n executed per content/04-procedure.xml."}, {"name": "review", "content": "Human reviewer signed off on date."}], "deviation_log_reference": "ops/deviation-log.md#L42", "signoff": {"reviewer": "growth lead", "date": "2026-06-10"}}
...
def self_test() -> int:
    ...
    if errs:
        ...
        return 1
    errs2 = validate({"slug": "activation-framework"})
    if not errs2:
