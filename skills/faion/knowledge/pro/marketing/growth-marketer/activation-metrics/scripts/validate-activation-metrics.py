...
Validates a report artefact produced by methodology 'activation-metrics' against the
...
def validate(obj) -> list:
    ...
    if "slug" in obj and obj["slug"] != "activation-metrics":
        errs.append(f"slug must equal 'activation-metrics'")
...
FIXTURE_OK = {"slug": "activation-metrics", "owner": "analytics owner", "period_start": "2026-05-01", "period_end": "2026-05-22", "metrics": [{"name": "activation_rate", "value": 0.31, "unit": "ratio"}], "findings": ["activation rate up 4pp WoW; biggest lift from onboarding checklist v2"], "deviation_log_reference": "ops/deviation-log.md#L88"}
...
def self_test() -> int:
    ...
    if errs:
        ...
        return 1
    errs2 = validate({"slug": "activation-metrics"})
    if not errs2:
