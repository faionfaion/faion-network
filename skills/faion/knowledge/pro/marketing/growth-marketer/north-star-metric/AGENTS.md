...
summary: Generates an NSM decision record: candidate metrics, value-equation, retention/revenue correlation, chosen NSM + input metrics tree.
content_id: "bbc0ce8282b5c9aa"
complexity: medium
...
- The downstream consumer expects an artefact whose shape matches `produces=decision-record`.
...
- Output consumer wants a different shape than `produces=decision-record` — pick a methodology whose contract matches.
