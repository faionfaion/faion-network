...
summary: Partnership playbook-step: identify (affiliate/co-marketing/integration/reseller/white-label), score on reach/relevance/trust/effort/risk, structure mutual deal, outreach with partner's benefit first, 30/60/90 maintenance cadence.
content_id: "2c8878dbbc04f773"
complexity: medium
...
**One-sentence:** Partnership playbook-step: identify (affiliate/co-marketing/integration/reseller/white-label), score on reach/relevance/trust/effort/risk, structure mutual deal, outreach with partner's benefit first, 30/60/90 maintenance cadence.

**One-paragraph:** Partnership Strategy Five-Stage Framework pins the discipline that turns this workflow from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in `content/02-output-contract.xml` drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale. The artefact lives in the team's versioned space and is refreshed on a stated cadence.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`ops-partnership-strategy` scope).
...
- Вибору типу партнерства під ціль: affiliate vs co-marketing vs integration vs reseller vs white-label.
- Скорингу кандидатів на reach / relevance / trust / effort / risk перед outreach.
- Outreach-листа, де перший абзац — про вигоду партнера, а не про себе.
- 30/60/90 review cadence з measurable outcomes, не «let's keep in touch».
...
| File | Purpose |
|------|---------|
| `templates/ops-partnership-strategy.md` | Working skeleton for the `ops-partnership-strategy` artefact with required fields and `not_applicable: <reason>` markers per row. |
...
|------|---------|--------------|
| `scripts/validate-ops-partnership-strategy.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
