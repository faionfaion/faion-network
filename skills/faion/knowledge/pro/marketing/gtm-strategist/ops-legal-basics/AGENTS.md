...
summary: Legal foundation checklist: Terms of Service, Privacy Policy, and trademark registration with field-level coverage for payment, IP, liability cap, termination, governing law, GDPR/CCPA disclosure.
content_id: "0cff42c4236f55f7"
complexity: medium
produces: checklist
est_tokens: 4200
...
**One-sentence:** Legal foundation checklist: Terms of Service, Privacy Policy, and trademark registration with field-level coverage for payment, IP, liability cap, termination, governing law, GDPR/CCPA disclosure.

**One-paragraph:** Legal Fundamentals for Founders: ToS, Privacy, and Trademark Basics pins the discipline that turns this workflow from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in `content/02-output-contract.xml` drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale. The artefact lives in the team's versioned space and is refreshed on a stated cadence.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`ops-legal-basics` scope).
...
- Founders перед першим платним customer-ом: ToS / Privacy / TM не як галочка, а як шит.
- GDPR / CCPA disclosure вимог при роботі з EU або California audience.
- Liability cap і governing law як термінів, що не копіюються бездумно з шаблонів.
- Trademark registration через USPTO до того, як бренд стане ціллю атаки.
...
| File | Purpose |
|------|---------|
| `templates/ops-legal-basics.md` | Working skeleton for the `ops-legal-basics` artefact with required fields and `not_applicable: <reason>` markers per row. |
...
|------|---------|--------------|
| `scripts/validate-ops-legal-basics.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
