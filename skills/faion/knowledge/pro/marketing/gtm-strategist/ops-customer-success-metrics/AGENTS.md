...
summary: Customer success rubric: weighted health score over usage/feature-adoption/sentiment/engagement/billing with cohort retention curves, NPS/CSAT, and leading-indicator alerts.
content_id: "32a7d62efc4df319"
complexity: medium
produces: rubric
est_tokens: 4200
...
**One-sentence:** Customer success rubric: weighted health score over usage/feature-adoption/sentiment/engagement/billing with cohort retention curves, NPS/CSAT, and leading-indicator alerts.

**One-paragraph:** Customer Success Metrics and Health Scoring pins the discipline that turns this workflow from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in `content/02-output-contract.xml` drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale. The artefact lives in the team's versioned space and is refreshed on a stated cadence.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`ops-customer-success-metrics` scope).
...
- Налаштування health-score з ваг usage / feature-adoption / sentiment / engagement / billing.
- Cohort retention curves і NPS / CSAT моніторинг.
- Алертів на usage-drops і support escalations до того, як account churns.
- Звітності NRR / GRR / expansion ARR для board / investor update.
...
| File | Purpose |
|------|---------|
| `templates/ops-customer-success-metrics.md` | Working skeleton for the `ops-customer-success-metrics` artefact with required fields and `not_applicable: <reason>` markers per row. |
...
|------|---------|--------------|
| `scripts/validate-ops-customer-success-metrics.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
