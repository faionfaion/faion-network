...
summary: Annual planning spec: five-stage Reflect/Envision/Strategize/Plan/Execute framework capped at 3 priorities with quantified kill-switches per key result and four review cadences.
content_id: "656d4118741ca77b"
complexity: medium
produces: spec
est_tokens: 4900
...
**One-sentence:** Annual planning spec: five-stage Reflect/Envision/Strategize/Plan/Execute framework capped at 3 priorities with quantified kill-switches per key result and four review cadences.

**One-paragraph:** Annual Planning Process pins the discipline that turns this workflow from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in `content/02-output-contract.xml` drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale. The artefact lives in the team's versioned space and is refreshed on a stated cadence.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`ops-annual-planning-process` scope).
...
- Соло-операторів і мікроагенцій, що пишуть річний план у грудні-січні.
- Командного перегляду пріоритетів end-of-quarter без розмиття до 7-item списків.
- Декомпозиції annual goal у monthly task tree з kill-switch per KR.
- Реалістичної відмови від пунктів, що не вписуються в cap = 3 priorities.
...
| File | Purpose |
|------|---------|
| `templates/ops-annual-planning-process.md` | Working skeleton for the `ops-annual-planning-process` artefact with required fields and `not_applicable: <reason>` markers per row. |
...
|------|---------|--------------|
| `scripts/validate-ops-annual-planning-process.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
