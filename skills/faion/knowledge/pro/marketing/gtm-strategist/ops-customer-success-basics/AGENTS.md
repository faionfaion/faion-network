...
summary: Customer success spec: Define/Measure/Engage/Enable/Expand stages with event-triggered touchpoints, max one auto-reach per account per week, no autonomous refunds.
content_id: "9e1b2024a4d3681b"
complexity: medium
produces: spec
est_tokens: 4900
...
- Соло- і малих SaaS-операторів, що будують proactive retention з нуля.
- Налаштування north-star метрики продукту і health-score per account.
- Event-triggered touchpoints (не calendar-based mass blasts).
- Гейтера на refunds / downgrades: жоден агент не виписує credits сам.
...
| File | Purpose |
|------|---------|
| `templates/ops-customer-success-basics.md` | Working skeleton for the `ops-customer-success-basics` artefact with required fields and `not_applicable: <reason>` markers per row. |
...
|------|---------|--------------|
| `scripts/validate-ops-customer-success-basics.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
