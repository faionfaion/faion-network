#!/usr/bin/env python3
"""validate-meta-audience-targeting.py — schema validator for the meta-audience-targeting methodology artefact.
...
SCHEMA = { '$id': 'https://faion.net/schemas/meta-audience-targeting.json',
  '$schema': 'https://json-schema.org/draft/2020-12/schema',
  'properties': { 'advantage_plus': {'required': ['enabled', 'rationale'], 'type': 'object'},
                  'behavioral_filters': { 'items': { 'enum': [ 'engaged_shoppers',
                                                               'returned_travelers',
                                                               'frequent_travelers',
                                                               'small_business_owners',
                                                               'tech_early_adopters']},
                                          'type': 'array'},
                  'dynamic_creative': { 'required': ['enabled', 'audience_breadth'],
                                        'type': 'object'},
                  'exclusion_matrix': { 'properties': { 'bofu': {'minItems': 1, 'type': 'array'},
                                                        'mofu': {'minItems': 1, 'type': 'array'},
                                                        'tofu': {'minItems': 1, 'type': 'array'}},
                                        'required': ['tofu', 'mofu', 'bofu'],
                                        'type': 'object'},
                  'interest_stacks': { 'items': { 'properties': { 'mode': {'enum': ['or']},
                                                                  'themes': { 'maxItems': 3,
                                                                              'minItems': 2,
                                                                              'type': 'array'}},
                                                  'required': ['mode', 'themes'],
                                                  'type': 'object'},
                                       'minItems': 1,
                                       'type': 'array'}},
  'required': [ 'interest_stacks',
                'behavioral_filters',
                'advantage_plus',
                'exclusion_matrix',
                'dynamic_creative'],
...
OK = { 'advantage_plus': {'enabled': False, 'rationale': 'niche B2B product'},
  'behavioral_filters': ['tech_early_adopters', 'engaged_shoppers'],
  'dynamic_creative': {'audience_breadth': 'manual_tier_2', 'enabled': False},
  'exclusion_matrix': { 'bofu': ['converters_7d'],
                        'mofu': ['purchasers_30d'],
                        'tofu': ['current_customers', 'recent_converters']},
  'interest_stacks': [{'mode': 'or', 'themes': ['SaaS', 'Developer Tools', 'DevOps']}]}
BAD = { 'advantage_plus': {'enabled': True},
  'behavioral_filters': ['page_visited_7d'],
  'dynamic_creative': {'audience_breadth': 'narrow', 'enabled': True},
  'exclusion_matrix': {},
  'interest_stacks': [{'mode': 'and', 'themes': ['A', 'B', 'C', 'D', 'E']}]}
