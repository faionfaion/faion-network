#!/usr/bin/env python3
"""validate-google-shopping-ads.py — schema validator for the google-shopping-ads methodology artefact.
...
SCHEMA = { '$id': 'https://faion.net/schemas/google-shopping-ads.json',
  '$schema': 'https://json-schema.org/draft/2020-12/schema',
  'properties': { 'feed_health_pct': {'maximum': 100, 'minimum': 80, 'type': 'number'},
                  'negative_keywords': { 'items': {'type': 'string'},
                                         'minItems': 1,
                                         'type': 'array'},
                  'priority_tiers': { 'items': {'enum': ['high', 'medium', 'low', 'clearance']},
                                      'minItems': 2,
                                      'type': 'array'},
                  'product_groups': { 'items': { 'required': ['name', 'partition_by'],
                                                 'type': 'object'},
                                      'minItems': 2,
                                      'type': 'array'},
                  'roas_targets': { 'properties': {'high': {'minimum': 4, 'type': 'number'}},
                                    'required': ['high'],
                                    'type': 'object'}},
  'required': [ 'feed_health_pct',
                'priority_tiers',
                'product_groups',
                'negative_keywords',
                'roas_targets'],
...
OK = { 'feed_health_pct': 92,
  'negative_keywords': ['free', 'used', 'torrent', 'jobs'],
  'priority_tiers': ['high', 'medium', 'clearance'],
  'product_groups': [ {'name': 'high-margin', 'partition_by': 'margin>30%'},
                      {'name': 'clearance', 'partition_by': 'tag=clearance'}],
  'roas_targets': {'clearance': 1.6, 'high': 4.5, 'medium': 2.8}}
BAD = { 'feed_health_pct': 60,
  'negative_keywords': [],
  'priority_tiers': ['high'],
  'product_groups': [{'name': 'all'}],
  'roas_targets': {}}
