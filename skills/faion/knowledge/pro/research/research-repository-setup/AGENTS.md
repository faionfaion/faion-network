---
slug: research-repository-setup
tier: pro
group: research
domain: research
version: 1.0.0
...
maintainers: [faion]
content_id: "f082a9c5f2383e88"
summary: Research Repository Setup — pinned method for the UX/UI designer: fixed shape + named owner + evidence anchors + outcome review, so user research sprint: discovery to recommendations (4 weeks) stops being folklore and starts being a reviewable operating tool.
tags: [research, pro, method, repository, setup]
---
# Research Repository Setup

## Summary

**One-sentence:** Research Repository Setup — pinned method for the UX/UI designer: fixed shape + named owner + evidence anchors + outcome review, so user research sprint: discovery to recommendations (4 weeks) stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In user / market research, the UX/UI designer runs user research sprint: discovery to recommendations (4 weeks) on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Continuous discovery presumes a research repo (tagged transcripts, themes, decisions). No methodology defines structure or tooling. `research-repository-setup` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the UX/UI designer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-ux-ui-designer/User research sprint: discovery to recommendations (4 weeks)`
