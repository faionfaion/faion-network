#!/usr/bin/env python3
"""validate-market-research-tam-sam-som.py

Validate an artefact produced by the market-research-tam-sam-som methodology against the JSON
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/market-research-tam-sam-som.json", "type": "object", "required": ["category", "geography", "icp", "tam", "sam", "som", "triangulation", "som_confidence"], "properties": {"category": {"type": "string"}, "geography": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "icp": {"type": "string"}, "tam": {"type": "object", "required": ["value_usd", "top_down_source", "bottom_up_source"]}, "sam": {"type": "object"}, "som": {"type": "object"}, "triangulation": {"type": "object", "required": ["delta_pct", "reconciliation"]}, "som_confidence": {"enum": ["high", "medium", "low"]}}}')
