#!/usr/bin/env python3
"""validate-accessibility-first-design.py — validate a accessibility-first-design output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'component', 'owner', 'structure', 'focus_order', 'aria_budget', 'contrast_tokens', 'motion_alternates']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = []
ARRAY_FIELDS = ['focus_order', 'aria_budget', 'motion_alternates']
OBJECT_FIELDS = ['structure', 'contrast_tokens']
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'component', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "afd-modal-2026-0001", "version": "1.1.0", "last_reviewed": "2026-05-23", "component": "checkout-confirm-modal", "owner": "@designer-a", "structure": {"heading_level": "h2", "landmark": "dialog"}, "focus_order": ["close-button", "cancel-button", "confirm-button"], "aria_budget": [{"attr": "aria-modal=true", "rationale": "native <dialog> not used; emulate modal semantics"}], "contrast_tokens": {"text-on-surface": "4.7:1", "primary-button": "5.2:1"}, "motion_alternates": [{"animation": "modal-slide-in", "reduced": "opacity-fade-150ms"}]}')
INVALID_FIXTURE = json.loads('{"component": "modal", "aria_budget": [{"attr": "aria-label=button"}], "motion_alternates": []}')
