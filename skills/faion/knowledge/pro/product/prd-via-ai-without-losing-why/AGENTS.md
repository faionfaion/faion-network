...
maintainers: [faion]
content_id: "195071a2e499eea1"
summary: Prd Via AI Without Losing Why — pinned method for the product manager: fixed shape + named owner + evidence anchors + outcome review, so validate an ai feature before it consumes a quarter stops being folklore and starts being a reviewable operating tool.
tags: [product, pro, method, prd, via, without, losing, why]
---
# Prd Via AI Without Losing Why

## Summary

**One-sentence:** Prd Via AI Without Losing Why — pinned method for the product manager: fixed shape + named owner + evidence anchors + outcome review, so validate an ai feature before it consumes a quarter stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In product management, the product manager runs validate an ai feature before it consumes a quarter on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Spec-writing exists at solo tier but it predates AI-drafted PRDs. The new failure mode is verbose AI-generated specs that lose the 'why' and bury the user job. Need a methodology that uses AI for the structure and humans for the why anchor. `prd-via-ai-without-losing-why` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the product manager starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-product-manager/Validate an AI feature before it consumes a quarter`
