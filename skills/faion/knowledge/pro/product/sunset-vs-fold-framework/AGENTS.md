...
summary: Decision framework for whether to sunset a product (kill cleanly) vs fold it (merge capability into another) when discontinuing a line of business.
content_id: "bb22b86ec0af6118"
complexity: medium
produces: decision-record
est_tokens: 2800
tags: [sunset, fold, decision-framework, product-lifecycle, discontinuation]
---
# Sunset vs Fold Framework

## Summary

**One-sentence:** Decision framework for whether to sunset a product (kill cleanly) vs fold it (merge capability into another) when discontinuing a line of business.

**One-paragraph:** Decision framework for whether to sunset a product (kill cleanly) vs fold it (merge capability into another) when discontinuing a line of business. The methodology produces a `decision-record` artefact gated by an explicit output contract (JSON Schema draft-07) + decision tree referencing core rules. Apply when the preconditions in `## Applies If` ALL hold and none of the `## Skip If` disqualifiers fires. Skip and reach for a sibling methodology otherwise.

**Ефективно для:**

- Repeatable cycles де треба явний decision-record, не ad-hoc notes.
...
- Operating context matches the produces shape (`decision-record`) — outcome can be inspected as a discrete artefact.
...
| `synthesize-decision-record` | opus | Final cross-input judgment producing the decision-record. |
...
|------|---------|--------------|
| `scripts/validate-sunset-vs-fold-framework.py` | Validate output artefact against JSON Schema | Pre-commit + CI on each artefact change |
