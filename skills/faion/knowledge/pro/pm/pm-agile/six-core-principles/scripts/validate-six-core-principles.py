#!/usr/bin/env python3
"""validate-six-core-principles.py

Validate the `rubric` artefact for the `six-core-principles` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/six-core-principles.json', 'title': 'Six Core Principles (PMBOK 7)', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'criteria', 'overall'], 'properties': {'slug': {'type': 'string', 'const': 'six-core-principles'}, 'criteria': {'type': 'array', 'minItems': 1, 'items': {'type': 'object', 'required': ['id', 'score', 'evidence'], 'properties': {'id': {'type': 'string'}, 'score': {'type': 'integer', 'minimum': 0, 'maximum': 3}, 'evidence': {'type': 'string'}}}}, 'overall': {'enum': ['GREEN', 'YELLOW', 'RED']}}}

OK_FIXTURE = {'slug': 'six-core-principles', 'criteria': [{'id': 'principle-observable-required', 'score': 3, 'evidence': 'decision log entry #142'}], 'overall': 'GREEN'}

BAD_FIXTURE = {'slug': 'six-core-principles', 'criteria': [{'id': 'x', 'score': 9, 'evidence': ''}], 'overall': 'BLUE'}
