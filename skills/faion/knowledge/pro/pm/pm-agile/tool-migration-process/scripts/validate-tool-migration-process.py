#!/usr/bin/env python3
"""validate-tool-migration-process.py

Validate the `playbook-step` artefact for the `tool-migration-process` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/tool-migration-process.json', 'title': 'Cross-Tool PM Migration — Process', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'steps'], 'properties': {'slug': {'type': 'string', 'const': 'tool-migration-process'}, 'steps': {'type': 'array', 'minItems': 1, 'items': {'type': 'object', 'required': ['n', 'name', 'action'], 'properties': {'n': {'type': 'integer', 'minimum': 1}, 'name': {'type': 'string'}, 'action': {'type': 'string'}, 'decision_gate': {'type': 'string'}}}}}}

OK_FIXTURE = {'slug': 'tool-migration-process', 'steps': [{'n': 1, 'name': 'prepare', 'action': 'Collect inputs', 'decision_gate': 'All inputs present'}]}

BAD_FIXTURE = {'slug': 'tool-migration-process', 'steps': []}
