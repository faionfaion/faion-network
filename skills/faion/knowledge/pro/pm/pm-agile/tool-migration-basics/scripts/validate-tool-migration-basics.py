#!/usr/bin/env python3
"""validate-tool-migration-basics.py

Validate the `spec` artefact for the `tool-migration-basics` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/tool-migration-basics.json', 'title': 'Cross-Tool Migration Basics', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'sections', 'approved_by'], 'properties': {'slug': {'type': 'string', 'const': 'tool-migration-basics'}, 'sections': {'type': 'array', 'minItems': 1, 'items': {'type': 'object', 'required': ['title', 'body'], 'properties': {'title': {'type': 'string', 'minLength': 3}, 'body': {'type': 'string', 'minLength': 10}}}}, 'approved_by': {'type': 'string'}, 'approved_at': {'type': 'string'}}}

OK_FIXTURE = {'slug': 'tool-migration-basics', 'sections': [{'title': 'Scope', 'body': 'In: A, B, C. Out: D.'}, {'title': 'Acceptance', 'body': 'Measurable thresholds per deliverable.'}], 'approved_by': 'Sponsor', 'approved_at': '2026-05-23'}

BAD_FIXTURE = {'slug': 'tool-migration-basics', 'sections': [{'title': 'x'}]}
