...
Validate the `config` artefact for the `project-integration` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/project-integration.json', 'title': 'Project Integration Management', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'config'], 'properties': {'slug': {'type': 'string', 'const': 'project-integration'}, 'config': {'type': 'object', 'minProperties': 1}, 'format': {'enum': ['yaml', 'toml', 'json', 'markdown']}}}

OK_FIXTURE = {'slug': 'project-integration', 'config': {'key': 'value'}, 'format': 'yaml'}

BAD_FIXTURE = {'slug': 'project-integration'}
