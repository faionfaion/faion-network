#!/usr/bin/env python3
"""validate-project-closure.py

Validate the `spec` artefact for the `project-closure` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/project-closure.json', 'title': 'Project Closure', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'sections', 'approved_by'], 'properties': {'slug': {'type': 'string', 'const': 'project-closure'}, 'sections': {'type': 'array', 'minItems': 1, 'items': {'type': 'object', 'required': ['title', 'body'], 'properties': {'title': {'type': 'string', 'minLength': 3}, 'body': {'type': 'string', 'minLength': 10}}}}, 'approved_by': {'type': 'string'}, 'approved_at': {'type': 'string'}}}

OK_FIXTURE = {'slug': 'project-closure', 'sections': [{'title': 'Scope', 'body': 'In: A, B, C. Out: D.'}, {'title': 'Acceptance', 'body': 'Measurable thresholds per deliverable.'}], 'approved_by': 'Sponsor', 'approved_at': '2026-05-23'}

BAD_FIXTURE = {'slug': 'project-closure', 'sections': [{'title': 'x'}]}
