...
Validate the `config` artefact for the `quality-management` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/quality-management.json', 'title': 'Quality Management', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'config'], 'properties': {'slug': {'type': 'string', 'const': 'quality-management'}, 'config': {'type': 'object', 'minProperties': 1}, 'format': {'enum': ['yaml', 'toml', 'json', 'markdown']}}}

OK_FIXTURE = {'slug': 'quality-management', 'config': {'key': 'value'}, 'format': 'yaml'}

BAD_FIXTURE = {'slug': 'quality-management'}
