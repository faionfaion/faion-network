...
Validate the `config` artefact for the `change-control` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/change-control.json', 'title': 'Change Control', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'config'], 'properties': {'slug': {'type': 'string', 'const': 'change-control'}, 'config': {'type': 'object', 'minProperties': 1}, 'format': {'enum': ['yaml', 'toml', 'json', 'markdown']}}}

OK_FIXTURE = {'slug': 'change-control', 'config': {'key': 'value'}, 'format': 'yaml'}

BAD_FIXTURE = {'slug': 'change-control'}
