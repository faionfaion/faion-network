#!/usr/bin/env python3
"""validate-raci-matrix.py

Validate the config artefact for the raci-matrix methodology against the schema
...
VERSION = "1.1.0"
PRODUCES = "config"
RULES = ['exactly-one-accountable', 'at-least-one-responsible', 'consulted-cap-4', 'walk-in-kickoff', 'role-titles-not-names', 'diff-not-rewrite']
...
OK = {"__faion_header__": {"methodology": "raci-matrix", "version": "1.1.0", "produces": "config", "rules_satisfied": ["exactly-one-accountable", "at-least-one-responsible", "consulted-cap-4"]}, "config_path": "config/raci-matrix.yaml", "entries": [{"key": "k1", "value": "v1"}]}
BAD = {"artefact_path": "docs/raci-matrix.md"}
