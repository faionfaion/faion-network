...
maintainers: [faion]
summary: End-to-end playbook for ai leverage estimation model that walks an operator from trigger to closed outcome with named artefacts at each step.
...
**One-sentence:** End-to-end playbook for ai leverage estimation model that walks an operator from trigger to closed outcome with named artefacts at each step.

**One-paragraph:** End-to-end playbook for ai leverage estimation model that walks an operator from trigger to closed outcome with named artefacts at each step. Existing cost-estimation methodologies (PERT, three-point) assume pre-AI productivity. Senior outsource devs now ship 2–5x on glue code and 1.0–1.2x on regulated work. No methodology quantifies the per-task-class multiplier; senior dev under- or over-quotes accordingly.

## Applies If (ALL must hold)

- You are executing the cross-cutting workflow addressed by ai leverage estimation model end to end.
