---
slug: regulatory-uncertainty-buffer
tier: pro
group: pm
domain: pm
version: 1.0.0
...
maintainers: [faion]
content_id: "0d81a967a8621e9a"
summary: Regulatory Uncertainty Buffer — pinned buffer for the outsource delivery specialist: fixed shape + named owner + evidence anchors + outcome review, so fixed-price vs t&m estimation with ai risk buffer stops being folklore and starts being a reviewable operating tool.
tags: [pm, pro, buffer, regulatory, uncertainty]
---
# Regulatory Uncertainty Buffer

## Summary

**One-sentence:** Regulatory Uncertainty Buffer — pinned buffer for the outsource delivery specialist: fixed shape + named owner + evidence anchors + outcome review, so fixed-price vs t&m estimation with ai risk buffer stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In project / programme management, the outsource delivery specialist runs fixed-price vs t&m estimation with ai risk buffer on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Estimating regulated work needs an explicit buffer for 'auditor will ask for X mid-build'. risk-management is generic; nothing addresses the specific regulatory-discovery cost class. `regulatory-uncertainty-buffer` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the outsource delivery specialist starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored buffer committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p4-outsource-specialist/Fixed-Price vs T&M Estimation with AI Risk Buffer`
