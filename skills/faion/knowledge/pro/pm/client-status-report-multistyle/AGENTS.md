...
summary: Generates a tiered status report — executive 1-pager, PM detail, engineering log — derived from one source-of-truth weekly state.
content_id: "4caf30deeddc6eeb"
complexity: medium
produces: report
est_tokens: 4500
tags: [client-status-report-multistyle, pm, pro, report]
---
# Client Status Report Multistyle

## Summary

**One-sentence:** Generates a tiered status report — executive 1-pager, PM detail, engineering log — derived from one source-of-truth weekly state.

**One-paragraph:** Client Status Report Multistyle addresses the gap identified by the `role-project-manager/Async cross-timezone delivery cadence (P4 outsource)` playbook: Async outsource engagements need one source-of-truth that ladders into exec / PM / engineering views. Hand-crafting three reports duplicates effort and drifts. Mechanism: a typed input → bounded transformation → contract-checked output. Primary output: a versioned `report` artefact carrying a named accountable owner, input citations, and a review date — downstream agents and human reviewers consume it without re-deriving the rationale.
...
- Task is an instance of `role-project-manager/Async cross-timezone delivery cadence (P4 outsource)` OR a closely-adjacent variant in the same engagement shape.
...
|----------|--------|--------|
| Recent context for the `role-project-manager/Async cross-timezone delivery cadence (P4 outsource)` task (last 30 days) | Markdown / chat log | engagement notes |
...
|------|---------|--------------|
| `scripts/validate-client-status-report-multistyle.py` | Enforce the Client Status Report Multistyle output contract against the JSON Schema. | After subagent returns, before downstream consumer reads. |
