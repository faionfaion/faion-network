...
maintainers: [faion]
content_id: "7141da37d5395d7a"
summary: RAG Policy Thresholds — pinned method for the project manager: fixed shape + named owner + evidence anchors + outcome review, so write the weekly client status report (p4) / leadership update (p6) stops being folklore and starts being a reviewable operating tool.
tags: [pm, pro, method, rag, policy, thresholds]
---
# RAG Policy Thresholds

## Summary

**One-sentence:** RAG Policy Thresholds — pinned method for the project manager: fixed shape + named owner + evidence anchors + outcome review, so write the weekly client status report (p4) / leadership update (p6) stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In project / programme management, the project manager runs write the weekly client status report (p4) / leadership update (p6) on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. RAG colors are inconsistent across PMs and clients; a tier-pro standard would reduce client-PM trust friction. `rag-policy-thresholds` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the project manager starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-project-manager/Write the weekly client status report (P4) / leadership update (P6)`
