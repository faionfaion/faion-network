...
maintainers: [faion]
content_id: "3892c23e5eb703bd"
summary: Retainer Vs Project Rubric — pinned rubric for the micro-agency founder: fixed shape + named owner + evidence anchors + outcome review, so productize one service from custom to fixed stops being folklore and starts being a reviewable operating tool.
tags: [pm, pro, rubric, retainer, project]
---
# Retainer Vs Project Rubric

## Summary

**One-sentence:** Retainer Vs Project Rubric — pinned rubric for the micro-agency founder: fixed shape + named owner + evidence anchors + outcome review, so productize one service from custom to fixed stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In project / programme management, the micro-agency founder runs productize one service from custom to fixed on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. ops-pricing-strategy covers pricing models but does not give an agency a decision rubric for when to push a client onto retainer vs keep them on project. For micro-agencies retainer revenue is the difference between cashflow panic and cashflow stability; this needs its own opinionated decision tree (predictability of work, churn risk, scope-creep risk, margin profile). `retainer-vs-project-rubric` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the micro-agency founder starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored rubric committed to the team's knowledge space.
