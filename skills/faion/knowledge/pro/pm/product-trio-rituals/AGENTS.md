...
maintainers: [faion]
content_id: "b68549fd49f7ba6f"
summary: Product Trio Rituals — pinned ritual for the product manager: fixed shape + named owner + evidence anchors + outcome review, so reset prioritization war with mixed-method scoring stops being folklore and starts being a reviewable operating tool.
tags: [pm, pro, ritual, product, trio, rituals]
---
# Product Trio Rituals

## Summary

**One-sentence:** Product Trio Rituals — pinned ritual for the product manager: fixed shape + named owner + evidence anchors + outcome review, so reset prioritization war with mixed-method scoring stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In project / programme management, the product manager runs reset prioritization war with mixed-method scoring on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Stakeholder-management covers external; continuous-discovery-habits covers the PM-as-individual. Nothing covers the PM + designer + tech-lead triad: weekly cadence, decision rights, conflict escalation, shared artifacts. P6 PMs cannot operate without this. `product-trio-rituals` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the product manager starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored ritual committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-product-manager/Reset prioritization war with mixed-method scoring`
