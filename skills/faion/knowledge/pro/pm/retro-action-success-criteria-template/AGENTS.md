---
slug: retro-action-success-criteria-template
tier: pro
group: pm
domain: pm
version: 1.0.0
...
maintainers: [faion]
content_id: "65ada5fd9c2e8f79"
summary: Retro Action Success Criteria Template — pinned template for the product-dev team: fixed shape + named owner + evidence anchors + outcome review, so bi-weekly retro with mistake-memory feedback stops being folklore and starts being a reviewable operating tool.
tags: [pm, pro, template, retro, action, success, criteria]
---
# Retro Action Success Criteria Template

## Summary

**One-sentence:** Retro Action Success Criteria Template — pinned template for the product-dev team: fixed shape + named owner + evidence anchors + outcome review, so bi-weekly retro with mistake-memory feedback stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In project / programme management, the product-dev team runs bi-weekly retro with mistake-memory feedback on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. Retros generate vague action items because no methodology forces measurable success criteria per experiment. `retro-action-success-criteria-template` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the product-dev team starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored template committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `p6-product-dev-team/Bi-weekly retro with mistake-memory feedback`
