...
maintainers: [faion]
content_id: "54468786d710b630"
summary: Prompt Injection Test Suite — pinned method for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so ship a qa program for an ai-powered product stops being folklore and starts being a reviewable operating tool.
tags: [ai, pro, method, prompt, injection, test, suite]
---
# Prompt Injection Test Suite

## Summary

**One-sentence:** Prompt Injection Test Suite — pinned method for the QA engineer: fixed shape + named owner + evidence anchors + outcome review, so ship a qa program for an ai-powered product stops being folklore and starts being a reviewable operating tool.

**One-paragraph:** In AI / agent engineering, the QA engineer runs ship a qa program for an ai-powered product on a recurring cadence — but the corpus only covers the upstream concepts, not the artefact that closes the loop. OWASP LLM Top 10 puts prompt injection at #1. Solo-tier security-testing.md covers OWASP Web Top 10 only — no LLM Top 10. Need a methodology for building a prompt-injection / jailbreak / data-exfil test suite that runs in CI on every prompt or model change. `prompt-injection-test-suite` pins the artefact: a fixed shape, named owner, evidence anchors, and a published review cadence. It is loaded when the QA engineer starts the block named in the trigger and produces a committed artefact reviewed against outcomes at the next iteration. Mechanism: rule-bound output contract + per-application evidence + outcome review. Primary output: a versioned, owned, evidence-anchored method committed to the team's knowledge space.

## Applies If (ALL must hold)

- the block this methodology unblocks is on the operating cadence: - `role-qa-engineer/Ship a QA program for an AI-powered product`
