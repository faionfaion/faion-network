#!/usr/bin/env python3
"""validate-eval-in-prod-sampling-policy.py — validate a Eval-in-Prod Sampling Policy artefact JSON.

Usage:
  validate-eval-in-prod-sampling-policy.py <artefact.json>
...
ID_RE = re.compile(r"^eips-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^eips-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "eips-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/eval-in-prod-sampling-policy.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-eval-in-prod-sampling-policy.py <artefact.json>\n")
        return 2
    try:
