#!/usr/bin/env python3
"""validate-cost-quality-tradeoff-framework.py — validate a Cost-Quality Tradeoff Framework artefact JSON.

Usage:
  validate-cost-quality-tradeoff-framework.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^cqt-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "cqt-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/cost-quality-tradeoff-framework.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-cost-quality-tradeoff-framework.py <artefact.json>\n")
        return 2
    try:
