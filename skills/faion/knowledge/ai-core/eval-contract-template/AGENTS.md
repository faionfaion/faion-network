...
**One-sentence:** Produces an eval-contract spec: golden set, scorer list (deterministic + judge), threshold, sampling policy, regression gate, named owner.

**Ефективно для:** AI features merging prompt changes on reviewer vibes — there is no eval contract gating PRs, so regressions ship undetected.

**One-paragraph:** This methodology pins the recurring decision around "eval contract template" into a typed artefact governed by 5 testable rules. Inputs are typed and sourced; the output is contract-checked; a single accountable owner is named; the decision tree routes preconditions to a run/skip outcome. Source material grounded in: Vadim — Workflow-First Production Evals 2026.

## Applies If (ALL must hold)

- Task is an instance of the recurring "eval contract template" decision OR a closely-adjacent variant.
...
|---|---|---|
| Last 30 days of context for the recurring "eval contract template" task | text / logs | system of record |
...
|---|---|---|---|
| `content/01-core-rules.xml` | essential | 5 testable rules customised to "eval contract template" | ~900 |
...
| File | Purpose |
|---|---|
| `templates/eval-contract-template.json` | JSON schema for the output contract |
| `templates/eval-contract-template.md.j2` | Markdown skeleton with required fields |
| `templates/eval-contract-template.md` | Markdown skeleton with required fields Generated from `templates/eval-contract-template.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|---|---|---|
| `scripts/validate-eval-contract-template.py` | Enforce the output contract | After subagent returns, before downstream consumer reads |
...
### `templates/eval-contract-template.json`

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://faion.network/schema/eval-contract-template.json",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
