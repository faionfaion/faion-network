#!/usr/bin/env python3
"""validate-data-exfiltration-canary-tokens.py — validate a Data Exfiltration Canary Tokens artefact JSON.

Usage:
  validate-data-exfiltration-canary-tokens.py <artefact.json>
...
ID_RE = re.compile(r"^dect-[a-z0-9-]+$")
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^dect-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "dect-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/data-exfiltration-canary-tokens.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-data-exfiltration-canary-tokens.py <artefact.json>\n")
        return 2
    try:
