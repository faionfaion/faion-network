#!/usr/bin/env python3
"""validate-human-in-the-loop-pattern-catalog.py — validate a Human-in-the-Loop Pattern Catalog artefact JSON.

Usage:
  validate-human-in-the-loop-pattern-catalog.py <artefact.json>
...
def _check(doc: dict[str, Any]) -> list[str]:
    ...
    if not ID_RE.match(str(doc["artefact_id"])):
        errs.append(f"artefact_id must match ^hpc-[a-z0-9-]+$, got {doc['artefact_id']!r}")
...
def _self_test() -> int:
    fixture = {
        "artefact_id": "hpc-self-test-001",
        "owner": "alex@acme.com",
        "decision": "decision shape consistent with templates/human-in-the-loop-pattern-catalog.md example",
...
def main(argv: list[str]) -> int:
    ...
    if "--self-test" in argv:
        return _self_test()
    if len(argv) != 2:
        sys.stderr.write("usage: validate-human-in-the-loop-pattern-catalog.py <artefact.json>\n")
        return 2
    try:
