#!/usr/bin/env python3
"""validate-go-tygo-frontend-contract.py — validate the config artefact for the lang-go-tygo-frontend-contract methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lang-go-tygo-frontend-contract.json", "type": "object", "required": ["tygo_yaml_path", "output_dir", "packages", "ci_diff_gate", "type_mappings"], "properties": {"tygo_yaml_path": {"type": "string"}, "output_dir": {"type": "string"}, "packages": {"type": "array", "minItems": 1, "items": {"type": "string"}}, "ci_diff_gate": {"const": true}, "type_mappings": {"type": "object", "minProperties": 1, "additionalProperties": {"type": "string"}}}}""")
VALID_FIXTURE = json.loads(r"""{"tygo_yaml_path": "tygo.yaml", "output_dir": "frontend/src/api-types/", "packages": ["github.com/acme/api/internal/dto"], "ci_diff_gate": true, "type_mappings": {"time.Time": "string", "decimal.Decimal": "string", "json.RawMessage": "unknown"}}""")
INVALID_FIXTURE = json.loads(r"""{"tygo_yaml_path": "tygo.yaml", "packages": [], "ci_diff_gate": false, "type_mappings": {}}""")
