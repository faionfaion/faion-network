#!/usr/bin/env python3
"""validate-precommit-floor.py — validate the config artefact for the lint-precommit-floor methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lint-precommit-floor.json", "type": "object", "required": ["framework", "config_path", "hook_categories", "setup_command", "ci_mirror"], "properties": {"framework": {"enum": ["pre-commit", "lefthook", "husky+lint-staged"]}, "config_path": {"type": "string"}, "hook_categories": {"type": "array", "minItems": 4, "uniqueItems": true, "items": {"enum": ["format", "lint", "secret-scan", "type-check", "iac-lint"]}, "contains": {"enum": ["secret-scan"]}}, "setup_command": {"type": "string"}, "ci_mirror": {"const": true}}}""")
VALID_FIXTURE = json.loads(r"""{"framework": "pre-commit", "config_path": ".pre-commit-config.yaml", "hook_categories": ["format", "lint", "secret-scan", "type-check"], "setup_command": "make hooks", "ci_mirror": true}""")
INVALID_FIXTURE = json.loads(r"""{"framework": "manual", "config_path": "hooks.txt", "hook_categories": ["lint"], "ci_mirror": false}""")
