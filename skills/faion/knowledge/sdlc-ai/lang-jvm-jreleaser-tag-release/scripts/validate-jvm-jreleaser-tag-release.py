#!/usr/bin/env python3
"""validate-jvm-jreleaser-tag-release.py — validate the config artefact for the lang-jvm-jreleaser-tag-release methodology.
...
SCHEMA = json.loads(r"""{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/lang-jvm-jreleaser-tag-release.json", "type": "object", "required": ["jreleaser_yml", "tag_pattern", "signing_enabled", "conventional_commits_enforced", "github_release_enabled"], "properties": {"jreleaser_yml": {"type": "string"}, "tag_pattern": {"const": "^v[0-9]+\\.[0-9]+\\.[0-9]+$"}, "signing_enabled": {"const": true}, "conventional_commits_enforced": {"const": true}, "github_release_enabled": {"const": true}, "distribution_targets": {"type": "array", "minItems": 1, "items": {"enum": ["maven-central", "github-packages", "sonatype-snapshots"]}}}}""")
VALID_FIXTURE = json.loads(r"""{"jreleaser_yml": "jreleaser.yml", "tag_pattern": "^v[0-9]+\\.[0-9]+\\.[0-9]+$", "signing_enabled": true, "conventional_commits_enforced": true, "github_release_enabled": true, "distribution_targets": ["maven-central", "github-packages"]}""")
INVALID_FIXTURE = json.loads(r"""{"jreleaser_yml": "release.yml", "signing_enabled": false}""")
