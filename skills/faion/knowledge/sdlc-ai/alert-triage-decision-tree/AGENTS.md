...
### `templates/triage-record.json`

```json
{
  "alert_id": "alm-0000000000",
  "severity": "p3",
  "blast_radius": "single-user",
  "route": "async-ticket",
  "owner": "team-handle",
  "sla_minutes": 1440,
  "audit_reason": "low-impact placeholder"
}
```

### `templates/tree-config.yaml`

```yaml
tree:
  - when: "severity == p0 || blast_radius == global"
    route: page
    sla_minutes: 5
  - when: "severity == p1 && blast_radius in [cohort, single-tenant]"
    route: page
    sla_minutes: 15
  - when: "severity == p2"
    route: async-ticket
    sla_minutes: 240
  - when: "severity == p3 && signal_normalized == true"
    route: auto-suppress
    sla_minutes: 0
```
