#!/usr/bin/env python3
"""validate-anti-pattern-rationale-template.py — stdlib-only validator for the anti-pattern-rationale-template output artefact.
...
REQUIRED = ['artefact_id', 'owner', 'version', 'last_reviewed', 'ui_surface', 'anti_pattern_name', 'evidence_link', 'hypothesis_for_cause', 'severity', 'recommended_action', 'rationale', 'inputs_used']
ENUMS = {'severity': ['0', '1', '2', '3', '4']}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['inputs_used']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'owner', 'ui_surface', 'anti_pattern_name', 'evidence_link', 'hypothesis_for_cause', 'recommended_action', 'rationale']
...
def validate(obj):
    ...
    for k, allowed in ENUMS.items():
        if k in obj and str(obj[k]) not in allowed:
...
VALID_FIXTURE = json.loads('{"artefact_id": "anti-pattern-rationale-template-example", "owner": "@solo-founder", "version": "1.0.0", "last_reviewed": "2026-05-23", "ui_surface": "ui_surface value", "anti_pattern_name": "anti_pattern_name value", "evidence_link": "evidence_link value", "hypothesis_for_cause": "hypothesis_for_cause value", "severity": "0", "recommended_action": "recommended_action value", "rationale": "rationale value", "inputs_used": ["item-1", "item-2", "item-3"]}')
