<!-- purpose: 4-week employer-brand social/careers content calendar with per-post EVP mapping -->
<!-- consumes: named EVP pillars, employee spotlight material, open-role postings -->
<!-- produces: filled employer-brand content-calendar markdown -->
<!-- depends-on: none -->
<!-- token-budget-impact: ~490 tokens filled -->

# 4-Week Employer Brand Content Calendar

## Week 1: Employee Spotlight

**Channel:** LinkedIn
**EVP Pillar:** <evp_pillar>
**Hook:** (≤10 words to stop the scroll)
**Body:** (≤200 words, first-person employee voice, past-tense story)
**CTA:** (one action: "See open roles" / "Read more" / "Meet the team")
**Hashtags:** (3-5 max)
**Asset brief:** (headshot + quote card / short video / team photo)
**Status:** [ ] Draft [ ] Employee approved [ ] Consent on file [ ] Approved [ ] Scheduled

---

## Week 2: Behind-the-Scenes

**Channel:** Instagram / LinkedIn
**EVP Pillar:**
**Hook:**
**Body:** (≤90s video brief OR ≤150 words caption)
**CTA:**
**Hashtags:**
**Asset brief:** (video brief: location, people, action, music tone)
**Status:** [ ] Draft [ ] Approved [ ] Scheduled

---

## Week 3: Culture Moment

**Channel:** LinkedIn (long-form)
**EVP Pillar:**
**Hook:**
**Body:** (≤300 words, company voice, specific event or milestone)
**CTA:**
**Hashtags:**
**Asset brief:** (photo, infographic, or no asset)
**Status:** [ ] Draft [ ] Approved [ ] Scheduled

---

## Week 4: Open Role Promo

**Channel:** LinkedIn + Indeed + <others>
**EVP Pillar:** (role-specific — match to audience)
**Role:** <job_title_link>
**Hook:** ("What you'll build" not "We're hiring")
**Body:** (≤100 words: what they'll do, one EVP proof point, link)
**CTA:** "Apply now" + UTM-tagged URL
**Companion post:** "What we're building" context (separate post, same day)
**Status:** [ ] Draft [ ] Approved [ ] Scheduled

---

## UTM Tracking Convention

All careers page links: `?utm_source=<channel>&utm_medium=social&utm_campaign=employer-brand-<YYYY-MM>`

## Approval Requirements

- Any post with DEI claims: HRBP sign-off required
- Any post with employee name/photo: written consent on file
- Any Glassdoor response: brand manager sign-off before post
- No auto-publish: all posts queue in Buffer/Hootsuite for human final send
