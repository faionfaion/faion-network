<!-- purpose: Survey doc skeleton (hypothesis + questions + sample plan) -->
<!-- consumes: see content/02-output-contract.xml inputs -->
<!-- produces: artefact conforming to content/02-output-contract.xml -->
<!-- depends-on: content/01-core-rules.xml -->
<!-- token-budget-impact: ~200-1500 tokens when loaded as context -->
## Survey: <title>

### Research Objective
**Question:** [What we want to learn — one specific researchable question]
**Decision:** [What business decision this informs]
**Audience:** [Who will take it — specific segment, not "everyone"]
**Target N:** [Sample size needed — anchor to decision precision, not vanity]

### Screener Questions
1. <qualifying_question>
   - [Option A] → Continue
   - <option_b> → End survey (disqualify)

### Attention Check
At Q5 or midpoint: "Select 'Somewhat agree' to confirm you are reading carefully."
→ Drop respondents who fail; report drop rate in analysis.

### Core Questions

#### Q1: [Topic]
[Question text — specific, single-concept, neutrally worded]
- [ ] Option A
- [ ] Option B
- [ ] Option C
Type: <type>

#### Q2: [Topic]
...

### Demographics (place last)
- Role / title
- Company size
- Geography
- [Other segment variables matching user-personas.md]

### Open-Ended (1-2 maximum)
"Is there anything else you'd like to share about [topic]?"

### Distribution Plan
- **Channel:** <channel>
- **Duration:** [How long the survey stays open]
- **Incentive:** [If any — required for paid panels]

### Analysis Plan
- <metric_1>: Compare across <segment_variable>
- <metric_2>: Correlate with <factor>
- Segments defined before fielding: [list segments]
- Post-hoc slices flagged as exploratory: [list if any planned]
