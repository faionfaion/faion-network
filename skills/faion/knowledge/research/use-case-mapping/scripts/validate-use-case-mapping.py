#!/usr/bin/env python3
"""validate-use-case-mapping.py — validate a Use Case Mapping artefact JSON against the output contract.

Usage:
  validate-use-case-mapping.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('use_case_id', 'primary_actor', 'goal', 'preconditions', 'main_flow', 'alternative_flows', 'postcondition', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['primary_actor missing or empty', 'main_flow length < 1', 'alternative_flows length < 1', 'postcondition empty']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-use-case-mapping.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
