<!-- purpose: Live worksheet for a 7P-framework plus Paul-Graham-questions ideation session. -->
<!-- consumes: 7P prompts + PG four questions answered by the operator, per AGENTS.md Prerequisites -->
<!-- produces: scored idea worksheet with a ranked top-3 handed to niche-evaluation -->
<!-- depends-on: content/01-core-rules.xml (honest-passion-answer, pg-four-as-gate) -->
<!-- token-budget-impact: ~250-600 tokens when loaded as context -->

# Ideation Worksheet

## Session: <date>

## 7P Framework Ideas

### Pain Ideas
| # | Idea | Market (1-5) | Fit (1-5) | Urgency (1-5) | Total | Notes |
|---|------|-------------|-----------|---------------|-------|-------|
| 1 | | | | | | |
| 2 | | | | | | |
| 3 | | | | | | |

### Passion Ideas
| # | Idea | Market | Fit | Urgency | Total | Notes |
|---|------|--------|-----|---------|-------|-------|
| 1 | | | | | | |
| 2 | | | | | | |

### Profession Ideas
| # | Idea | Market | Fit | Urgency | Total | Notes |
|---|------|--------|-----|---------|-------|-------|
| 1 | | | | | | |
| 2 | | | | | | |

### Process, Platform, People, Product
_(repeat same table structure per P)_

---

## Paul Graham Questions

**1. What is tedious but necessary?**
- [stream of consciousness, 30 min, no editing]

**2. What is surprisingly hard to do?**
-

**3. What do you find yourself building for yourself?**
-

**4. What would you pay for that does not exist?**
-

---

## Top 3 to Validate

| Rank | Idea | Source (P or PG) | Total Score | Rationale |
|------|------|-----------------|-------------|-----------|
| 1 | | | /15 | |
| 2 | | | /15 | |
| 3 | | | /15 | |

→ Hand these 3 to niche-evaluation
