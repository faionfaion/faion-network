#!/usr/bin/env python3
"""validate-value-proposition-design.py — validate a Value Proposition Design artefact JSON against the output contract.

Usage:
  validate-value-proposition-design.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('customer_profile', 'value_map', 'alignment_gaps', 'value_prop_statement', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['any job without citation', 'any reliever without referenced pain id', 'value_prop_statement length > 140 chars', 'alignment_gaps missing when value_map < customer_profile coverage']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-value-proposition-design.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
