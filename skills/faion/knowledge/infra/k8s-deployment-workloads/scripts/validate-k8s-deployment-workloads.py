#!/usr/bin/env python3
"""validate-k8s-deployment-workloads.py

Validate the output artefact for the `k8s-deployment-workloads` methodology against the JSON
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/k8s-deployment-workloads.json', 'type': 'object', 'required': ['service', 'kind', 'rationale', 'manifest_path', 'owner', 'produced_at'], 'properties': {'service': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'}, 'kind': {'enum': ['Deployment', 'StatefulSet', 'DaemonSet', 'Job', 'CronJob']}, 'rationale': {'type': 'string', 'minLength': 20}, 'manifest_path': {'type': 'string'}, 'persistence_required': {'type': 'boolean'}, 'schedule': {'type': 'string'}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string'}, 'role': {'type': 'string'}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}}, 'additionalProperties': True}
...
OK = {'service': 'acme-api', 'kind': 'Deployment', 'rationale': 'Stateless HTTP API; horizontal scale + rolling-update; no stable identity required.', 'manifest_path': 'k8s/acme-api/deployment.yaml', 'persistence_required': False, 'owner': {'name': 'Tetiana Lytvyn', 'role': 'platform engineer'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'service': 'acme-redis', 'kind': 'Deployment'}
