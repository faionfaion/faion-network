...
**One-sentence:** Restore drill template: fixed-shape weekly backup-verification artefact with RTO targets per data class, evidence anchors, and named owner for outcome review.

**One-paragraph:** Restore Drill Template pins the discipline that turns restore drill from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in 02-output-contract drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`restore-drill-template` scope).
...
- Команд, де restore drill жив досі у головах SRE / DevOps, а не в репозиторії.
...
| File | Purpose |
|------|---------|
| `templates/restore-drill-template.md.j2` | Working skeleton for the `restore-drill-template` artefact with required fields and `not_applicable: <reason>` markers per row. |
| `templates/restore-drill-template.md` | Working skeleton for the `restore-drill-template` artefact with required fields and `not_applicable: <reason>` markers per row. Generated from `templates/restore-drill-template.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-restore-drill-template.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
