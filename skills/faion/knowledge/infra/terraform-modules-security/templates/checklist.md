<!--
purpose: Terraform Modules — Security - checklist skeleton matching `produces=checklist`
consumes: typed inputs from AGENTS.md Prerequisites table
produces: filled checklist artefact validated by scripts/validate-terraform-modules-security.py
depends-on: content/01-core-rules.xml, content/02-output-contract.xml
token-budget-impact: ~600 tokens for a typical instance
-->
# Terraform Modules — Security — {artefact_id}

| Field | Value |
|-------|-------|
| artefact_id | {artefact_id} |
| produces | checklist |
| owner | <owner> |
| version | <version> |
| last_reviewed | <last_reviewed> |
| status | draft \| active \| review \| deprecated |

## Inputs used

- <input_name> — {source-uri-or-path}

## Decision

{one-line decision the artefact records}

## Rationale

{>=40 chars; must cite at least one item from "Inputs used"}

## Trace refs

- {ticket / commit / transcript id}
