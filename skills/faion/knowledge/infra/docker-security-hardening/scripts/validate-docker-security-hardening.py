#!/usr/bin/env python3
"""validate-docker-security-hardening.py

Validate the artefact produced by the `docker-security-hardening` methodology against the
...
REQUIRED = ['user', 'capabilities', 'read_only_fs', 'secrets_policy', 'scanner', 'signing', 'owner', 'last_reviewed']
...
OK = json.loads('{"user": "app", "capabilities": {"drop": ["ALL"], "add": ["NET_BIND_SERVICE"]}, "read_only_fs": true, "secrets_policy": "vault", "scanner": {"tool": "trivy", "fail_on_severity": "HIGH"}, "signing": {"enabled": true, "tool": "cosign"}, "owner": "sec@acme.io", "last_reviewed": "2026-05-23"}')
BAD = json.loads('{"user": "root", "capabilities": {"drop": []}, "owner": "team"}')
