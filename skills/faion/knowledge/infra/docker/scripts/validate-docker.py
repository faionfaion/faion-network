...
Validate the artefact produced by the `docker` methodology against the
...
REQUIRED = ['base_image', 'stages', 'user', 'healthcheck', 'dockerignore_present', 'owner', 'last_reviewed']
...
OK = json.loads('{"base_image": {"name": "python:3.12-slim", "digest_pinned": true}, "stages": [{"name": "builder", "role": "build"}, {"name": "runtime", "role": "run"}], "user": "app", "healthcheck": {"cmd": "curl -fsS http://localhost:8080/healthz", "interval": "30s"}, "dockerignore_present": true, "owner": "sre@acme.io", "last_reviewed": "2026-05-23"}')
BAD = json.loads('{"base_image": {"name": "ubuntu", "digest_pinned": false}, "user": "root", "owner": "team"}')
