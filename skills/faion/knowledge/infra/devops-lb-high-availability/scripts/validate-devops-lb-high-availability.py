#!/usr/bin/env python3
"""validate-devops-lb-high-availability.py

Validate the artefact produced by the `devops-lb-high-availability` methodology against the
...
REQUIRED = ['topology', 'cloud_lb_product', 'regions', 'failover_rto_s', 'game_day_cadence', 'owner', 'last_reviewed']
...
OK = json.loads('{"topology": "cloud_managed", "cloud_lb_product": "aws_alb", "regions": [{"region": "eu-central-1", "azs": ["a", "b", "c"]}], "failover_rto_s": 30, "game_day_cadence": "quarterly", "owner": "sre@acme.io", "last_reviewed": "2026-05-23"}')
BAD = json.loads('{"topology": "active_passive_vrrp", "owner": "team"}')
