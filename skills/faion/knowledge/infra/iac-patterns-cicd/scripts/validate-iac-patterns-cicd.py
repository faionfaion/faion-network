#!/usr/bin/env python3
"""validate-iac-patterns-cicd.py

Validate the output artefact for the `iac-patterns-cicd` methodology against the JSON
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/iac-patterns-cicd.json', 'type': 'object', 'required': ['repo', 'ci_provider', 'auth', 'stages', 'drift_detection', 'owner', 'produced_at'], 'properties': {'repo': {'type': 'string', 'minLength': 3}, 'ci_provider': {'enum': ['github-actions', 'gitlab-ci', 'buildkite', 'atlantis', 'spacelift']}, 'auth': {'enum': ['wif', 'oidc']}, 'stages': {'type': 'array', 'items': {'type': 'object', 'required': ['name', 'trigger', 'approvers'], 'properties': {'name': {'type': 'string'}, 'trigger': {'enum': ['pr', 'merge', 'manual', 'schedule']}, 'approvers': {'type': 'integer', 'minimum': 0}}}, 'minItems': 2}, 'drift_detection': {'type': 'object', 'required': ['enabled', 'schedule'], 'properties': {'enabled': {'type': 'boolean', 'const': True}, 'schedule': {'type': 'string'}}}, 'promotion_order': {'type': 'array', 'items': {'type': 'string'}}, 'owner': {'type': 'object', 'required': ['name', 'role'], 'properties': {'name': {'type': 'string'}, 'role': {'type': 'string'}}}, 'produced_at': {'type': 'string', 'format': 'date-time'}}, 'additionalProperties': True}
...
OK = {'repo': 'acme/infra-terraform', 'ci_provider': 'github-actions', 'auth': 'wif', 'stages': [{'name': 'plan', 'trigger': 'pr', 'approvers': 0}, {'name': 'apply-dev', 'trigger': 'merge', 'approvers': 1}, {'name': 'apply-prod', 'trigger': 'manual', 'approvers': 2}], 'drift_detection': {'enabled': True, 'schedule': '0 3 * * *'}, 'promotion_order': ['dev', 'staging', 'prod'], 'owner': {'name': 'Olha Petrenko', 'role': 'platform lead'}, 'produced_at': '2026-05-23T10:00:00Z'}
BAD = {'repo': 'acme/infra', 'auth': 'static'}
