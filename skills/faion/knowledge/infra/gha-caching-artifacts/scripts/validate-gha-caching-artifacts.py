#!/usr/bin/env python3
"""validate-gha-caching-artifacts.py

Validate the artefact produced by the `gha-caching-artifacts` methodology against the
...
REQUIRED = ['caches', 'artifacts', 'docker_cache', 'owner', 'last_reviewed']
...
OK = json.loads("{\"caches\": [{\"path\": \"~/.npm\", \"key\": \"${{ runner.os }}-node-${{ hashFiles('**/package-lock.json') }}\", \"restore_keys\": [\"${{ runner.os }}-node-\"]}], \"artifacts\": [{\"name\": \"build-output\", \"path\": \"dist/\", \"retention_days\": 7}], \"docker_cache\": {\"enabled\": true, \"cache_from\": \"type=gha\", \"cache_to\": \"type=gha,mode=max\"}, \"owner\": \"ci-lead@team.io\", \"last_reviewed\": \"2026-05-23\"}")
BAD = json.loads("{\"caches\": null, \"owner\": \"team\"}")
