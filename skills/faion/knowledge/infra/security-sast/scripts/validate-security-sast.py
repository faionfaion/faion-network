#!/usr/bin/env python3
"""validate-security-sast.py

Validate the config artefact for the security-sast methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/security-sast.json", "type": "object", "required": ["tool", "tool_version", "severity_block", "baseline_strategy", "sarif_uploaded", "pr_gate"], "properties": {"tool": {"enum": ["semgrep", "codeql", "snyk-code", "sonarqube", "bandit", "gosec", "brakeman"]}, "tool_version": {"type": "string", "minLength": 1, "not": {"enum": ["master", "latest", "main"]}}, "severity_block": {"type": "array", "items": {"type": "string"}, "minItems": 1}, "severity_inform": {"type": "array", "items": {"type": "string"}}, "baseline_strategy": {"enum": ["ignore-existing", "fail-on-existing", "none"]}, "sarif_uploaded": {"type": "boolean", "const": true}, "pr_gate": {"type": "boolean", "const": true}}}')
REQUIRED = ['tool', 'tool_version', 'severity_block', 'baseline_strategy', 'sarif_uploaded', 'pr_gate']
...
OK = json.loads('{"tool": "semgrep", "tool_version": "1.65.0", "severity_block": ["ERROR"], "severity_inform": ["WARNING", "INFO"], "baseline_strategy": "ignore-existing", "sarif_uploaded": true, "pr_gate": true}')
BAD = json.loads('{"tool": "semgrep", "tool_version": "latest", "severity_block": ["ERROR", "WARNING", "INFO"], "baseline_strategy": "none", "sarif_uploaded": false, "pr_gate": false}')
