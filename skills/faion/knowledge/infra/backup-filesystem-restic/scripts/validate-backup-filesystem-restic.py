#!/usr/bin/env python3
"""validate-backup-filesystem-restic.py

Validate the artefact for the backup-filesystem-restic methodology against the JSON Schema in
...
REQUIRED = ['backend', 'password_source', 'retention', 'excludes', 'check_schedule', 'scheduler']

ENUMS = { 'backend': ['s3', 'b2', 'sftp', 'rest', 'local'], 'password_source': ['vault', 'aws-secrets-manager', 'gcp-secret-manager', 'password-file-0600'], 'check_schedule': ['daily', 'weekly'] }
CONSTS = { 'scheduler': 'systemd-timer' }
INT_BOUNDS = {  }
ARRAY_MIN = { 'excludes': 4 }
OBJECT_REQUIRED = { 'retention': ['keep_daily', 'keep_weekly', 'keep_monthly'] }
...
OK = {'backend': 's3', 'password_source': 'vault', 'retention': {'keep_daily': 1, 'keep_weekly': 1, 'keep_monthly': 1}, 'excludes': ['x', 'x', 'x', 'x'], 'check_schedule': 'daily', 'scheduler': 'systemd-timer'}
BAD = {'backend': None}
