#!/usr/bin/env python3
"""validate-image-digest-pinning-policy.py

Validate a `config` artefact for the image-digest-pinning-policy methodology against the
...
REQUIRED = ['slug', 'version', 'settings']
...
def validate(obj):
    ...
    if not isinstance(s, dict) or len(s) < 3:
        errs.append("settings must be an object with >=3 keys")
    return errs

OK = {'slug': 'image-digest-pinning-policy', 'version': '1.0.0', 'settings': {'key1': 'value1', 'key2': 'value2', 'key3': 'value3'}, 'applied_to': ['prod'], 'notes': 'Generated by image-digest-pinning-policy methodology.'}
BAD = {'slug': 'image-digest-pinning-policy'}
