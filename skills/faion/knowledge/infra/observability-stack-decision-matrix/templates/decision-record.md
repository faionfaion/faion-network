<!--
purpose: Observability Stack Decision Matrix - decision-record skeleton matching `produces=decision-record`
consumes: typed inputs from AGENTS.md Prerequisites table
produces: filled decision-record artefact validated by scripts/validate-observability-stack-decision-matrix.py
depends-on: content/01-core-rules.xml, content/02-output-contract.xml
token-budget-impact: ~600 tokens for a typical instance
-->
# Observability Stack Decision Matrix — <artefact_id>

| Field | Value |
|-------|-------|
| artefact_id | <artefact_id> |
| produces | decision-record |
| owner | <owner> |
| version | <version> |
| last_reviewed | <last_reviewed> |
| status | draft \| active \| review \| deprecated |

## Inputs used

- <input_name> — {source-uri-or-path}

## Decision

{one-line decision the artefact records}

## Rationale

{>=40 chars; must cite at least one item from "Inputs used"}

## Trace refs

- {ticket / commit / transcript id}
