#!/usr/bin/env python3
"""validate-backup-strategies.py

Validate the spec artefact for the backup-strategies methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/backup-strategies.json", "type": "object", "required": ["workload", "rpo_hours", "rto_hours", "retention_days", "copies", "restore_verification"], "properties": {"workload": {"type": "string", "minLength": 1}, "rpo_hours": {"type": "number", "minimum": 0}, "rto_hours": {"type": "number", "minimum": 0}, "retention_days": {"type": "integer", "minimum": 1}, "copies": {"type": "array", "minItems": 3, "items": {"type": "object", "required": ["media", "offsite", "immutable"], "properties": {"media": {"type": "string", "minLength": 1}, "offsite": {"type": "boolean"}, "immutable": {"type": "boolean"}, "lock_mode": {"enum": ["COMPLIANCE", "GOVERNANCE", "azure-immutable", "worm-tape"]}}}}, "restore_verification": {"type": "object", "required": ["cadence", "method", "last_pass"], "properties": {"cadence": {"enum": ["nightly", "weekly", "monthly"]}, "method": {"type": "string", "minLength": 1}, "last_pass": {"type": "string", "pattern": "^\\\\d{4}-\\\\d{2}-\\\\d{2}$"}}}}}')
REQUIRED = ['workload', 'rpo_hours', 'rto_hours', 'retention_days', 'copies', 'restore_verification']
...
OK = json.loads('{"workload": "checkout-db", "rpo_hours": 1, "rto_hours": 4, "retention_days": 90, "copies": [{"media": "local-nas", "offsite": false, "immutable": false}, {"media": "s3-standard-ia", "offsite": true, "immutable": false}, {"media": "s3-object-lock-compliance", "offsite": true, "immutable": true, "lock_mode": "COMPLIANCE"}], "restore_verification": {"cadence": "nightly", "method": "pg_restore-to-isolated-db", "last_pass": "2026-05-22"}}')
BAD = json.loads('{"workload": "checkout-db", "rpo_hours": 1, "rto_hours": 4, "retention_days": 90, "copies": [{"media": "local-nas", "offsite": false, "immutable": false}, {"media": "s3-glacier", "offsite": true, "immutable": true, "lock_mode": "GOVERNANCE"}], "restore_verification": {"cadence": "monthly", "method": "manual-spot-check", "last_pass": "2025-12-01"}}')
