<!--
purpose: Multi-Machine Outsource Dev Env Bootstrap - spec skeleton matching `produces=spec`
consumes: typed inputs from AGENTS.md Prerequisites table
produces: filled spec artefact validated by scripts/validate-multi-machine-outsource-dev-env-bootstrap.py
depends-on: content/01-core-rules.xml, content/02-output-contract.xml
token-budget-impact: ~600 tokens for a typical instance
-->
# Multi-Machine Outsource Dev Env Bootstrap — <artefact_id>

| Field | Value |
|-------|-------|
| artefact_id | <artefact_id> |
| produces | spec |
| owner | <owner> |
| version | <version> |
| last_reviewed | <last_reviewed> |
| status | draft \| active \| review \| deprecated |

## Inputs used

- <input_name> — {source-uri-or-path}

## Decision

{one-line decision the artefact records}

## Rationale

{>=40 chars; must cite at least one item from "Inputs used"}

## Trace refs

- {ticket / commit / transcript id}
