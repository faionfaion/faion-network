#!/usr/bin/env python3
"""validate-platform-engineering.py

Validate the spec artefact for the platform-engineering methodology against the
...
SCHEMA = json.loads('{"$schema": "http://json-schema.org/draft-07/schema#", "$id": "https://faion.net/schemas/platform-engineering.json", "type": "object", "required": ["org", "current_state", "decisions", "metrics", "non_goals_year1"], "properties": {"org": {"type": "string", "minLength": 1}, "current_state": {"type": "object", "required": ["devops_tickets_month", "ttfd_days", "service_count"], "properties": {"devops_tickets_month": {"type": "integer", "minimum": 0}, "ttfd_days": {"type": "number", "minimum": 0}, "dev_nps": {"type": "integer"}, "service_count": {"type": "integer", "minimum": 0}}}, "decisions": {"type": "object", "required": ["portal", "orchestrator", "iac", "golden_path_first", "rbac_includes_ai_agents"], "properties": {"portal": {"enum": ["backstage", "port", "cortex", "none-yet"]}, "orchestrator": {"enum": ["crossplane", "humanitec", "kratix", "homegrown"]}, "iac": {"enum": ["terraform", "pulumi", "both"]}, "golden_path_first": {"type": "string", "minLength": 1}, "rbac_includes_ai_agents": {"type": "boolean"}}}, "metrics": {"type": "object", "required": ["dora", "platform"], "properties": {"dora": {"type": "array", "items": {"type": "string"}, "minItems": 4}, "platform": {"type": "array", "items": {"type": "string"}, "minItems": 3}}}, "non_goals_year1": {"type": "array", "items": {"type": "string"}, "minItems": 1}}}')
REQUIRED = ['org', 'current_state', 'decisions', 'metrics', 'non_goals_year1']
...
OK = json.loads('{"org": "acme", "current_state": {"devops_tickets_month": 220, "ttfd_days": 14, "dev_nps": -5, "service_count": 38}, "decisions": {"portal": "backstage", "orchestrator": "crossplane", "iac": "terraform", "golden_path_first": "go-microservice", "rbac_includes_ai_agents": true}, "metrics": {"dora": ["DF", "LT", "MTTR", "CFR"], "platform": ["adoption_rate", "ttfd_new_dev", "ticket_trend", "nps"]}, "non_goals_year1": ["multi-cloud", "self-service databases"]}')
BAD = json.loads('{"org": "acme", "current_state": {"devops_tickets_month": 220, "ttfd_days": 14, "service_count": 38}, "decisions": {"portal": "backstage", "orchestrator": "crossplane", "iac": "terraform", "golden_path_first": "go-microservice", "rbac_includes_ai_agents": false}, "metrics": {"dora": ["DF"], "platform": []}, "non_goals_year1": []}')
