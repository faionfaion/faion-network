...
**One-sentence:** SLO design from user journeys: anchors SLIs and SLO targets on observable user journeys (not infra metrics) before the first production deploy so error budget tracks real harm.

**One-paragraph:** SLO Design From User Journeys pins the discipline that turns SLO design from tribal knowledge into a reviewable, owned, version-controlled operating artefact. The methodology constrains input shape, output shape, evidence anchors, and named ownership; the JSON Schema in 02-output-contract drives a stdlib validator at commit time. Outputs of the wrong shape are rejected at review; outputs without evidence are demoted to hypotheses; outputs without a named owner are tagged stale.

## Applies If (ALL must hold)

- The team operates the system the methodology targets (`slo-design-from-user-journeys` scope).
...
- Команд, де SLO design жив досі у головах SRE / DevOps, а не в репозиторії.
...
| File | Purpose |
|------|---------|
| `templates/slo-design-from-user-journeys.md.j2` | Working skeleton for the `slo-design-from-user-journeys` artefact with required fields and `not_applicable: <reason>` markers per row. |
| `templates/slo-design-from-user-journeys.md` | Working skeleton for the `slo-design-from-user-journeys` artefact with required fields and `not_applicable: <reason>` markers per row. Generated from `templates/slo-design-from-user-journeys.md.j2` by `tpl-jinja --migrate`; do not hand-edit. |
...
|------|---------|--------------|
| `scripts/validate-slo-design-from-user-journeys.py` | Validate artefact against the JSON Schema in `content/02-output-contract.xml`. Stdlib-only; supports `--help` and `--self-test`. | CI on artefact change; pre-commit. |
