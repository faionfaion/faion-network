#!/usr/bin/env python3
"""validate-key-trends-summary.py — validate a Key Trends Summary 2025-2026 artefact JSON against the output contract.

Usage:
  validate-key-trends-summary.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('report_id', 'trends', 'loaded_at', 'next_refresh_due', 'audience', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['trends length != 6', 'any trend without source OR implication', 'next_refresh_due > loaded_at + 90 days']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-key-trends-summary.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
