#!/usr/bin/env python3
"""validate-backlog-grooming-roadmapping.py — validate a Backlog Grooming and Roadmapping artefact JSON against the output contract.

Usage:
  validate-backlog-grooming-roadmapping.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('backlog_items', 'p0_items', 'scoring_framework', 'now_items', 'next_items', 'later_items', 'last_groomed_at', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['p0_items length > 3', 'any backlog item without rationale', 'AI-generated score without [estimate] flag', 'any item with explicit date instead of Now/Next/Later horizon']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-backlog-grooming-roadmapping.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
