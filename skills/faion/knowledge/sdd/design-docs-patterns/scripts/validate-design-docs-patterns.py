#!/usr/bin/env python3
"""validate-design-docs-patterns.py — validate a Design Docs Patterns artefact JSON against the output contract.

Usage:
  validate-design-docs-patterns.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('doc_id', 'title', 'scope', 'format', 'sections', 'non_goals', 'alternatives', 'review_deadline', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['sections missing any of {context, goals, non-goals, proposed, alternatives, open-questions}', 'alternatives length < 2', 'non_goals empty', 'review_deadline missing']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-design-docs-patterns.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
