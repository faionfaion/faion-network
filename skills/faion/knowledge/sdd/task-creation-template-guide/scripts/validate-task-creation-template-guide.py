#!/usr/bin/env python3
"""validate-task-creation-template-guide.py — validate a task-creation-template-guide output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'task_id', 'fill_steps_completed', 'rubric_score', 'sections_present', 'component_in_map', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = ['rubric_score']
NUM_FIELDS = []
ARRAY_FIELDS = ['fill_steps_completed', 'sections_present']
OBJECT_FIELDS = []
BOOL_FIELDS = ['component_in_map']
STRING_FIELDS = ['artefact_id', 'task_id', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "task-creation-template-guide-example", "version": "1.0.0", "last_reviewed": "2026-05-23", "task_id": "task_id value", "fill_steps_completed": ["item-1", "item-2", "item-3"], "rubric_score": 1, "sections_present": ["item-1", "item-2", "item-3"], "component_in_map": true, "owner": "@solo-founder"}')
