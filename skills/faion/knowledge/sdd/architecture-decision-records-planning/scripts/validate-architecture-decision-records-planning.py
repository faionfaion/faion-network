#!/usr/bin/env python3
"""validate-architecture-decision-records.py — validate a architecture-decision-records output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'adr_number', 'title', 'status', 'context', 'decision', 'alternatives', 'consequences', 'owner']
ENUMS = {'status': ['Proposed', 'Accepted', 'Deprecated', 'Superseded']}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = ['adr_number']
NUM_FIELDS = []
ARRAY_FIELDS = ['alternatives', 'related_adrs']
OBJECT_FIELDS = ['consequences']
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'title', 'context', 'decision', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "architecture-decision-records-example", "version": "1.0.0", "last_reviewed": "2026-05-23", "adr_number": 1, "title": "title value", "status": "Proposed", "context": "context value", "decision": "decision value", "alternatives": ["item-1", "item-2", "item-3"], "consequences": {"key": "value"}, "related_adrs": ["item-1", "item-2", "item-3"], "owner": "@solo-founder"}')
