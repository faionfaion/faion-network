#!/usr/bin/env python3
"""validate-design-docs-big-tech.py — validate a Design Docs at Big Tech Companies artefact JSON against the output contract.

Usage:
  validate-design-docs-big-tech.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('doc_format', 'scope', 'audience', 'page_budget', 'review_deadline', 'alternatives', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ['doc_format not in allowed set', "alternatives missing 'do nothing'", 'page_budget > 10 OR < 1', 'review_deadline missing']
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-design-docs-big-tech.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
