#!/usr/bin/env python3
"""validate-api-gateway-observability.py — stdlib-only validator for the api-gateway-observability output artefact.
...
REQUIRED = ['artefact_id', 'owner', 'version', 'last_reviewed', 'gateway', 'metrics', 'logging', 'tracing']
...
OK_JSON = '{"artefact_id": "api-gateway-observability-acme-2026-05-23", "owner": "Ruslan Faion <ruslan@faion.net>", "version": "1.0.0", "last_reviewed": "2026-05-23", "gateway": {"product": "kong", "version": "3.6"}, "metrics": {"prometheus": {"enabled": true, "listen": "0.0.0.0:9090"}}, "logging": {"format": "json", "fields": ["correlation_id", "consumer_id"]}, "tracing": {"otlp": {"endpoint": "http://tempo:4317", "propagator": "tracecontext"}}}'
BAD_JSON = '{"owner": "team", "metrics": {}, "logging": {"format": "plain"}}'
