#!/usr/bin/env python3
"""validate-security-by-design-feature-checklist.py

Validate the output artefact for the `security-by-design-feature-checklist` methodology against the JSON Schema
...
SCHEMA = {   '$schema': 'http://json-schema.org/draft-07/schema#',
    '$id': 'https://faion.net/schemas/security-by-design-feature-checklist.json',
    'type': 'object',
    'required': [   'slug',
                    'title',
                    'owner',
    ...
    'properties': {   'slug': {'type': 'string', 'pattern': '^[a-z][a-z0-9-]+$'},
                      ...
                      'items': {   'type': 'array',
                                   'minItems': 1,
                                   'items': {   'type': 'object',
                                                'required': ['id', 'prompt', 'decision'],
                                                'properties': {   'id': {'type': 'string'},
                                                                  'prompt': {'type': 'string'},
                                                                  'decision': {   'enum': [   'mitigated',
                                                                                              'accepted',
                                                                                              'deferred',
                                                                                              'not-applicable']},
...
OK = {   'slug': 'security-by-design-feature-checklist',
    'title': 'Security by Design Feature Checklist',
    ...
    'items': [   {   'id': 'q1',
                     'prompt': 'Does the design touch identity?',
                     'decision': 'mitigated',
                     'rationale': 'OIDC via Auth0 covers'}]}
BAD = {"slug": "BAD_security-by-design-feature-checklist", "owner": "someone"}
