...
Validate the `config` artefact for the `communications-management` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/communications-management.json', 'title': 'Communications Management', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'config'], 'properties': {'slug': {'type': 'string', 'const': 'communications-management'}, 'config': {'type': 'object', 'minProperties': 1}, 'format': {'enum': ['yaml', 'toml', 'json', 'markdown']}}}

OK_FIXTURE = {'slug': 'communications-management', 'config': {'key': 'value'}, 'format': 'yaml'}

BAD_FIXTURE = {'slug': 'communications-management'}
