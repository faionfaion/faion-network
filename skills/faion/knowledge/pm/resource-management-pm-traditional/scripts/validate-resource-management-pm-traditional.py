...
Validate the `config` artefact for the `resource-management` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/resource-management.json', 'title': 'Resource Management', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'config'], 'properties': {'slug': {'type': 'string', 'const': 'resource-management'}, 'config': {'type': 'object', 'minProperties': 1}, 'format': {'enum': ['yaml', 'toml', 'json', 'markdown']}}}

OK_FIXTURE = {'slug': 'resource-management', 'config': {'key': 'value'}, 'format': 'yaml'}

BAD_FIXTURE = {'slug': 'resource-management'}
