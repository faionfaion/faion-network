<!-- purpose: Closure checklist template: acceptance, resources, contracts, lessons, archive, handover -->
<!-- consumes: see content/02-output-contract.xml inputs -->
<!-- produces: artefact conforming to content/02-output-contract.xml -->
<!-- depends-on: content/01-core-rules.xml -->
<!-- token-budget-impact: ~200-1000 tokens when loaded as context -->

# Project Closure Checklist: <project_name>

**Project Manager:** <name>
**Close Date:** <date>

## Deliverable Acceptance
- [ ] All deliverables complete
- [ ] Acceptance sign-off obtained in writing
- [ ] Outstanding issues resolved or documented

## Administrative Closure
- [ ] Final costs recorded
- [ ] Invoices processed
- [ ] Budget closed
- [ ] Purchase orders closed
- [ ] Contracts terminated

## Resource Release
- [ ] Team members released
- [ ] Equipment returned
- [ ] Access permissions revoked
- [ ] Resource manager notified

## Knowledge Management
- [ ] Lessons learned session conducted (before team disperses)
- [ ] Final report written
- [ ] Documents archived with manifest
- [ ] Knowledge base updated

## Transition
- [ ] Operations handover complete (runbook reviewed day -7)
- [ ] Support documentation provided
- [ ] Monitoring transferred
- [ ] Escalation contacts defined

## Communication
- [ ] Stakeholders notified of closure
- [ ] Final status report distributed
- [ ] Team recognition done
- [ ] Project declared closed
