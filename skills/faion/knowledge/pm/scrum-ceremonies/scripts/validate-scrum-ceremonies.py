#!/usr/bin/env python3
"""validate-scrum-ceremonies.py

Validate the `playbook-step` artefact for the `scrum-ceremonies` methodology against the
...
SCHEMA = {'$schema': 'http://json-schema.org/draft-07/schema#', '$id': 'https://faion.net/schemas/scrum-ceremonies.json', 'title': 'Scrum Ceremonies', 'type': 'object', 'additionalProperties': False, 'required': ['slug', 'steps'], 'properties': {'slug': {'type': 'string', 'const': 'scrum-ceremonies'}, 'steps': {'type': 'array', 'minItems': 1, 'items': {'type': 'object', 'required': ['n', 'name', 'action'], 'properties': {'n': {'type': 'integer', 'minimum': 1}, 'name': {'type': 'string'}, 'action': {'type': 'string'}, 'decision_gate': {'type': 'string'}}}}}}

OK_FIXTURE = {'slug': 'scrum-ceremonies', 'steps': [{'n': 1, 'name': 'prepare', 'action': 'Collect inputs', 'decision_gate': 'All inputs present'}]}

BAD_FIXTURE = {'slug': 'scrum-ceremonies', 'steps': []}
