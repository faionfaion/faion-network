#!/usr/bin/env python3
"""validate-risk-threshold-policy.py

Validate the config artefact for the risk-threshold-policy methodology against the schema
...
VERSION = "1.1.0"
PRODUCES = "config"
RULES = ['explicit-severity-bands', 'named-target-per-band', 'sla-per-band', 'de-escalation-criteria', 'co-signed-policy']
...
OK = {"__faion_header__": {"methodology": "risk-threshold-policy", "version": "1.1.0", "produces": "config", "rules_satisfied": ["explicit-severity-bands", "named-target-per-band", "sla-per-band"]}, "config_path": "config/risk-threshold-policy.yaml", "entries": [{"key": "k1", "value": "v1"}]}
BAD = {"artefact_path": "docs/risk-threshold-policy.md"}
