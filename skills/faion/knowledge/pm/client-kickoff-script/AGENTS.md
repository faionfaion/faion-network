...
**One-sentence:** Generates a kickoff-meeting script — agenda, decisions required, RACI, comms cadence, escalation path, success metrics, signed by both sides.

**One-paragraph:** Client Kickoff Script addresses the gap identified by the `role-project-manager/Project initiation to first delivery (kickoff to week 4)` playbook: Kickoffs that drift end in late-week-4 rescoping. A signed script pins the operating contract on day one. Mechanism: a typed input → bounded transformation → contract-checked output. Primary output: a versioned `spec` artefact carrying a named accountable owner, input citations, and a review date — downstream agents and human reviewers consume it without re-deriving the rationale.
...
- Task is an instance of `role-project-manager/Project initiation to first delivery (kickoff to week 4)` OR a closely-adjacent variant in the same engagement shape.
...
|----------|--------|--------|
| Recent context for the `role-project-manager/Project initiation to first delivery (kickoff to week 4)` task (last 30 days) | Markdown / chat log | engagement notes |
...
|------|---------|--------------|
| `scripts/validate-client-kickoff-script.py` | Enforce the Client Kickoff Script output contract against the JSON Schema. | After subagent returns, before downstream consumer reads. |
...
### `templates/client-kickoff-script.json`

```json
{
  "$schema": "http://json-schema.org/draft-07/schema#",
  "$id": "https://faion.net/schemas/pm/client-kickoff-script.json",
  "title": "Client Kickoff Script",
  "type": "object",
  "required": [
    "artefact_id",
    "owner",
...
### `templates/client-kickoff-script.example.json`

```json
{
  "artefact_id": "client-kickoff-script-2026-05-23-acme",
