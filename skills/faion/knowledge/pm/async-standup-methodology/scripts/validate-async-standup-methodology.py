#!/usr/bin/env python3
"""validate-async-standup-methodology.py — validate a async-standup-methodology output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'channel', 'post_window_local', 'participants', 'blocker_policy', 'weekly_rollup_day', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed']
INT_FIELDS = []
NUM_FIELDS = []
ARRAY_FIELDS = ['participants']
OBJECT_FIELDS = ['blocker_policy']
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'channel', 'post_window_local', 'weekly_rollup_day', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "async-standup-2026-q2", "version": "1.0.0", "last_reviewed": "2026-05-23", "channel": "#daily-standup", "post_window_local": "by 11:00 local", "participants": ["@ruslan", "@contractor-1", "@contractor-2"], "blocker_policy": {"escalation_owner": "@ruslan", "eta_hours": 24}, "weekly_rollup_day": "Friday", "owner": "@ruslan"}')
INVALID_FIXTURE = json.loads('{"channel": "#daily-standup", "participants": ["@ruslan"], "blocker_policy": {}, "owner": "team"}')
