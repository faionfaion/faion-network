#!/usr/bin/env python3
"""validate-burndown-diagnosis-cheatsheet.py — validate a burndown-diagnosis-cheatsheet output JSON against the methodology schema.
...
REQUIRED = ['artefact_id', 'version', 'last_reviewed', 'sprint_id', 'diagnosis_date', 'drift_pct', 'diagnosed_causes', 'remediations', 'owner']
ENUMS = {}
SEMVER_FIELDS = ['version']
DATE_FIELDS = ['last_reviewed', 'diagnosis_date']
INT_FIELDS = []
NUM_FIELDS = ['drift_pct']
ARRAY_FIELDS = ['diagnosed_causes', 'remediations']
OBJECT_FIELDS = []
BOOL_FIELDS = []
STRING_FIELDS = ['artefact_id', 'sprint_id', 'owner']
...
VALID_FIXTURE = json.loads('{"artefact_id": "burndown-diag-sprint-2026-w20", "version": "1.0.0", "last_reviewed": "2026-05-23", "sprint_id": "2026-W20", "diagnosis_date": "2026-05-22", "drift_pct": 35, "diagnosed_causes": [{"cause": "scope-creep", "evidence": ["STORY-117 added day 3", "STORY-118 added day 4"]}, {"cause": "blocker-stack", "evidence": ["BLOCK-7 stale 4 days waiting on design"]}], "remediations": [{"action": "Move STORY-117/118 to next sprint", "owner": "@ruslan", "due": "2026-05-23"}, {"action": "Escalate BLOCK-7 to async designer; set 24h ETA", "owner": "@ruslan", "due": "2026-05-23"}], "owner": "@ruslan"}')
INVALID_FIXTURE = json.loads('{"sprint_id": "2026-W20", "diagnosed_causes": [{"cause": "we\'re slow"}], "remediations": [{"action": "try harder"}], "owner": "team"}')
