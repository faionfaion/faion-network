#!/usr/bin/env python3
"""validate-portfolio-triage-indie.py — validate a Portfolio Triage Indie artefact JSON against the output contract.

Usage:
  validate-portfolio-triage-indie.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('iteration_id', 'operator', 'rows', 'outcome_review', 'knowledge_space_url', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["any row missing evidence_link", "owner in collapsed plural set", "outcome_review empty when iteration_id > 1", "next_action == 'monitor'", "rows array empty"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-portfolio-triage-indie.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
