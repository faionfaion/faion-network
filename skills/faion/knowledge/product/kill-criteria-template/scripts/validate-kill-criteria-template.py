#!/usr/bin/env python3
"""validate-kill-criteria-template.py — validate a Kill Criteria Template artefact JSON against the output contract.

Usage:
  validate-kill-criteria-template.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('bet_name', 'launch_date', 'primary_metric', 'primary_metric_threshold', 'baseline_snapshot', 'review_dates', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["primary_metric_threshold non-numeric OR missing", "owner is collapsed plural (team / we / us)", "review_dates empty", "launch_date in past AND created_at > launch_date", "primary_metric is a multi-metric AND/OR expression"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-kill-criteria-template.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
