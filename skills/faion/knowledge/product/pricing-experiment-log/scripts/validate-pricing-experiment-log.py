#!/usr/bin/env python3
"""validate-pricing-experiment-log.py — validate a Pricing Experiment Log artefact JSON against the output contract.

Usage:
  validate-pricing-experiment-log.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('experiment_id', 'hypothesis', 'toggle', 'baseline', 'measurement_window', 'observed', 'decision', 'evidence_links', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["hypothesis missing numeric expectation", "window_locked_at after toggle_at", "decision not in {keep, revert, iterate}", "evidence_links empty", "baseline empty"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-pricing-experiment-log.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
