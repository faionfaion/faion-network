#!/usr/bin/env python3
"""validate-mvp-instrumentation-checklist.py — validate a MVP Instrumentation Checklist artefact JSON against the output contract.

Usage:
  validate-mvp-instrumentation-checklist.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('product_name', 'acquire', 'activate', 'retain', 'revenue', 'dashboard_url', 'launch_gated', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["any of {acquire, activate, retain, revenue}.event_name empty", "activate.event_name == acquire.event_name", "dashboard_url empty or localhost", "launch_gated != true", "owner is collapsed plural"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-mvp-instrumentation-checklist.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
