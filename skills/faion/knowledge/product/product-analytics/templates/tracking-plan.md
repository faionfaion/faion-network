<!--
purpose: Tracking plan — user properties, events (object-action, trigger, properties, owner) and naming conventions, written before instrumentation.
consumes: product surfaces inventory and the decisions the events must inform (see Prerequisites)
produces: a tracking plan
depends-on: content/01-core-rules.xml (r3-plan-before-track)
