#!/usr/bin/env python3
"""validate-maintain-mode-sops-solo.py — validate a Maintain Mode SOPs Solo artefact JSON against the output contract.

Usage:
  validate-maintain-mode-sops-solo.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('product_name', 'weekly_check', 'monthly_reconcile', 'quarterly_upgrade', 'on_call_rule', 'sunset_trigger', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["weekly_check duration > 30 min", "on_call_rule missing max_response_window_hours", "sunset_trigger empty", "owner is collapsed plural", "any cadence not calendar-locked"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'weekly_check': {'k': 'v'},
        'monthly_reconcile': {'k': 'v'},
        'quarterly_upgrade': {'k': 'v'},
        'on_call_rule': {'k': 'v'},
        'sunset_trigger': 'sample-sunset_trigger',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-maintain-mode-sops-solo.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
