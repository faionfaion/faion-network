#!/usr/bin/env python3
"""validate-multi-product-portfolio-management.py — validate a Multi Product Portfolio Management artefact JSON against the output contract.

Usage:
  validate-multi-product-portfolio-management.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('operator', 'products', 'cross_product_kill_rule', 'quarterly_review_at', 'total_weekly_time_cap', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["any product mode not in {build, grow, maintain, sunset}", "sum(time_budget_hours) > total_weekly_time_cap", "cross_product_kill_rule empty", "quarterly_review_at empty or non-ISO", "any capital_allocation_usd non-numeric"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'products': [{'k': 'v'}],
        'cross_product_kill_rule': 'sample-cross_product_kill_rule',
        'quarterly_review_at': 'sample-quarterly_review_at',
        'total_weekly_time_cap': 1,
        'version': '1.1.0',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-multi-product-portfolio-management.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
