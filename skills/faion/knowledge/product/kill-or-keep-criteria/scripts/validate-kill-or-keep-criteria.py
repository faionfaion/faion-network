#!/usr/bin/env python3
"""validate-kill-or-keep-criteria.py — validate a Kill Or Keep Criteria artefact JSON against the output contract.

Usage:
  validate-kill-or-keep-criteria.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('project_name', 'mrr_snapshot', 'traffic_snapshot', 'joy_score', 'opportunity_cost_candidate', 'evidence_links', 'verdict', 'next_action', 'template_version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["verdict not in {kill, keep}", "evidence_links missing for any floor", "opportunity_cost_candidate empty OR generic ('something else')", "joy_score outside 1-10 range", "next_action empty"]
...
def _self_test() -> int:
    fixture: dict[str, Any] = {
        ...
        'traffic_snapshot': 1,
        'joy_score': 1,
        'opportunity_cost_candidate': 'sample-opportunity_cost_candidate',
        'evidence_links': {'k': 'v'},
        'verdict': 'sample-verdict',
        'next_action': 'sample-next_action',
        'template_version': 'sample-template_version',
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-kill-or-keep-criteria.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
