#!/usr/bin/env python3
"""validate-okr-setting.py — validate a OKR Setting artefact JSON against the output contract.

Usage:
  validate-okr-setting.py <artefact.json>
...
REQUIRED: tuple[str, ...] = ('quarter', 'quarter_dates', 'objectives', 'biweekly_checkin', 'owner', 'version', 'last_reviewed')
FORBIDDEN_HINTS: tuple[str, ...] = ["objectives count > 3", "any objective with KRs count < 2 OR > 4", "any KR with non-numeric target", "biweekly_checkin empty", "any objective with empty owner OR owner in collapsed-plural set"]
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 2:
        sys.stderr.write(
            "usage: validate-okr-setting.py <artefact.json> [--self-test|--help]\n"
        )
        return 2
    try:
