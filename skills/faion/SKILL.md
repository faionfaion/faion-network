...
| "How to do X" — concrete task with steps | L1: `playbooks/taxonomy.xml` (11 goal categories) → L2: `playbooks/by-goal/<goal>/INDEX.xml` → leaf: `playbooks/<goal>/<slug>/AGENTS.md` + `content/01-playbook.xml` |
...
Beyond knowledge methodologies, the faion umbrella also hosts **455 playbooks** at `playbooks/<goal>/<slug>/` — `AGENTS.md` (stages, each with a decision gate) plus `content/01-playbook.xml`. A playbook is a standalone how-to for one task; the folder it sits in is the **goal character** of that task, not who runs it and not what it costs.

Tier is carried per playbook in its own `meta.json` and enforced through `skills/tier-manifest.json`, never by directory. Each playbook cites ≥1 methodology from `knowledge/<domain>/<slug>/`, at a tier ≤ its own.

Route L1 `playbooks/taxonomy.xml` → L2 `playbooks/by-goal/<goal>/INDEX.xml` → leaf. `by-goal/` holds the 11 generated indexes and no content; regenerate them with `python3 scripts/regen-playbook-index.py`.
