...
success_criteria:
  ...
  - Audit and fix intents run the project's existing validators before any investigation subagent is dispatched.
...
| `content/03-decision-tree.xml` | User intent to phase mapping (no-args, audit, brainstorm, fix, log, commit, skill, full cycle); validators-before-subagents routing rule. |
| `content/04-memory-files.xml` | The 4 memory files — purpose, format, append-vs-overwrite, identifier rules. |
| `content/05-anti-patterns.xml` | Apply without approval, single option, edit without read, restart without confirm, rung inflation, giant commit, skill from one-off task. |
