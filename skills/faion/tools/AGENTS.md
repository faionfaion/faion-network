...
| `template-builder/` | free | `tpl-build.py` — assembles blocks plus literal `.md` into Markdown and self-contained HTML, refusing a required parameter by name rather than substituting empty; `tpl-params.py` — lists declared parameters and emits the questions to ask, keeping a sensitive value out of the project store; `tpl-migrate.py` — proposes a `variables:` declaration for a legacy `<Angle>` template and names every placeholder it will not declare, writing nothing without `--write`; `tpl-jinja.py` — converts one Markdown template into the source `<name>.md.j2`, a self-contained `<name>.html.j2` generated from the Markdown structure, a draft-07 `<name>.vars.schema.json` and a regenerated `<name>.md`, refusing an already-HTML-escaped or section-gated source whole rather than guessing, with `--migrate` doing the whole per-template operation — files, AGENTS.md rows and inline body — atomically or not at all, and `--check` re-deriving every generated form to catch a hand edit; `tpl-render.py` — renders that pair sandboxed, autoescaped and `StrictUndefined`, never treating a value as template source |
