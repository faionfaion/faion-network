...
## Invoke
```
sh {script} --dir {project} [--venv {path}] [--requirements {file}] [--verify-import {mod,mod}] [--python {exe}] [--force] [--self-test]
```

## Inputs
- `--dir {project}` — directory the venv belongs to. Required unless self-testing.
...
- `--self-test` — drive the CLI over a temporary tree and exit. Touches no network: its only pip call is against an empty requirements file. Optional.
...
- Exit: `0` ready · `1` venv creation, pip install, or import verification failed, or a self-test check failed · `2` usage error, missing `--dir`, missing requirements file, missing interpreter.
