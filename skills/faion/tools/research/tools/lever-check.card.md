...
## Invoke
```
python3 {script} --ledger {levers.jsonl} --concept {verdict.json} [--report {decisions.md}]
```

## Inputs
- `--ledger {levers.jsonl}` — the commercial-lever ledger `source-table` wrote, one JSON object per line, each with an `id`. Required. Blank lines and `#` comments ignored.
...
- Exit: `0` every lever answered and well-formed · `1` at least one unanswered, undisposed, applied with nowhere to land, declined with no reason or no class, answered twice, or answering an id the ledger does not carry · `2` an input is missing, unreadable, not JSON, or carries a `commercial_findings` that is not an array.
