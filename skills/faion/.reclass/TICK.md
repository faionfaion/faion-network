...
Read `skills/faion/.reclass/candidates.ambiguous.tsv`. If missing or 0 lines:

1. If there are staged/unstaged changes under `skills/faion/knowledge/`, commit them: `refactor: reclass final batch`.
...
Read `skills/faion/.reclass/RULES.md` sections **Tier definitions** and **Decision procedure**.

## Step 3 — take next 40 paths

```
head -n 40 skills/faion/.reclass/candidates.ambiguous.tsv
```
...
Append to `skills/faion/.reclass/decisions.log` (one line per decision):
```
...
```bash
sed -i '1,40d' skills/faion/.reclass/candidates.ambiguous.tsv
```
