---
slug: faion-as-programmatic-context-source
tier: pro
...
maintainers:
  - faion
summary: Agent builder wires faion CLI as structured methodology lookup → no markdown parsing, emits faion-citation tokens.
content_id: 8f7f1c1e30906cfc
methodology_refs:
...
The wrapper is the entire moat. Without a strict-schema tool wrapper, the agent is parsing markdown, which means it is also hallucinating around markdown. Discriminated-union output with required fields lets the agent branch on result type, not on string heuristics.

Manifest-then-fetch is the access pattern. The agent first asks for a manifest (slug list, content_ids, tier, version), then fetches only the bodies it needs. This keeps the context budget tight even on agents that consult many methodologies per turn. Auto-evict tool results that are no longer referenced; old methodology bodies in context are a quality regression.

Progressive disclosure of skills means: the agent does not see the entire faion catalogue every turn. Surface only the tier-relevant subset for the current task. This is also a tier-enforcement boundary — the agent must not see content the user is not entitled to.

Citation tokens are the trust contract. Every grounded answer the agent produces should carry one or more `faion://{slug}@{version}#{content_id}` tokens that the user can resolve back to the source. Without this, the user has no way to audit the answer and the faion brand value goes unrecognized. Brainstorm flags methodology-as-json-feed, faion-cli agent adapter pattern, citation contract, and methodology versioning as gaps.

Eval the citation contract. A grounded answer with no citation is a bug.
