...
maintainers:
  - faion-network
summary: Security scan triage (weekly). Walk SAST/DAST/dep-scan output once a week; convert noise into a small, ranked, owner-tagged worklist.
...
Get an honest map of where things stand today for: Security scan triage (weekly).
...
Methodologies:
- `pro/_gaps/qa-security-triage-runbook` (gap)
...
- `qa-security-triage-runbook` — bare-slug reference from source; methodology not yet authored
