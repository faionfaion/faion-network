---
slug: traceability-matrix-weekly-refresh
tier: pro
...
maintainers:
  - faion-network
summary: Once a week the matrix reflects every shipped story, every open AC, and every stranded requirement. Stranded items get an owner and a date by end of session.
content_id: 34e5cbd69d674382
methodology_refs:
...
# Traceability matrix weekly refresh

**Persona:** role-ba - **Tier:** pro - **Complexity:** light - **Angle:** atomic

## Context

Once a week the matrix reflects every shipped story, every open AC, and every stranded requirement. Stranded items get an owner and a date by end of session.
...
- `jira-trace-export-script`
- `requirements-management`
- `traceability-matrix-template-csv`

BLOCK policy: this playbook cannot move to `published` until `gaps[]` is empty.

Run `faion get-content traceability-matrix-weekly-refresh --format context` to pull the full chain into an agent-readable payload.
