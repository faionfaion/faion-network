---
slug: jira-ticket-scoping-session
tier: pro
...
maintainers:
  - faion
summary: "30-90 min working session → turns vague 'we need this feature' into a ticket with INVEST user story, AC, dependencies, story-points placeholder, compliance/security owner tagged."
content_id: bd0af55cd0c26268
methodology_refs:
  ...
  - definition-of-ready-checklist-outsource
---
...
Scoping JIRA tickets live with the client PM is the highest-leverage 60 minutes an outsource senior spends in a sprint. Async ping-pong on the same ticket costs 3-5x more wall-clock time and produces worse acceptance criteria. Live drafting forces the conversation that async hides.
...
Framing fails when you start typing the user story before the client PM has explained the actual business need. Active-listening discipline pays back tenfold here — let them speak first, summarise, ask the dumb question, then write.
...
Live drafting on a shared screen is the magic. The client PM sees you write the AC; they push back in real time; you negotiate scope while the context is hot. Async tickets lose this calibration step and require a full second meeting to recover.
...
Tagging dependencies + DoR is the part most seniors skip in the interest of finishing the call. Skip it and the ticket gets reopened at sprint planning. Spend the last 5 minutes here and the dev who picks it up never pings you.
...
Run this 1-3 times per sprint depending on how much the client PM trusts you to write tickets unsupervised. Pair with the story-points poker playbook for sizing and the audit-grade code review playbook for compliance-relevant work.
