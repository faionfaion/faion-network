...
maintainers:
  - faion
summary: Stuck on prompt iteration → clear go/no-go on fine-tuning with LoRA SFT/DPO data prep, training, eval, or documented stay on prompt+retrieval.
...
Stage 2 is the actual playbook. Most teams that walk this path discover they should NOT fine-tune — the cost projection beats the quality lift only for narrow, repetitive tasks with abundant clean traces. If the decision rubric says 'stay on prompt+retrieval', commit that decision in writing and exit the playbook. Do not fine-tune for political reasons.

If you decide to fine-tune, the Data Prep stage is where most projects fail. Trace data is dirty: it contains the agent's own mistakes, user-side typos, and PII. SFT must be filtered to known-good completions; DPO needs paired examples with clear preference signal. Do not merge a tuned checkpoint trained on production traces without dataset-provenance review.

Shadow-deploy is mandatory. A tuned model that beats baseline on the eval set but loses in production is the failure mode. Stage 5 mandates `fine-tuning-openai-eval` and a shadow window of at least one release cycle. Promote on green, rollback on regression.

LoRA vs full SFT is mostly a budget decision. LoRA wins for cost and checkpoint portability; full SFT wins for harder behavioural shifts. The brainstorm lists an explicit gap on the tune-vs-prompt economic model — until that lands, run the projection by hand with cost-per-success telemetry from playbook 10.

Stay on prompt+retrieval is a valid GA decision. Log it.
