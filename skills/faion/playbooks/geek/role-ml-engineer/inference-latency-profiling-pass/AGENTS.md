---
slug: inference-latency-profiling-pass
tier: geek
...
maintainers:
  - faion-network
summary: "When p95 latency creeps up or a feature is borderline: profile the hot path end-to-end (retriever → rerank → model → tools) and ship one latency cut."
content_id: f08de784bbcd733a
methodology_refs:
...
# Inference latency profiling pass

A 7-stage playbook for the atomic angle. Tier: **geek**. Complexity: **medium**.

## Context

When p95 latency creeps up or a feature is borderline: profile the hot path end-to-end (retriever → rerank → model → tools) and ship one latency cut.
...
- `knowledge/geek/ai/ai-agents/rerank-before-reasoning`
- `knowledge/geek/ai/ai-agents/trajectory-eval-otel`
