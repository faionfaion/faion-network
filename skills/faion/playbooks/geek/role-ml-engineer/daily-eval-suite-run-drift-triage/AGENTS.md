...
maintainers:
  - faion-network
summary: Eval suite (golden + canary set) ran overnight; engineer reviews dashboards, files an incident if any metric breached threshold, otherwise green-flags the day. Output is a one- line eval verdict in...
...
# Daily eval-suite run + drift triage

A 5-stage playbook for the atomic angle. Tier: **geek**. Complexity: **light**.

## Context

Eval suite (golden + canary set) ran overnight; engineer reviews dashboards, files an incident if any metric breached threshold, otherwise green-flags the day. Output is a one-line eval verdict in the team channel.
...
- `knowledge/geek/ai/ml-engineer/llm-observability`
...
- `knowledge/geek/ai/ml-ops/evaluation-metrics`
