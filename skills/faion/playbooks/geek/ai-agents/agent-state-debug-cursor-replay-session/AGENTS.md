...
maintainers:
  - faion
summary: Agent ran 40 turns and lost the plot → reconstruct trajectory, find bad transition, ship a guardrail.
...
One focused session per incident, not an open-ended investigation. The deeper the agent's run, the more tempting it is to chase every possible branch. Resist; the goal is one bad transition, one guardrail.

Replay determinism is the precondition. If you cannot reproduce the bad run locally with record-replay, the failure is non-deterministic and you need to broaden the trace pull, not narrow the inspection. Most 'lost the plot' incidents reproduce; if yours does not, suspect the wall-clock-dependent tools first.

Inspection is layered. Start at the highest-level state (scratchpad, filesystem memory) and work down. Compaction is the silent killer: a reference the agent depended on was evicted three turns ago, and every downstream step is a hallucinated reconstruction. Auto-eviction history shows when references were dropped; preserve-refs configuration shows whether they should have been.

Categorization at Stage 3 forces a single attribution before any code change. Most teams skip this and fix three things at once; six weeks later the regression recurs in a slightly different shape because the real cause was the fourth thing nobody fixed. Pick one category, ship one guardrail, add one eval row. The brainstorm marks agent replay harness cookbook and context-bleed detection recipe as open gaps.
