---
slug: adr-draft-for-a-single-decision
tier: solo
...
maintainers:
  - faion-network
summary: "Done = one ADR (status proposed) committed to repo: context, options considered with trade-off matrix, decision, consequences, links to spec / design / quality-attribute analysis; ready for async r..."
content_id: 75a17ac29a4a8d5b
methodology_refs:
...
# ADR draft for a single decision

**Persona:** software architect · **Tier:** solo · **Complexity:** medium · **Angle:** atomic

## Context

Done = one ADR (status proposed) committed to repo: context, options considered with trade-off matrix, decision, consequences, links to spec / design / quality-attribute analysis; ready for async review by 2+ engineers.
...
- ADR draft for a single decision ships with written success criteria met and evidence attached
...
- `architecture-decision-records` — `faion/knowledge/solo/sdd/sdd/architecture-decision-records` (tier: solo)
...
- `adr-reversibility-tagging` (expected tier: solo)
- `adr-staleness-audit` (expected tier: solo)
