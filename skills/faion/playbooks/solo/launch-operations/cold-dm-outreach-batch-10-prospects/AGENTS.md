---
slug: cold-dm-outreach-batch-10-prospects
tier: solo
...
maintainers:
  - faion
summary: Empty inbox → 10 personalised cold DMs sent and logged with status by EOD.
content_id: a6664b4f448973c0
methodology_refs:
  - pain-point-research
...
# Cold DM outreach batch (10 prospects)

## Why this playbook exists
Indie hackers run the entire pipeline alone — research, build, ship, market, support, and decide — under tight calendar constraints and often from a nomad schedule. Empty inbox → 10 personalised cold DMs sent and logged with status by EOD. This playbook turns that ambition into a sequenced set of stages with explicit decision gates so the founder doesn't confuse motion for progress and doesn't keep paying for products that quietly stopped working.

## What changes after running it
One sitting per week: 10 personalised cold DMs sent on X or LinkedIn to ICP-fit accounts, all logged with status in a minimal CRM so the next batch can compound on prior outcomes.

## Stage flow
### Build List
10 ICP-fit accounts with a personal hook on each.

**Tasks**
- Pull from saved ICP source
- Note one personal hook per account (recent post, mutual interest)
- Confirm contact channel is open

**Decision gate**
> Advance only once each row has a personal hook.

### Draft
10 DMs that pass the mom-test.

**Tasks**
- Open with the personal hook, not your pitch
- State a clear ask within 3 lines
- Pass the 'would they reply?' bar before sending

**Decision gate**
> Advance only after a peer or fresh eye signs off.

### Send
All 10 DMs sent inside a focus block.

**Tasks**
- Send in batches of 5 across two channels max
- Log each in the mini-CRM with timestamp
- Trigger a 48-hour reminder for follow-up

**Decision gate**
> Done once all 10 are sent and logged.
