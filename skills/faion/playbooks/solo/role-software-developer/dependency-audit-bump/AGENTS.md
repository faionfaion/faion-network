...
maintainers:
  - faion-network
summary: All direct deps are at latest stable OR explicitly pinned with a reason. Known CVEs are patched or risk-accepted in writing. Build is green; e2e suite ran on bumped versions.
content_id: 928a958ce260ce41
methodology_refs:
...
All direct deps are at latest stable OR explicitly pinned with a reason. Known CVEs are patched or risk-accepted in writing. Build is green; e2e suite ran on bumped versions.
...
- Dependency audit + bump ships with written success criteria met and evidence attached
...
- `pnpm-package-management` — `faion/knowledge/free/dev/software-developer/pnpm-package-management` (tier: free)
- `python-poetry-setup` — `faion/knowledge/free/dev/software-developer/python-poetry-setup` (tier: free)
...
- `dep-bump-batching-strategy` (expected tier: solo)
- `cve-triage-decision-tree` (expected tier: solo)
