...
# Deploy-to-staging + smoke + sign-off
...
- `cd-basics` — `faion/knowledge/solo/dev/automation-tooling/cd-basics` (tier: solo)
...
- `feature-flags-rollout-targeting` — `faion/knowledge/solo/dev/automation-tooling/feature-flags-rollout-targeting` (tier: solo)
...
- `staging-smoke-checklist-by-surface` (expected tier: solo)
- `deploy-notes-template-with-rollback` (expected tier: solo)
