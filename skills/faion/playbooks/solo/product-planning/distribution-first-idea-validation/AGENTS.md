---
slug: distribution-first-idea-validation
tier: solo
...
maintainers:
  - faion
summary: Owned audience → next product chosen by audience signal plus paid pre-orders before code.
content_id: dd1a91339c12dc49
methodology_refs:
  - mom-test
...
Indie hackers run the entire pipeline alone — research, build, ship, market, support, and decide — under tight calendar constraints and often from a nomad schedule. Owned audience → next product chosen by audience signal plus paid pre-orders before code. This playbook turns that ambition into a sequenced set of stages with explicit decision gates so the founder doesn't confuse motion for progress and doesn't keep paying for products that quietly stopped working.

## What changes after running it
Pick the next product by mining an audience you already own (or are growing), not by guessing market need. Done = a signed waitlist plus paying pre-orders before line 1 of code, validated through audience-first conversations.

## Stage flow
### Mine
Audience tells you what they actually pay for.

**Tasks**
- Pull 30 days of DMs, replies, and comments
- Cluster pain points by frequency and intensity
- Mom-test the top 3 pains over DM

**Decision gate**
> Advance only once each pain has at least 3 unprompted mentions.

### Frame
Turn the strongest pain into a product hypothesis.

**Tasks**
- Write a 1-sentence product framing
- Define pricing range from prior monetisation data
- Sketch a micro-MVP scope

**Decision gate**
> Advance only if 3 audience members react positively to the framing.

### Pre-sell
Waitlist plus paid pre-orders before any code.

**Tasks**
- Ship a 1-page waitlist landing with explicit pre-order CTA
- Open Stripe pre-order at a real price
- Run a 7-day audience-only push

**Decision gate**
> Advance only with ≥10 pre-orders or kill the bet.
