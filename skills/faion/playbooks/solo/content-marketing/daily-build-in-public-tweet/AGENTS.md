...
maintainers:
  - faion
summary: Empty page → one published build-in-public tweet thread by EOD.
...
Indie hackers run the entire pipeline alone — research, build, ship, market, support, and decide — under tight calendar constraints and often from a nomad schedule. Empty page → one published build-in-public tweet thread by EOD. This playbook turns that ambition into a sequenced set of stages with explicit decision gates so the founder doesn't confuse motion for progress and doesn't keep paying for products that quietly stopped working.
...
## Stage flow
### Capture
Pick the day's most tweetable build moment.
...
### Draft
A tight tweet that opens with a hook and ends with a payoff.
...
### Ship
Post and engage with early replies.
