---
slug: regression-suite-hardening-flaky-test-elimination
tier: solo
...
maintainers:
  - faion-network
summary: "Regression suite hardening: flaky test elimination. Done when: CI flake rate < 1% sustained for 4 weeks, every quarantined test has a triage owner + ETA, no test is muted without a tracked root-cau..."
content_id: b40a3da65a577334
methodology_refs:
  - e2e-testing
  - mocking-strategies
  - test-fixtures
...
# Regression suite hardening: flaky test elimination

## Context

Done when: CI flake rate < 1% sustained for 4 weeks, every quarantined test has a triage owner + ETA, no test is muted without a tracked root-cause ticket, retry logic removed except for documented infrastructure cases, and a flake-budget policy is enforced in PR review.

## Outcome

By the time this playbook closes:

- Done-state achieved as described in the scope: Done when: CI flake rate < 1% sustained for 4 weeks, every quarantined test has a triage owner + ETA, no test is muted without a tracked root-cause ticket, retry logic removed except for documented infrastructure cases, and a flake-budget policy is enforced in PR review.
...
Get an honest map of where things stand today for: Regression suite hardening: flaky test elimination.
...
Methodologies:
- `geek/sdlc-ai/test-golden-master-legacy-rewrite`
...
- `flake-budget-pr-rule` — listed in gaps_for_this_playbook from source brainstorm
- `flaky-test-triage-playbook` — listed in gaps_for_this_playbook from source brainstorm
- `deterministic-test-data-pattern` — listed in gaps_for_this_playbook from source brainstorm
