---
slug: release-candidate-smoke-pack
tier: solo
...
maintainers:
  - faion-network
summary: "Release-candidate smoke pack. Pre-deploy gate: run a small, fast, high-signal smoke pack against the RC and produce a go/no-go in writing."
content_id: 97a7a6f64f960ae6
methodology_refs:
  - e2e-testing
  - integration-testing
  - testing-patterns
...
Pre-deploy gate: run a small, fast, high-signal smoke pack against the RC and produce a go/no-go in writing.

## Outcome

By the time this playbook closes:

- Done-state achieved as described in the scope: Pre-deploy gate: run a small, fast, high-signal smoke pack against the RC and produce a go/no-go in writing.
...
Get an honest map of where things stand today for: Release-candidate smoke pack.
...
Methodologies:
- `solo/_gaps/qa-rc-smoke-pack-template` (gap)
...
- `qa-rc-smoke-pack-template` — bare-slug reference from source; methodology not yet authored
- `qa-rollback-trigger-canon` — listed in gaps_for_this_playbook from source brainstorm
