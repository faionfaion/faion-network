---
slug: performance-test-run-analysis
tier: solo
...
maintainers:
  - faion-network
summary: Performance test run + analysis. Single load/perf run against a target SLO; produce a verdict (ship / hold / investigate) backed by numbers, not vibes.
content_id: dfe2dcfb77facea1
methodology_refs:
  - code-quality-trends
  - perf-test-basics
  - perf-test-tools
...
# Performance test run + analysis

## Context

Single load/perf run against a target SLO; produce a verdict (ship / hold / investigate) backed by numbers, not vibes.

## Outcome

By the time this playbook closes:

- Done-state achieved as described in the scope: Single load/perf run against a target SLO; produce a verdict (ship / hold / investigate) backed by numbers, not vibes.
...
Get an honest map of where things stand today for: Performance test run + analysis.
...
- `qa-perf-run-verdict-template` — listed in gaps_for_this_playbook from source brainstorm
