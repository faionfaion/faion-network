---
slug: weekly-bug-bash-facilitation-friday
tier: solo
...
maintainers:
  - faion-network
summary: Weekly bug-bash facilitation (Friday). Run a 90–120 minute cross-role bash on a single risk area; convert hour-of-attention into a ranked defect list.
content_id: b2321aa30c4ac455
methodology_refs:
  - code-quality-trends
  - code-review-process
  - qa-bug-bash-runbook
...
# Weekly bug-bash facilitation (Friday)

## Context

Run a 90–120 minute cross-role bash on a single risk area; convert hour-of-attention into a ranked defect list.

## Outcome

By the time this playbook closes:

- Done-state achieved as described in the scope: Run a 90–120 minute cross-role bash on a single risk area; convert hour-of-attention into a ranked defect list.
...
Get an honest map of where things stand today for: Weekly bug-bash facilitation (Friday).
...
Methodologies:
- `solo/_gaps/qa-bug-bash-runbook` (gap)
...
- `qa-bug-bash-runbook` — bare-slug reference from source; methodology not yet authored
