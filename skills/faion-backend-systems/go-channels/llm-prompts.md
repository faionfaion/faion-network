# LLM Prompts for Go Channels

Effective prompts for AI-assisted development with go channels.
...
```
I need to implement go channels for my project.
...
2. Technology choices from: Go runtime, Context package
...
```
Help me choose the best tool for go channels.
...
```
Generate go channels implementation for [language].

**Requirements:**
- Focus: Channel patterns, buffering, select, context
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing go channels code.
...
```
Analyze my go channels implementation for performance issues.
...
```
Refactor this go channels code for better maintainability.
...
```
I'm experiencing [issue] with go channels.
...
```
Review my go channels implementation for best practices.
...
```
Generate comprehensive tests for go channels.
...
```
What test scenarios should I cover for go channels?
...
```
Generate documentation for this go channels implementation.
...
```
Create step-by-step tutorial for implementing go channels.
...
```
Migrate go channels from [old tool] to [new tool].
...
```
1. "Design go channels for [project]"
   -> Get architecture
...
```
Generate go channels in [language]:
- Feature: [description]
- Tool: Go runtime
