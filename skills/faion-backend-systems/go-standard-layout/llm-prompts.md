# LLM Prompts for Go Standard Layout

Effective prompts for AI-assisted development with go standard layout.
...
```
I need to implement go standard layout for my project.
...
2. Technology choices from: Go modules, Make, Docker
...
```
Help me choose the best tool for go standard layout.
...
```
Generate go standard layout implementation for [language].

**Requirements:**
- Focus: Project layout conventions, best practices
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing go standard layout code.
...
```
Analyze my go standard layout implementation for performance issues.
...
```
Refactor this go standard layout code for better maintainability.
...
```
I'm experiencing [issue] with go standard layout.
...
```
Review my go standard layout implementation for best practices.
...
```
Generate comprehensive tests for go standard layout.
...
```
What test scenarios should I cover for go standard layout?
...
```
Generate documentation for this go standard layout implementation.
...
```
Create step-by-step tutorial for implementing go standard layout.
...
```
Migrate go standard layout from [old tool] to [new tool].
...
```
1. "Design go standard layout for [project]"
   -> Get architecture
...
```
Generate go standard layout in [language]:
- Feature: [description]
- Tool: Go modules
...
```
Review this go standard layout code:
[paste code]
...
```
Debug go standard layout issue:
Error: [message]
Code: [paste]
Context: [details]
