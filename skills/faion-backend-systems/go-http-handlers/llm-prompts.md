# LLM Prompts for Go HTTP Handlers

Effective prompts for AI-assisted development with go http handlers.
...
```
I need to implement go http handlers for my project.
...
2. Technology choices from: net/http, Gin, Echo, Chi
...
```
Help me choose the best tool for go http handlers.
...
```
Generate go http handlers implementation for [language].

**Requirements:**
- Focus: Handler patterns, middleware, routing
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing go http handlers code.
...
```
Analyze my go http handlers implementation for performance issues.
...
```
Refactor this go http handlers code for better maintainability.
...
```
I'm experiencing [issue] with go http handlers.
...
```
Review my go http handlers implementation for best practices.
...
```
Generate comprehensive tests for go http handlers.
...
```
What test scenarios should I cover for go http handlers?
...
```
Generate documentation for this go http handlers implementation.
...
```
Create step-by-step tutorial for implementing go http handlers.
...
```
Migrate go http handlers from [old tool] to [new tool].
...
```
1. "Design go http handlers for [project]"
   -> Get architecture
...
```
Generate go http handlers in [language]:
- Feature: [description]
- Tool: net/http
...
```
Review this go http handlers code:
[paste code]
