# LLM Prompts for Go Goroutines

Effective prompts for AI-assisted development with go goroutines.
...
```
I need to implement go goroutines for my project.
...
2. Technology choices from: Go runtime, pprof, Context
...
```
Help me choose the best tool for go goroutines.
...
```
Generate go goroutines implementation for [language].

**Requirements:**
- Focus: Goroutine lifecycle, leaks, patterns
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing go goroutines code.
...
```
Analyze my go goroutines implementation for performance issues.
...
```
Refactor this go goroutines code for better maintainability.
...
```
I'm experiencing [issue] with go goroutines.
...
```
Review my go goroutines implementation for best practices.
...
```
Generate comprehensive tests for go goroutines.
...
```
What test scenarios should I cover for go goroutines?
...
```
Generate documentation for this go goroutines implementation.
...
```
Create step-by-step tutorial for implementing go goroutines.
...
```
Migrate go goroutines from [old tool] to [new tool].
...
```
1. "Design go goroutines for [project]"
   -> Get architecture
...
```
Generate go goroutines in [language]:
- Feature: [description]
- Tool: Go runtime
...
```
Review this go goroutines code:
[paste code]
