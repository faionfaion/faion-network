# LLM Prompts for Error Handling

Effective prompts for AI-assisted development with error handling.
...
```
I need to implement error handling for my project.
...
2. Technology choices from: Sentry, Logging frameworks, Circuit breakers
...
```
Help me choose the best tool for error handling.
...
```
Generate error handling implementation for [language].

**Requirements:**
- Focus: Error types, recovery strategies, logging, monitoring
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing error handling code.
...
```
Analyze my error handling implementation for performance issues.
...
```
Refactor this error handling code for better maintainability.
...
```
I'm experiencing [issue] with error handling.
...
```
Review my error handling implementation for best practices.
...
```
Generate comprehensive tests for error handling.
...
```
What test scenarios should I cover for error handling?
...
```
Generate documentation for this error handling implementation.
...
```
Create step-by-step tutorial for implementing error handling.
...
```
Migrate error handling from [old tool] to [new tool].
...
```
1. "Design error handling for [project]"
   -> Get architecture
...
```
Generate error handling in [language]:
- Feature: [description]
- Tool: Sentry
