# LLM Prompts for Message Queues

Effective prompts for AI-assisted development with message queues.
...
```
I need to implement message queues for my project.
...
2. Technology choices from: RabbitMQ, Kafka, SQS, Redis Streams
...
```
Help me choose the best tool for message queues.
...
```
Generate message queues implementation for [language].

**Requirements:**
- Focus: Queue patterns, reliability, ordering
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing message queues code.
...
```
Analyze my message queues implementation for performance issues.
...
```
Refactor this message queues code for better maintainability.
...
```
I'm experiencing [issue] with message queues.
...
```
Review my message queues implementation for best practices.
...
```
Generate comprehensive tests for message queues.
...
```
What test scenarios should I cover for message queues?
...
```
Generate documentation for this message queues implementation.
...
```
Create step-by-step tutorial for implementing message queues.
...
```
Migrate message queues from [old tool] to [new tool].
...
```
1. "Design message queues for [project]"
   -> Get architecture
...
```
Generate message queues in [language]:
- Feature: [description]
- Tool: RabbitMQ
...
```
Review this message queues code:
[paste code]
