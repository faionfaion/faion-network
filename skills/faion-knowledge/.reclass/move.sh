...
SCRIPT_DIR=$REPO_ROOT/skills/faion-knowledge/.reclass
cd "$REPO_ROOT"
...
while IFS=$'\t' read -r current guess verdict src; do
  ...
  # src: skills/faion-knowledge/knowledge/<current>/<group>/<domain>/<methodology>
  rel=${src#skills/faion-knowledge/knowledge/}
  # rel: <current>/<group>/<domain>/<methodology>
  IFS='/' read -r _cur group domain methodology <<<"$rel"
  dst="skills/faion-knowledge/knowledge/$guess/$group/$domain/$methodology"
