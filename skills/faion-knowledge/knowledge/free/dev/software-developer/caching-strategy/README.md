...
Multi-level caching to reduce latency, decrease database load, and improve scalability.
...
- Understand invalidation needs (stale vs fresh)
- Size appropriately (memory is expensive)
- Monitor hit rates (low hit rate = wasted resources)
- Plan for cache failures (app must work without cache)

## Multi-Level Architecture

```
User → CDN → Application Cache → Distributed Cache → Database
     L1: Browser/CDN (static assets)
     L2: In-Memory (local app cache)
...
def cache_aside(key_prefix: str, ttl: int = 3600, key_builder=None):
    def decorator(func):
        ...
        wrapper.invalidate = lambda *args, **kwargs: redis_client.delete(
            f"{key_prefix}:{key_builder(*args, **kwargs)}" if key_builder else f"{key_prefix}:*"
        )
        return wrapper
    return decorator
...
class WriteBehindCache:
    ...
    async def _flush_all(self):
        for collection, docs in list(self.pending_writes.items()):
            ...
            operations = [
                UpdateOne({"_id": doc_id}, {"$set": doc_info["data"]}, upsert=True)
...
invalidator.set_with_tags(
    "product:123", json.dumps(product), 3600,
    tags=["products", "category:electronics"]
)

# Invalidate all related products
invalidator.invalidate_tags("category:electronics")

# Pattern 3: Version-based
def versioned_cache_key(entity: str, id: str) -> str:
    version = int(redis_client.get(f"cache:version:{entity}") or 0)
...
- Ignoring cache failures (app breaks when cache is down)
