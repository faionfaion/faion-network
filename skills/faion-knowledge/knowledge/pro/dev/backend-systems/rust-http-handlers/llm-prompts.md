# LLM Prompts for Rust HTTP Handlers

Effective prompts for AI-assisted development with rust http handlers.
...
```
I need to implement rust http handlers for my project.
...
2. Technology choices from: Axum, Actix-web, Warp
...
```
Help me choose the best tool for rust http handlers.
...
```
Generate rust http handlers implementation for [language].

**Requirements:**
- Focus: Handler patterns, extractors, responses
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing rust http handlers code.
...
```
Analyze my rust http handlers implementation for performance issues.
...
```
Refactor this rust http handlers code for better maintainability.
...
```
I'm experiencing [issue] with rust http handlers.
...
```
Review my rust http handlers implementation for best practices.
...
```
Generate comprehensive tests for rust http handlers.
...
```
What test scenarios should I cover for rust http handlers?
...
```
Generate documentation for this rust http handlers implementation.
...
```
Create step-by-step tutorial for implementing rust http handlers.
...
```
Migrate rust http handlers from [old tool] to [new tool].
...
```
1. "Design rust http handlers for [project]"
   -> Get architecture
...
```
Generate rust http handlers in [language]:
- Feature: [description]
- Tool: Axum
...
```
Review this rust http handlers code:
[paste code]
...
```
Debug rust http handlers issue:
Error: [message]
Code: [paste]
Context: [details]
