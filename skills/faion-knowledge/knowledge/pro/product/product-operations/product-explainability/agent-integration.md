...
- `faion-research-agent` (`skills/faion-knowledge/knowledge/pro/research/researcher/`) — runs the AI-probe stage: queries each AI surface with the canonical question set, captures responses, structures the diff. Reuse the existing `competitive-intelligence` mode template.
