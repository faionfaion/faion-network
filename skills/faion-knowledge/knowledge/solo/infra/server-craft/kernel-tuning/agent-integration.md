...
- `faion-knowledge` (infra/server-craft/swap-memory-management) — pairs with `vm.swappiness` and OOM tuning
- `faion-knowledge` (infra/server-craft/agent-dev-tuning) — inotify and file descriptor tuning for Claude Code workloads
- `faion-knowledge` (infra/server-craft/server-init-bootstrap) — kernel tuning is step 2 in bootstrap sequence
