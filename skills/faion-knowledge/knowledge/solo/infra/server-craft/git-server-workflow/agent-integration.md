...
- `faion-knowledge` (infra/server-craft/deploy-scripts) — the deploy.sh patterns that git-server-workflow triggers
- `faion-knowledge` (infra/server-craft/systemd-user-services) — services that get restarted after rsync
- `faion-knowledge` (infra/server-craft/agent-dev-tuning) — worktree-specific inotify and resource tuning for parallel agents
