...
- `faion-knowledge` (infra/server-craft/kernel-tuning) — inotify, sysctl, BBR for the underlying kernel
- `faion-knowledge` (infra/server-craft/swap-memory-management) — swap and MemoryMax for agent OOM protection
- `faion-knowledge` (infra/server-craft/systemd-user-services) — linger, per-service resource limits
- `faion-knowledge` (infra/server-craft/git-server-workflow) — worktree lifecycle for parallel agents
- `faion-knowledge` (infra/server-craft/tmux-power-user) — session persistence and multi-agent layout
