# LLM Prompts for Go Concurrency Patterns

Effective prompts for AI-assisted development with go concurrency patterns.
...
```
I need to implement go concurrency patterns for my project.
...
2. Technology choices from: sync package, Context, Errgroup
...
```
Help me choose the best tool for go concurrency patterns.
...
```
Generate go concurrency patterns implementation for [language].

**Requirements:**
- Focus: Goroutines, sync primitives, patterns
- Features:
  - [ ] [Feature 1]
  - [ ] [Feature 2]
  - [ ] [Feature 3]
...
```
Add [feature] to my existing go concurrency patterns code.
...
```
Analyze my go concurrency patterns implementation for performance issues.
...
```
Refactor this go concurrency patterns code for better maintainability.
...
```
I'm experiencing [issue] with go concurrency patterns.
...
```
Review my go concurrency patterns implementation for best practices.
...
```
Generate comprehensive tests for go concurrency patterns.
...
```
What test scenarios should I cover for go concurrency patterns?
...
```
Generate documentation for this go concurrency patterns implementation.
...
```
Create step-by-step tutorial for implementing go concurrency patterns.
...
```
Migrate go concurrency patterns from [old tool] to [new tool].
...
```
1. "Design go concurrency patterns for [project]"
   -> Get architecture
...
```
Generate go concurrency patterns in [language]:
- Feature: [description]
- Tool: sync package
...
```
Review this go concurrency patterns code:
[paste code]
...
```
Debug go concurrency patterns issue:
Error: [message]
Code: [paste]
Context: [details]
