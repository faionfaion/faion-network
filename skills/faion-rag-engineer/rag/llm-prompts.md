...
### Design Pattern
```
Prompt: Design a rag system that:
1. Handles {use_case}
...
### Code Generation
```
Prompt: Generate Python code for {specific_task} using rag.
...
Using rag, what's the issue and how to fix it?
```
...
### Test Generation
```
Prompt: Generate comprehensive tests for {function_name} that handles rag.
...
### Tutorial Generation
```
Prompt: Create a tutorial for implementing rag from scratch.
...
### Performance
```
Prompt: How can I improve the performance of {metric} in {language} for rag?
...
### Cost Optimization
```
Prompt: How to reduce API costs while maintaining quality for rag?
