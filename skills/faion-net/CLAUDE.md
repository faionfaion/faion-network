# Faion Network Orchestrator Skill

Universal orchestrator for end-to-end software project lifecycle. From idea to production, from research to marketing.

## Overview

This skill provides comprehensive coverage across 9 domains:
- **SDD** - Specification-Driven Development workflow
- **Research** - Idea generation, market research, validation
- **Product** - MVP/MLP planning, prioritization, roadmapping
- **Development** - Code generation, testing, API design, DevOps
- **Marketing** - GTM strategy, landing pages, SEO, ads, email
- **Project Management** - PMBOK 7/8, risk management, EVM
- **Business Analysis** - BABOK v3, requirements, traceability
- **UX/UI** - Research, usability testing, heuristic evaluation
- **AI/LLM** - RAG, embeddings, fine-tuning, multimodal, agents

## Key Statistics

| Metric | Count |
|--------|-------|
| Domain Skills | 13 |
| Agents | 60+ |
| Technical Skills | 40+ |
| Methodologies | 250+ |

## Directory Structure

```
faion-net/
├── SKILL.md                 # Main skill definition and capabilities
├── CLAUDE.md                # This file (navigation)
└── references/              # Domain-specific reference documentation
```

## Subfolders

| Folder | Description |
|--------|-------------|
| `references/` | Domain reference files covering workflow, directory structure, quality assurance, and all 9 knowledge domains |

## Key Files

| File | Purpose |
|------|---------|
| `SKILL.md` | Main skill definition with execution modes, capabilities, domain skills list, and 250+ methodologies catalog |
| `references/workflow.md` | SDD workflow phases (idea discovery through feature completion) |
| `references/directory-structure.md` | SDD folder layout and file purposes |
| `references/quality-assurance.md` | Confidence checks, hallucination prevention, reflexion learning |
| `references/*-domain.md` | Domain-specific agents, skills, and methodologies |

## Execution Modes

1. **Sub-agents (Recommended)** - Uses Task/Explore agents for complex tasks, parallel execution
2. **Main flow** - Direct execution in conversation for simple tasks, visibility

## Domain Skills

| Skill | Purpose |
|-------|---------|
| `faion-sdd` | Specs, designs, implementation plans, task lifecycle |
| `faion-feature-executor` | Sequential tasks with quality gates |
| `faion-researcher` | Market research, competitors, personas (9 modes) |
| `faion-product-manager` | MVP/MLP, RICE/MoSCoW, roadmaps (18 methodologies) |
| `faion-software-developer` | Python, JS/TS, APIs, DevOps (68 methodologies) |
| `faion-devops-engineer` | Docker, K8s, Terraform, CI/CD |
| `faion-ml-engineer` | LLM APIs, RAG, embeddings, fine-tuning |
| `faion-marketing-manager` | GTM, landing pages, SEO (72 methodologies) |
| `faion-project-manager` | PMBOK 7/8, risk, EVM (32 methodologies) |
| `faion-business-analyst` | BABOK v3, 6 Knowledge Areas (30 tasks) |
| `faion-ux-ui-designer` | Nielsen Norman 10, usability (32 methodologies) |
| `faion-claude-code` | Skills, agents, commands, MCP servers |

## Quick Reference

**Methodology Prefixes:**
- `M-RES-*` - Research (14 methodologies)
- `M-PRD-*` - Product (18 methodologies)
- `M-DEV-*` - Development (68 methodologies)
- `M-OPS-*` - DevOps (20 methodologies)
- `M-ML-*` - ML/AI (30 methodologies)
- `M-MKT-*` - Marketing (40 methodologies)
- `M-PM-*` - Project Management (20 methodologies)
- `M-PMT-*` - PM Tools (12 methodologies)
- `M-BA-*` - Business Analysis (12 methodologies)
- `M-UX-*` - UX/UI (32 methodologies)
- `M-SDD-*` - SDD (12 methodologies)
