...
| `regen-domains-xml.py [--write]` | L1 `domains.xml` + L2 knowledge `INDEX.xml` from `meta.json`. Dry-run by default. **Not yet a drop-in replacement** — it filters L2 by `status`, so a `draft` methodology drops out of the index while the manifest keeps it |
